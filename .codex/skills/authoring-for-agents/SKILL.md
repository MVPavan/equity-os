...
| The fix is… | Surface |
|---|---|
| A guardrail every session must hold | rule under `.codex/rules/` |
| Repo orientation: purpose, read order, verification commands | `AGENTS.md` / `CLAUDE.md` — short, pointing at detail elsewhere |
| A durable repo fact, decision, or verified pattern | `.codex/project/` overlay or learnings |
...
- Make the result discoverable on its own surface — a skill through its
  ...
  `.codex/project/docs-index.md`. Touch the root read order only for material
  every session needs.
