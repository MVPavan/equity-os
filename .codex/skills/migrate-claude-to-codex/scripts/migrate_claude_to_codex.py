...
prefix_rule(
    ...
    decision = "forbidden",
    justification = "Do not destroy local worktree or history state.",
)

prefix_rule(
    pattern = ["git", "clean"],
    decision = "forbidden",
    justification = "Do not remove untracked files without explicit approval.",
)
...
prefix_rule(
    ...
    decision = "forbidden",
    justification = "Do not force-delete branches.",
)

prefix_rule(
    pattern = ["git", ["checkout", "restore"], "."],
    decision = "forbidden",
    justification = "Do not rewrite the whole worktree.",
)

prefix_rule(
    pattern = ["bd", "init", ["--force", "--reinit-local"]],
    decision = "forbidden",
    justification = "Do not reinitialize the Beads store.",
)
"""
...
def normalize_text(text: str) -> str:
    ...
    return re.sub(
        r"\.codex/skills/([a-z0-9][a-z0-9-]*)\.md",
...
def copy_tree(
    ...
    *,
    ...
    for path in sorted(src.rglob("*")):
        ...
        if (
            rel.parts
            and rel.parts[0] in skipped
            and rel.as_posix() == f"{rel.parts[0]}/SKILL.md"
        ):
            if rel.parts[0] not in announced:
                report.skip(
                    f"{src / rel.parts[0]} shadowed by an existing .agents/skills skill"
                )
                announced.add(rel.parts[0])
            continue
...
def infer_sandbox(name: str, meta: dict[str, Any], body: str) -> str:
    ...
    if isinstance(tools, list) and tools:
        tool_set = set(tools)
        if tool_set.intersection({"Write", "Edit", "NotebookEdit"}):
            return "workspace-write"
        if tool_set.issubset({"Read", "Grep", "Glob", "WebFetch", "WebSearch"}):
            return "read-only"
    if name in {"implementer", "fixer", "worker"}:
        return "workspace-write"
...
def migrate(args: argparse.Namespace) -> int:
    ...
    shadowed_skills = (
        {
            ...
            if path.is_dir() and (agents_skills / path.name / "SKILL.md").exists()
        }
    ...
    copy_tree(
        ...
        force=args.force,
        report=report,
        skip_top_level=shadowed_skills,
    )
...
def verify(args: argparse.Namespace) -> int:
    ...
    codex_skill_names: dict[str, Path] = {}
    for skill in sorted((repo / ".codex" / "skills").glob("*/SKILL.md")):
        ...
        codex_skill_names[name] = skill
    for skill in sorted((repo / ".agents" / "skills").glob("*/SKILL.md")):
        meta, _ = split_frontmatter(skill.read_text(encoding="utf-8"))
        name = str(meta.get("name") or skill.parent.name)
        if name in codex_skill_names:
            errors.append(
                f"duplicate skill {name!r}: {codex_skill_names[name]} and {skill}"
            )
    for hook_script in sorted((repo / ".codex" / "hooks").glob("*.sh")):
        if not os.access(hook_script, os.X_OK):
            errors.append(f"{hook_script}: hook script is not executable")
