...
Inherit the session model and reasoning effort unless the user, `AGENTS.md`,
or the active skill explicitly requires an override. Prefer role-specific
instructions and clear task boundaries over speculative model pinning.
...
1. **Extract the brief**:
   `bash .codex/skills/execution/scripts/task-brief.sh <plan> <N> <workspace>`
...
binding constraints and `.codex/project/invariants.md`. Contradictions and
...
3. **Independent Codex review** uses the native `code-reviewer` role or a
   read-only subagent pointed at the same package and ledger. Capacity policy
   per `AGENTS.md`; do not route through a Claude-side adapter.
