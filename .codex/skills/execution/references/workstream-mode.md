...
> **Sequential by design.** One phase at a time, with compaction or a fresh
> resumed context between phases when the runtime supports it. Phases walk in
> roadmap order even when deps would allow
...
4. After the phase passes its gate + exit criterion:
   - render: `BD_RENDER=1 bash .codex/skills/beads/scripts/bd-render-tracking.sh <name>`
...
5. After compaction or context restart, **re-query bd** (`bd epic status --json`, `bd ready`) and
...
- Compact between phases when the runtime supports it; never assume conversation
  memory is the source of truth.
- Within a large phase, compact between stages when useful; then re-read the workspace
