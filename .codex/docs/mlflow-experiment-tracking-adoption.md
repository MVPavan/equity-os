...
Create these two files **under `.codex/`** — Claude Code reads skills from `.codex/skills/` and project
policy from `.codex/project/`. Do **not** put them under `.agents/` or a repo-root location. The skill is
auto-discovered and points at the policy so the top-level instruction files stay clean.

### `.codex/skills/experiment-tracking/SKILL.md`
...
Policy & conventions: `.codex/project/tracking.md`.
