...
> Staging doc. Captures live findings from the 2026-04-15 session, compares against prior-session learnings, and proposes a layered design. Not authoritative. Final version will land in `.codex/skills/use-codex.md` (the command) and `.codex/docs/codex-usage-guide.md` (the deep reference) once this thread closes.
...
Main Claude thread (coordinator)
 ...
 │            └── briefed with "read .codex/skills/use-codex.md, then run it"
...
Agent({
  ...
  prompt: "Read .codex/skills/use-codex.md for invocation rules.
...
### Layer 1 — `.codex/docs/codex-usage-guide.md` (deep reference)
...
### Layer 2 — `.codex/project/learnings.md`

Two entries:
- "Codex: Skill path is banned — zombie state blocks it silently. Use subagent or Bash. See `.codex/skills/use-codex.md`."
- "Codex: one heavyweight job at a time on shared runtime; `--effort minimal` is broken (use `low` floor)."

### Layer 3 — `.codex/skills/use-codex.md` (new, authoritative)
...
> For any Codex invocation, follow `.codex/skills/use-codex.md`.
