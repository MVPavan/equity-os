...
the `.beads/beads.md` policy (§4), and the `.codex/` skill + hook (§5–§6).
...
## 5. Skill — `.codex/skills/beads/SKILL.md`

> **Location matters — install agent config under `.codex/`, never `.agents/`.** Claude Code discovers
> skills from `.codex/skills/`, hooks from `.codex/hooks/`, and hook registration from
> `.codex/settings.json`. Put the beads skill at **`.codex/skills/beads/SKILL.md`** (one folder per
...
> canonical skill lives under `.codex/skills/`. (Some repos keep an `.agents/` placeholder that only
> documents that everything real lives in `.codex/`; do not put skills there.)

Create this triggerable skill (auto-discovered from `.codex/skills/`):
...
Create `.codex/hooks/bd-prime.sh` (self-locates `bd`; degrades to a silent no-op if absent — it must
...
```bash
chmod +x .codex/hooks/bd-prime.sh
```

Register it in `.codex/settings.json`. If the file already exists, **merge** this `SessionStart` entry
    ...
    "SessionStart": [
      {
        "matcher": "",
        "hooks": [
          { "type": "command", "command": "\"$CLAUDE_PROJECT_DIR\"/.codex/hooks/bd-prime.sh" }
        ]
      }
    ]
  }
