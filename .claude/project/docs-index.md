...
| `docs/research/upstox-lane-b-first-measurement.md` | **Before trusting or extending the Lane B name map.** The first end-to-end crosscheck over real data from both vendors (replayed, zero live requests): the two comparator defects it found, the diagnostic rule that a summary/full_statement anomaly plus a Screener mismatch on the same line localizes the fault to Upstox, and why Screener's `Total Liabilities` row is not liabilities |
