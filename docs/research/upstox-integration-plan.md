...
|---|---|---|---|
| A | What happens when Lane B finds a mismatch | **Log only.** Record it; nothing downstream changes | Chosen over warn/block because the base disagreement rate is unmeasured. Blocking on an unknown rate either halts the pipeline or gets switched off within a day. Promote to warn or block *after* Slice 6 step 5 reports the number. **Promoted 2026-09-04 (step 5(c)): per-field WARN classes in `config/laneb_triage.yaml`, exit still 0 by default, opt-in `--warn-exit`, no block — see the measurement doc Part 4.** |
