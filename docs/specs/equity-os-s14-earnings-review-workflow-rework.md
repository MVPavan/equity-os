...
4. Rejected-claim correction before or after approval/publication, immutable successor versions, evidence-package versioning, dependency-aware invalidation, minimal rerun, partial revalidation, and reapproval.
...
| Approval | `HUMAN_REVIEW → APPROVED` | A current S15 `ReviewDecision` has `decision=ACCEPT` and `approval_type=ANALYST_ACCEPTANCE`, binds the exact immutable `ReviewTask`, reviewed artifact/claim bytes, `PRODUCTION` mode, eligible provenance, and declared scope, and has no later revocation, expiration, supersession, or invalidation. |
...
| Post-approval correction | `APPROVED → INGESTED`, `EXTRACTED`, `RECONCILED`, `CALCULATED`, or `DRAFTED` | A current content-bound `REJECT` identifies the earliest affected step, or `EDIT` targets `DRAFTED`; its exact `POST_APPROVAL_CORRECTION` task/decision pair matches the currently approved production artifact, mode, provenance, and scope; and the correction atomically binds an immutable successor request/version, the invalidated prior acceptance, the exact invalidation closure, and evidence package `v(N+1)`. |
| Post-publication correction | `PUBLISHED → INGESTED`, `EXTRACTED`, `RECONCILED`, `CALCULATED`, or `DRAFTED` | The post-approval predicate holds and the transition additionally binds every successful receipt for the invalidated bytes. Each receipt remains immutable evidence that publication occurred but becomes non-current for publication authority. |

A rework target names the logical step that must execute next; it does not assert that the target step has already produced a valid replacement output. Its transition record must preserve `rework_resume_target=target`. After that new attempt commits, the run follows the declared forward edges. This rule applies equally to `HUMAN_REVIEW`, `APPROVED`, and `PUBLISHED` correction origins. A correction never rewrites the reviewed or published artifact: S15 supplies an immutable correction request/version and the corrected output becomes its immutable successor. `DEFER` records a review outcome without changing `HUMAN_REVIEW`.

For any otherwise allowed non-genesis base edge `from → target`, an attempt may record `from → FAILED` instead of committing `target`; its transition record must preserve `resume_target=target`. `FAILED` may transition only to that exact `resume_target` through a new idempotent attempt, or to `BLOCKED` with the same target. If required input, evidence, or authority is unavailable before an allowed non-genesis edge, `from → BLOCKED` is legal only with `blocked_resume_target=target`; after the recorded blocker is resolved, `BLOCKED` may transition only to that target. Thus failure and blocking edges are derived mechanically from the closed non-genesis base-edge set rather than selected freely. A derived `FAILED` or `BLOCKED` correction from `APPROVED` or `PUBLISHED` retains every base-edge requirement for the exact correction task/decision, successor request, invalidation closure, prior acceptance, `v(N+1)`, and affected receipts; later resume cannot make any invalidated authority current. Invalid or incomplete registration creates no valid run or initial state; it does not create a pre-registration `FAILED` or `BLOCKED` transition.

Human outcomes are:

- `ACCEPT`: advance a content-bound reviewed version toward `APPROVED`.
- `REJECT`: create a correction request and enter the rework algorithm below, including from previously `APPROVED` or `PUBLISHED` bytes.
- `EDIT`: create a new drafted successor version, invalidate affected downstream review/approval/publication authority, and require review of the new bytes.
...
Contains stable transition ID, run ID, prior-transition digest, `from_state`, `to_state`, transition kind, workflow-definition version, triggering attempt/output references, exact input and output digests, review-task ID/digest, review-outcome/approval type/decision ID/resolution digest when applicable, correction-successor and invalidation references, invalidated/superseded approval decision references, invalidated publication-receipt references, evidence-package ID/version/digest, `resume_target`, `blocked_resume_target`, or `rework_resume_target` when applicable, blocker and resolution evidence, actor/authority reference when applicable, transition time, and `transition_digest`. Fields that do not apply are explicit `null`, not omitted.
...
An approval edge must bind the exact S15 `ReviewTask` ID/digest and `ReviewDecision` ID/digest, typed `ANALYST_ACCEPTANCE`, reviewed artifact/claim hashes, `PRODUCTION` mode, provenance, and scope. A rework edge must bind the exact `REJECT` or `EDIT` decision plus correction-successor, invalidation, and new evidence-package records, with `rework_resume_target=to_state`. A rework edge from `APPROVED` or `PUBLISHED` must additionally bind the prior acceptance it makes non-current; a `PUBLISHED` edge must bind every affected successful receipt. `FAILED` and `BLOCKED` records must bind the exact target they preserve. A copied outcome string, bare task ID, unbound resolution, inferred mode/provenance, or inferred resume target is invalid.
...
S14 consumes the content-bound human decision contract supplied by S15: decision ID and type, typed approval when applicable, exact immutable review-task ID/digest, reviewed artifact/claim version and hash, artifact mode and provenance, actor and human actor type, authority basis, exact scope, timestamp, rationale where required, evidence, and immutable resolution digest. Before using it in a transition, S14 must obtain both complete canonical records, recognize `equity-os.s15.review-task.v1` and `equity-os.s15.review-decision.v1`, independently recompute both digests under S15 §3.8, resolve every provenance/evidence reference, and require exact agreement between task, decision, artifact, and requested transition. Unavailable canonical bytes, an unknown contract/domain version, omitted explicit null, extra member, digest mismatch, stale/revoked/expired/superseded authority, changed or claim-only scope, non-production mode, ineligible provenance, or a copied task/decision string blocks approval or publication. `REJECT` and `EDIT` may drive their exact correction transitions but never supply `ANALYST_ACCEPTANCE`.
...
`PublicationEligibilityResult` contains a stable result ID and binds the approved artifact ID/version/hash and accepted scope; current S15 `ReviewTask` ID/content digest, artifact mode, and provenance references; current `ANALYST_ACCEPTANCE` decision ID and resolution digest; target ID/version/digest; S01 `OperatingBoundary` ID/version/digest and its exact current `PRODUCT_OWNER_DECISION` ID/resolution digest; evaluator version, evaluated time, reasons, a three-valued result (`TRUE`, `FALSE`, or `UNKNOWN`), and `result_digest`.

The eligibility predicate is `TRUE` only when all of the following hold for the exact bound bytes:

1. The run's chained state is `APPROVED`; the complete task and decision bytes recompute; the decision has `decision=ACCEPT` and `approval_type=ANALYST_ACCEPTANCE`; task, decision, and artifact bind the same exact bytes, version, publication-capable report/update scope, `PRODUCTION` mode, and current eligible provenance; the acceptance is not revoked, expired, superseded, or invalidated; and no invalidation, review, failure, or blocker remains unresolved.
...
4. Every bound task, decision, artifact, provenance/evidence reference, ID, version, scope, authority record, and digest recomputes and matches.
...
Contains stable receipt ID, approved artifact ID/version/hash and accepted scope, current review-task ID/content digest, `PRODUCTION` mode and provenance references, current analyst-acceptance decision ID/resolution digest, run/evidence-package/thesis versions, publication-target ID/version/digest, `PublicationEligibilityResult` ID/digest, operating-boundary ID/version/digest, publication idempotency key, commit timestamp, atomic publication result, and `receipt_digest`. A successful receipt may be created only for `APPROVED → PUBLISHED` with the bound eligibility result equal to `TRUE`; a failed or partial publication remains non-published and retryable under the same idempotency contract. The receipt digest uses the same rule with domain separator `equity-os.s14.publication-receipt.v1` and excludes only `receipt_digest` from its canonical JSON preimage.

A successful receipt is immutable evidence that its exact bytes were published. It is current publication authority only while its task, typed production acceptance, artifact, target, boundary, and eligibility result remain current and no later valid transition invalidates or supersedes them. A post-publication correction preserves the receipt but names it in the correction transition's invalidation closure; it can never authorize the corrected successor or be treated as proof that the successor was published. Republication of corrected bytes requires a new acceptance, eligibility result, transition, idempotency key, and receipt.

## 5. Rework algorithm

For a rejected claim or a correction discovered after `APPROVED` or `PUBLISHED`, the implementation must perform these steps transactionally or with recoverable checkpoints:

1. Persist the immutable rejection/edit decision and correction request; record whether the current origin is `HUMAN_REVIEW`, `APPROVED`, or `PUBLISHED`.
...
5. Append the exact `origin → target` rework transition binding the decision, correction-successor request/version, invalidation set, and `v(N+1)` digests. From `APPROVED`, include the prior acceptance; from `PUBLISHED`, include that acceptance and every affected successful receipt. Persist these records atomically so no chain head can retain current approval/publication authority after its invalidation is committed.
...
10. Commit the corrected artifact as an immutable successor of the prior artifact, create a new immutable production `ReviewTask` for those exact bytes, and obtain a new typed content-bound human reapproval; prior task, acceptance, or receipt cannot advance the run.
11. Evaluate the publication predicate and publish only after all invalidation records are resolved, the corrected run is newly `APPROVED`, and the exact target is eligible. Previously published bytes remain auditable; the corrected successor is not published until its own receipt commits.
...
9. A rejection or post-approval/post-publication correction invalidates the exact affected closure. No invalidated calculation, claim, draft, review task, approval, eligibility result, or publication receipt remains current; every record remains immutable and auditable.
...
12. Publication consumes the current accepted S01 boundary and a current exact-scope typed `ANALYST_ACCEPTANCE` for a production task whose mode/provenance is content-bound. A target outside the boundary, a shadow/golden or wrong-scope task, or any target whose eligibility is not exactly `TRUE` cannot reach `PUBLISHED` or receive a successful receipt.
13. A correction after `APPROVED` or `PUBLISHED` must use a declared correction edge and immutable successor. Once its invalidation transition commits, stale acceptance or receipt evidence cannot preserve, restore, or transfer current authority.
...
| Approve each Quarter 1–3 update | `ANALYST_ACCEPTANCE` | Competent human analyst | Immutable review-task ID/digest, exact production report/thesis/evidence-package hashes, reviewed claim set, mode/provenance/scope, actor authority basis, decision, timestamp, and evidence | No current exact-task typed production approval record means no `APPROVED` or `PUBLISHED` transition. |
| Reapprove after rework | `ANALYST_ACCEPTANCE` | Competent human analyst | New immutable task/digest, corrected production bytes/mode/provenance/scope, resolved invalidation closure, partial/full validation evidence, superseded prior decision, actor authority basis, decision, timestamp, and digest | Prior task, approval, or receipt never carries across changed bytes or package versions. |
...
|---|---|---|
| S14-T01 Legal transitions and genesis | Create a run with one valid atomic `null → REGISTERED` genesis, then exercise absent, duplicate, non-first, non-null-prior, run/workflow/digest-mismatched, and registration-prerequisite-failing genesis cases; enumerate every later base edge, derived `FAILED`/`BLOCKED` edge, all five correction targets from each of `HUMAN_REVIEW`, `APPROVED`, and `PUBLISHED`, skips, regressions, target substitutions, digest-chain tampering, and unknown states. | Only one first content-bound genesis plus the closed later graph passes; invalid registration creates no run, the first forward edge proves `REGISTERED` from genesis, failure/block records resume only their bound target, terminal-state correction requires the bound invalidation/successor records, and all other cases fail closed. |
...
| S14-T07 Post-approval/publication correction | Correct once from `APPROVED` and once from `PUBLISHED`; omit or mismatch the prior acceptance, receipt, invalidation, successor, and `v(N+1)` bindings in turn. | Only the fully bound declared correction edge passes; old approval/receipt remains immutable but non-current, corrected bytes are an immutable successor, and approval/publication block until new exact-byte acceptance and, when republished, a new receipt. |
...
| S14-T12 Review-task/decision canonical bytes | Attempt approval and rework with the exact S15 canonical task and decision, unavailable task or decision bytes, an unknown/wrong domain version, an omitted explicit-null member, an extra member, reordered object keys, a bare copied task ID, and a changed field without a matching digest. | Reordered keys verify to the same RFC 8785 bytes; only the complete exact recognized matching task/decision pair with recomputed digests may drive its declared transition, and every unverifiable case blocks. |
| S14-T13 Typed production publication | Attempt publication with production, shadow, and golden tasks; claim-only and report/update scopes; clean, missing, stale, test-contaminated, and mismatched provenance; and `ACCEPT` decisions with `ANALYST_ACCEPTANCE`, null, or wrong approval type. | Only the current exact report/update-scope `ACCEPT` carrying typed `ANALYST_ACCEPTANCE` for matching `PRODUCTION` task bytes and eligible provenance can yield `TRUE`; every other fixture blocks without a successful receipt. |
