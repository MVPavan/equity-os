...
Each processing state may transition to `FAILED` for its current attempt. `FAILED` may transition only back to the same logical step through a new idempotent attempt, or to `BLOCKED` when required input, evidence, or authority is unavailable. `BLOCKED` returns only to the blocked step after the blocker is resolved with evidence.
...
No other transition is legal. A state label alone is not proof: each transition requires its declared immutable input/output references and actor/authority where applicable.

## 4. Interfaces and data contracts

### 4.1 `RunRecord`

Contains stable `run_id`, discovery company, quarter, run cutoff, source/evidence-package version, preceding approved thesis ID and content hash, workflow-definition version, current logical state, created time, and ordered transition references. Quarters 1–3 must name the immediately preceding approved thesis; absence or hash mismatch blocks registration.
...
### 4.5 `DependencyEdge` and `InvalidationRecord`
...
S14 consumes the content-bound human decision contract supplied by S15: decision, exact reviewed artifact/claim version and hash, actor, authority basis, timestamp, rationale where required, evidence, and immutable resolution digest. A copied decision string is not authority.

### 4.7 `PublicationReceipt`

Contains the approved artifact ID/hash, approval-record reference, run/evidence-package/thesis versions, publication target, timestamp, and atomic publication result. It may be created only from `APPROVED`; a failed or partial publication remains non-published and retryable under the same idempotency contract.
...
5. Re-enter only the earliest affected step with a new attempt and exact new input digest.
6. Reuse unaffected outputs only after their dependency digests still match.
7. Rerun affected calculations, claims, and draft sections; never reuse an invalidated output.
8. Partially revalidate the affected closure plus boundary links to retained outputs.
9. Obtain a new human reapproval bound to the corrected bytes.
10. Publish only after all invalidation records are resolved and the corrected run is `APPROVED`.
...
S14 is `active-only` at the pinned draft snapshot because B-01, B-02, and B-14 are all `Open`; it owns no `Deferred` register row. This contract activates no deferred capability. If an owned row becomes deferred or ownership expands to deferred scope, work stops pending authority reconciliation, amendment, and fresh review.
...
| S14-T04 Rejected extraction | Reject a claim caused by extraction error. | `v(N+1)` is created; affected reconciliation/calculation/claim/draft/review are invalidated and rerun; unrelated outputs remain only with matching dependencies. |
