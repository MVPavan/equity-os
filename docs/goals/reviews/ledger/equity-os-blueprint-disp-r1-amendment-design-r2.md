...
**Supersedes** `docs/goals/reviews/ledger/equity-os-blueprint-disp-r1-amendment-design-r1.md`
(SHA-256 `fd00a14ae7dcc3e7aa6854d307f46a7f9503278ec8c49f77a388a7b1d1ab75ee`),
which is **BLOCKED** and must not be executed. r1 in turn superseded
`…-disp-r1-amendment-design-r0.md`
(SHA-256 `675cb4877d9eef6b49ea8b825c8dc11fa9f1b5363e88df0dbc657e3d52727326`),
also **BLOCKED**.

Predetermined independent review path (r1 round, unchanged text carried
forward):
`docs/goals/reviews/ledger/equity-os-blueprint-disp-r1-amendment-design-r1-review-r0.md`
— **this path, and every other `-design-r1` path in §4.4, §5.3 and §7, still
names the superseded r1 round. See §9.2 open question 6: they must be
re-pointed to this document and to its own independent review before the §7
question is asked.**
...
The clean demonstration is the reviewer's **E2**: an entry that is byte-identical
to the conforming `HR-0005` except that it is named `HR-0006` — correctly
scoped, correctly resolved, correctly linked — is rejected at candidate `:2820`,
span D's first assert, because `amendment_overlap` is empty for any entry other
than `HR-0005` and `DISP-R-1` is therefore an unpermitted member of
`overlapping`. The reviewer's **E1** (entry with no resolution and no link) is
rejected at `:1209`. **E1/E2 are the independent reviewer's constructions, not
mine; I did not re-run them for r2.** H1 and H2 remain listed in §3.8 as
rejected attacks — they are, just not by span D.
...
Per **M3**: the reviewer's independently built H1/H2 are rejected earlier, at
`:1198`/`:1209`, so H1/H2 must not be read as evidence that span D's scope
assertion fires. The entry-identity demonstration of span D is the reviewer's
**E2** (`HR-0006`, otherwise byte-identical) at candidate `:2820` — see §3.5.
...
6. **This document's own path is not yet reflected in the artifacts it binds —
   raised by r2, not by the review.** Correcting F1 supersedes r1, but §4.4,
   §5.3 item 2 and the §7 question still name
   `…-disp-r1-amendment-design-r1.md` and its predetermined review
   `…-disp-r1-amendment-design-r1-review-r0.md`. Under the review's own F1
   reasoning — the approval binds the design's bytes, so the corrected design is
   what must be bound — `HR-EV-0005-DESIGN` must point at **this** document and
   `HR-EV-0005-REVIEW` at **this** document's independent review.
   I have **not** rewritten those paths: the r2 review's filename is an
   orchestrator decision (r1's own predetermined path was never used — the r1
   review landed at `scratchpad/disp-r1/review/design-r1-findings.md`), and
   guessing it would fabricate a binding. *What must happen:* whoever finalises
   the package fixes both paths in §4.4, §5.3 and §7 to the actual r2 design and
   r2 review paths **before** the §7 question is asked. **This is a blocking
   precondition, not a nicety** — the placeholder digests
   `<DISP_R1_DESIGN_SHA256>` / `<DISP_R1_REVIEW_SHA256>` are computed over
   whatever those paths name.
...
**Raised by r2, not by the review:** §9.2 open question 6 — §4.4, §5.3 and §7
still name the superseded `-design-r1` paths, which must be re-pointed to this
document and its own review before the §7 question is asked.
