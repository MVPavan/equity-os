# Ledger authority and remediation design r4

Next action: obtain one clean independent exact-byte Sol xhigh review of this
file at the predetermined path
`docs/goals/reviews/ledger/equity-os-blueprint-ledger-remediation-design-r4-review-r0.md`.
The review must declare `gpt-5.6-sol`, `xhigh`, verdict `CLEAN`, and a reviewed-
input SHA-256 equal to this file's freshly computed SHA-256. If and only if all
bindings validate, the orchestrator may ask the exact decision question in
[Pre-approval package](#pre-approval-package). No canonical mutation, HR-0004
entry, resolution, migration code, Beads change, staging, commit, or push is
authorized before the current user answers that completed question
affirmatively.

## 1. Status, failure corrected, and authority boundary

This document starts from the exact r3 design whose SHA-256 is
`ff269d9ed19909b252902ef427bca4ad420a6efa7b8ed019c205b05676a591d0` and
supersedes its operational design. It preserves every mechanically verified r3
non-finding and corrects exactly the three findings in the independent r3
review whose SHA-256 is
`de4272fc84887f169020b4cf0f448b66688e17e7ff7ac5bbb5412e17ec043816`.
It does not approve any earlier revision, amend the active goal, or authorize a
migration:

1. **Approval binds the exact independent review.** The literal question binds
   this r4 path and SHA-256, the predetermined review path and SHA-256, the
   review's `CLEAN` verdict, its reviewed-input SHA equal to the r4 SHA, and the
   `gpt-5.6-sol` `xhigh` reviewer lane. The completed question and response bytes
   are recorded after approval, and structural reconciliation verifies every
   bound review field against the immutable review artifact before any write.
2. **Stale rejection evidence is not current proof.** `DISP-R-1` retains its
   ...
   proof exists only after satisfied requirement coverage and a fresh content-
   bound clean Sol xhigh evidence review over the current bytes.
...
The protections carried forward into r4 also remain binding: the canonical
goal, ledger, and human-review artifact stay byte-identical through independent
review and the user's decision; HR-0004 is created and resolved only inside the
later transaction;
and its exact structured scope remains 144 IDs, comprising 141 affected
existing IDs plus the three new IDs. No later scope expansion is permitted.
...
- `rejection_record.no_implementation_evidence_ref_ids` is an immutable
  ...
  current content-bound `COMPLETE`/`CLEAN` `gpt-5.6-sol` `xhigh` review whose
  evidence refs include every historical ref and whose timestamp is no earlier
  than their current captures. Its reviewed-input and reviewed-inventory
  digests must equal the validator's current projections. False is a valid
  structural state, but it is an explicit preimplementation and terminal
  blocker. No description/scope substring or refreshed content digest may
  substitute for this closed predicate.
...
1. the exact r4 path and its freshly computed full SHA-256, substituted
   externally for every `<R4_SHA256>` in the literal question without editing
   this file;
...
3. the predetermined independent review path
   ...
   `<R4_REVIEW_SHA256>`, reviewer/session identity, UTC timestamp, and its exact
   binding values: reviewed r4 path, reviewed-input SHA equal to `<R4_SHA256>`,
   reviewer model `gpt-5.6-sol`, effort `xhigh`, and verdict `CLEAN`;
4. the exact 144-ID scope, count, and canonical-JSON digest
   `bf6fee00d0f4510316b42b50ec13f74148df9ed44e472f2ad8be114ee3add894`;
   and
5. the following exact user-facing question with both placeholders filled only
   in the externally presented copy after the review exists.

The immutable review artifact must contain exactly one unambiguous binding for
the reviewed design path, reviewed-input SHA-256, reviewer model, reviewer
effort, and verdict, with the values in item 3 and no conflicting duplicate.
Its own SHA-256 is computed only after those review bytes are final. This r4
file retains placeholders, so neither the design nor the review hashes itself.
...
conversation/goal-tool identifier, the r4 path/SHA, review path/SHA/verdict/
reviewed-input SHA/model/effort, scope digest, and all five pre-state hashes.
The completed question bytes themselves contain those same immutable review
bindings, so the stored question/response pair proves which exact review the
user approved. The HR-0004 entry and resolution use a
...
Before any canonical write, structural reconciliation reads the fixed r4 and
review paths, hashes their exact bytes, parses the review's unique binding, and
compares the actual design SHA, review SHA, verdict, reviewed-input SHA, model,
and effort with both the completed question and the candidate goal record. It
also requires the reviewed-input SHA to equal the actual r4 SHA. A missing,
ambiguous, changed, substituted, non-`CLEAN`, wrong-input, or wrong-lane review
...
review artifacts including this r4 file and its predetermined independent
review, every
...
2. Read and retain every preimage byte string in memory and in a private
   ...
   full r4 digest, predetermined clean-review path/digest/verdict/reviewed-input
   SHA/model/effort, exact completed question and response bytes, exact 144-ID
   manifest/digest, authority hashes, expected Git-index bytes, and the allowed-
   path dirty-tree baseline. The r4/review bindings must also pass the
   independent structural reconciliation comparison in §5.3. Any mismatch
...
1. **Recheck authority.** Verify every §2 prehash, the exact r4 and predetermined
   review bindings, completed approval question/response bytes, exact allowed
   path set, unchanged Git index, and unchanged unrelated dirty baseline under
   the exclusive lock.
...
- Terminal: required `--repo-root`, the same paired candidate paths as
  ...
  conditions as JSON with exit 2; it never grants a terminal state. Rejected
  state is accounted only after the record-authority checks; it does not waive
  the separate current no-implementation-proof predicate.
...
and the 454-history prefix invariant. In reconciliation mode it also owns the
exact r4/review binding comparison from §5.3, the exact
`DISP-R-1 -> REQ-DISP-R-1-NO-IMPLEMENTATION` proof map, and the rule that false
current proof is structurally valid. It requires the §3.6 unresolved object and
false current-proof result in this post-state. It never imports generator
constants.
...
`CURRENT_SOL_XHIGH_EVIDENCE_REVIEW_MISSING`. Digest refresh alone may not remove
either reason code.
...
exit-2 not-ready result and required `DISP-R-1` blocker object.
...
- canonical posthashes equal the prepared candidate hashes and differ from a
  ...
  the actual r4/review paths, byte hashes, `CLEAN` verdict, reviewed-input SHA,
  model, and effort equal every value bound in the completed question, response
  record, and structural reconciliation result;
...
- HR-0004 has exactly one active `RECONCILE_AUTHORITY` resolution, exact
  144-ID structured scope, exact approval/review/r4/prehash evidence, and valid
...
- r4 path/SHA, predetermined review path/SHA, parsed `CLEAN` verdict, reviewed-
  input SHA, reviewer model/effort, exact completed question and response bytes/
  digests, HR entry/resolution digests, and expanded 144-ID list/digest;
...
1. Commission one clean, independent, exact-byte Sol xhigh review of this r4
   file at the predetermined r4-review-r0 path. Any finding or binding mismatch
   stops the lane and requires a new reviewed design revision.
2. After `CLEAN`, compute the r4, review, and scope digests; externally fill only
   the two §5.2 placeholders; and ask that completed question verbatim. Preserve
   all canonical pre-state bytes while waiting.
...
exact r4 SHA and a fresh current-user approval explicitly bound to that review
and the completed §5 question. A failed precondition, changed r4 or review byte,
non-clean/wrong-input/wrong-lane review, incomplete answer, missing exact-message
evidence, validator failure, or rollback uncertainty ends the attempt fail-
closed with canonical pre-state restored.
