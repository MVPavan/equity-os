# Inventory-review recording design — r3
...
### 0.0 r2 → r3 — why this round exists (bead `eqos-w78`)

**Changelog.** r3 amended: closed label-alias table (Orchestrator decision).
The amendment is in place — this document is still r3 and is still UNREVIEWED —
and is confined to this §0.0 (change 4 and the "Measured effect" paragraph) and
to §2.2.1 (the label alias table, the three-cell row shape, the per-field
one-accepted-row rule, and the two renamed skip reasons). No other section
moved.
...
### 0.0a Round lineage below this point
...
| `captured_at` | UTC RFC3339, `<=` the review `timestamp`. **r3 §2.2.1: derived — set equal to the review `timestamp`. Never read from the artifact and never declared in the manifest.** |
...
*(New in r3, per §0.0. This subsection is the **whole** contract between a
verdict artifact and the recorder. Anything not stated here is not parsed.)*
...
`role_binding_sha256` ← column 3. This is the only three-cell shape read;
`| Role binding | CONTEXT.md | 8f2795af… |` appears in the input-hash tables of
58 of the 74 prose-cohort artifacts, always agreeing with that artifact's own
two-cell rows. Every other three-cell row (`| Active goal | <path> | <sha> |`,
...
in the 444 artifacts on disk (survey: `scratchpad/w78/alias-survey.txt`), and
...
| `verdict` | `verdict` | 2-cell (the verdict line is the usual carrier) |
...
is a check on the body, never a source for it.
...
**Value normalization.** In both forms, in this order:
...
3. For `review_round` only: the value must match `^r?(\d+)$` and normalizes to
...
**The verdict line.** A line matching

```
^[ \t]*(?:\*\*)?[ \t]*[Vv][Ee][Rr][Dd][Ii][Cc][Tt][ \t]*:[ \t]*(?P<verdict>[A-Za-z0-9_]+)[ \t]*(?:\*\*)?[ \t]*$
```

sets `verdict`. The word *verdict* is matched **case-insensitively** and may be
bold-wrapped (`**verdict: CLEAN**`, `**Verdict: CLEAN**`, `verdict: CLEAN` are
all the same line); the **value is taken exact-case** and is never
case-folded — `CLEAN` is the only recordable value and `Clean` is not it. A
verdict line takes precedence over form A/B on the same line.
...
identity is not evidence of anything. Measured over all 444 artifacts: 116
(artifact, field) pairs have two accepted rows and **0** disagree.
...
| Reason | Condition |
|---|---|
| `MISSING_FIELD` | one or more of the eleven fields has no accepted row after forms A, A′ and B are applied |
| `AMBIGUOUS_FIELD` | a field has two or more accepted rows carrying different normalized values |
...
| `MANIFEST_DISAGREEMENT` | a parsed field ≠ the manifest's value for that field |
...
naming the file and the reason, which aborts the batch. That is deliberate: a
...
artifacts present at r3 as amended: **all 444 satisfy it** (370 under the
...
required it): §2.2.1 derives the evidence object's `captured_at` from this
entry's `timestamp`. `review_round` is the normalized digits (`"0"`, not
`"r0"`), matching §2.2.1's normalization. Every one of the eleven parsed fields
must equal the artifact's parsed value byte-for-byte, or the batch aborts with
...
9. **Scale of the untested surface.** Probes A–H cover the digest, ordering,
   ...
   *(r1: r0-review M-9d correctly refused to let this concession stand alone.
   §3.10 now makes a five-leg rollback rehearsal *(r2, r1-review Minor 4: the
