# Inventory review — DISP-M-3 / EVIDENCE / r0
...
```json
{"evidence_refs":[{"captured_at":"2026-08-13T02:49:11Z","content_sha256":"a7d8a6209c47dcce38693046ce19563c89e4dde47e0d9310d3bc949693c5017e","digest_mode":"UTF8_LINE_SPAN","end_line":180,"evidence_ref_id":"EV-DISP-M-3-SOURCE","path":"docs/blueprint/funda-third-order-review-disposition-report.md","scope":"Exact authoritative source occurrence for DISP-M-3","start_line":168},{"captured_at":"2026-08-13T02:49:11Z","content_sha256":"f1a60a20e24a0d2a457aa71f8d9006316ea461d237ea1faa054fc24176ba3dc4","digest_mode":"FILE_BYTES","end_line":null,"evidence_ref_id":"EV-DISP-M-3-SPEC-DRAFT","path":"docs/specs/equity-os-s13-claim-schema-vocabulary-evidence.md","scope":"Current draft specification bytes for DISP-M-3","start_line":null}],"required_evidence":[{"approval_ids":[],"description":"Current proof satisfying: ### M-3 — Predicate and metric vocabulary governance\n\n**Disposition: Accept with a simpler Phase 0.5 implementation.**\n\nTyped claims are ineffective without controlled predicates and metric definitions. The first version needs:\n\n- a small versioned metric registry;\n- a small versioned claim-predicate registry;\n- aliases and deprecated terms;\n- definition, expected object type, units/dimensions, and scope rules;\n- a human approval rule for additions.\n\nEmbedding-assisted duplicate suggestions are optional later. They should not be a Phase 0.5 dependency for a registry containing only dozens of entries.","evidence_id":"REQ-DISP-M-3-ACCEPTANCE","evidence_ref_ids":[],"evidence_type":"ARTIFACT","proof_mode":"CONTENT_HASH","scope":"DISP-M-3 acceptance and delivery scope","status":"UNRESOLVED"}],"verification_command":{"commands":[],"mode":"UNRESOLVED","not_applicable_review":null}}
```
...
- `reviewed_inventory_sha256` (pre-record): `2d20f797bc8232d0243289954b74c110c5391fd8d1ccc72839841cdcae6d0999`
...
|---|---|---|---|---|---|
| `REQ-DISP-M-3-ACCEPTANCE` | `ARTIFACT` | `CONTENT_HASH` | `UNRESOLVED` | `[]` | `[]` |
...
**Clause-by-clause coverage.** Four of the clause's five "first version needs"
bullets — a small versioned metric registry; a small versioned claim-predicate
registry; aliases and deprecated terms; definition, expected object type,
units/dimensions and scope rules — are registry design content provable by hashing
the approved S13 specification bytes, and the closing optionality carve-out for
embedding-assisted duplicate suggestions is likewise design content. One
`ARTIFACT`/`CONTENT_HASH` item classifies those correctly, and no command
obligation is stated anywhere in the clause.

**The fifth bullet is not covered.** "a human approval rule for additions" is a
human approval obligation. The contract's representation for such a proof is a
`TYPED_APPROVAL` item naming a component-local approval requirement (goal
L~487-491), and this row has no such item — because it has no such approval
requirement either. That second gap is the subject of this component's `APPROVAL`
review, which returned `ISSUES_FOUND`
(`docs/goals/reviews/ledger/inventory/DISP-M-3/APPROVAL-r0.md`). The two gaps are
dependent: a `TYPED_APPROVAL` evidence item cannot be added here until the missing
approval requirement exists to be named, so they must be remediated together. It is
recorded as **finding 2** below.

**`evidence_refs`.** Two objects, both recomputed and current:
`EV-DISP-M-3-SOURCE` (`UTF8_LINE_SPAN`, disposition report L168-180) and
`EV-DISP-M-3-SPEC-DRAFT` (`FILE_BYTES`, S13 draft).
...
**What is missing.** This row's `required_approvals` contains `APR-DISP-M-3-01`,
`DELEGATED_ARTIFACT_APPROVAL`, `Delegated fresh Sol xhigh specification reviewer`,
scope `M-3 under S13`. Under goal L598-600 a record satisfying that requirement
...
`REVIEWER`-role review**". The persisted review of S13's current
...
`EV-DISP-M-3-SPEC-DRAFT` is a `FILE_BYTES` reference to `docs/specs/equity-os-s13-claim-schema-vocabulary-evidence.md` — the exact
...
2. **The reviewed generator makes the pairing unconditional.**
   ...
   emits `REQ-DISP-M-3-SPEC-REVIEW` for this component. The canonical ledger contains
   no such item.
...
4. **The proof cannot be borrowed from a sibling row.** Goal L439 requires review
   evidence to be "current and **component-local**". S13 owns register rows
   B-06, B-12, C-04, each of which carries its own `REQ-REG-*-SPEC-REVIEW`; none of them
...
`REQ-DISP-M-3-SPEC-REVIEW`, `evidence_type` `REVIEW`, `proof_mode` `CONTENT_HASH`,
`status` `UNRESOLVED`, `evidence_ref_ids` `[]`, `approval_ids` `[]`, `scope`
`M-3 under S13`, description "Persisted clean fresh Sol xhigh review of the
...
## Finding 2 — no typed-approval evidence for the clause's human approval rule

**Severity:** Important. **Load-bearing:** yes. **Dependent on the `APPROVAL`
finding.**

The clause's fifth "first version needs" bullet is "a human approval rule for
additions". A human approval obligation is proved in this contract by a
`TYPED_APPROVAL` evidence item that "names one or more component-local requirements
that are `SATISFIED` by unique approval records" (goal L~487-491) — the shape
`DISP-M-1` and `DISP-M-5` use for their `ANALYST_ACCEPTANCE` obligations. This row
has no such evidence item, because it has no such approval requirement to name;
that missing requirement is finding 1 of
`docs/goals/reviews/ledger/inventory/DISP-M-3/APPROVAL-r0.md`.

The two gaps must be remediated together and in order: the approval requirement
first, then a `TYPED_APPROVAL` evidence item whose `approval_ids` names it. Adding
the evidence item alone would produce an item naming no component-local
requirement, which the contract does not permit.
...
recording design r2 §5.4 the `EVIDENCE` review on `DISP-M-3` therefore stays
