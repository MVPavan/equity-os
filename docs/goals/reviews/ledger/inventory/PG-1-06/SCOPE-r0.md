# Inventory review — PG-1-06 / SCOPE / r0
...
```json
{"activation_predicate":null,"disposition_refs":[],"gate_refs":[],"related_register_ids":["C-08","C-16"],"scope_derivation":{"authority_effect":null,"derived_program_disposition":"REQUIRED_NOW","related_register_ids":["C-08","C-16"],"rule":"RELATED_REGISTER_SCOPE"}}
```
...
- `reviewed_inventory_sha256` (pre-record): `6f9ab4a7e94f65e45d3a9ee52f9a3015313edcd09a2d628c8f84462eb9e8f810`
...
**`related_register_ids` = `["C-08", "C-16"]`, one register per conjunct plus the
subject they act on.** The clause has two conjuncts:

- "deterministic calculations satisfy their declared exact/tolerance/seeded
  replay class" — the *subject*, deterministic calculations, is `C-08` (register
  L79, "Implement minimum deterministic calculations"); the *predicate*, the
  declared replay class, is `C-16` (register L87), whose acceptance reads
  "Exact-class operators replay exactly; floating-point/optimization outputs meet
  **declared tolerances**; stochastic operators **store seeds** and test
  distributions". "exact/tolerance/seeded" is that sentence compressed into three
  words.
- "the approved narrative is bound to an artifact hash" — `C-16` again, whose
  acceptance ends "**approved narrative bytes are immutable and bound to content
  hash**".

So C-16 carries both conjuncts and C-08 carries the object the first conjunct
ranges over. C-16 lists C-08 among its dependencies, which is consistent with the
clause naming both.

**Contrary candidate, considered and rejected.** `C-09` ("Implement complete run
manifest", acceptance "Inputs, cutoff, source/evidence-package versions, tools,
models, prompts, code versions, costs, calculations, QA, approvals, and **exact
published-artifact hash** are registered") is the strongest alternative for the
second conjunct, since it is the only other register row naming an artifact hash.
It is rejected: C-09 *registers* the hash in a manifest, whereas the clause
requires the approved narrative to be *bound* to it, which is C-16's verbatim
obligation; and C-09 depends on C-16, so relating it would pull the consumer in
behind the producer. `REG-C-09.gate_refs` is empty — §F has no run-manifest
bullet — so this would be an invented edge, not a missing one.

**Back-reference verified.** `REG-C-08.gate_refs == ["PG-1-04", "PG-1-06"]` and
`REG-C-16.gate_refs == ["PG-1-06"]`; the `gate_map` equality holds on both. C-08
is partitioned cleanly between the two gates — `PG-1-04` takes its fail-closed
conjunct, this gate takes its replay-class conjunct — so neither edge is padded.

**Aggregation → `REQUIRED_NOW`.** Both `REG-C-08` and `REG-C-16` are
`REQUIRED_NOW` (`source_status=Open`); aggregate is `REQUIRED_NOW` under goal
...
related registers rather than duplicating it: here `C-08` → `G-1`, `6.9`; `C-16` → `G-1`, `M-4`, `6.9`. That inheritance
...
clause.
Both registers carry `G-1` ("Narrative reproducibility", disposition report L47)
and `6.9` ("Bit-exact computation is not universal", L396) — and 6.9 is the
finding that produced the clause's three-way replay taxonomy, since it says
"'bit-exact' should apply only to operators designed for exact replay.
Floating-point, optimization, and stochastic calculations require declared
tolerances, pinned environments, and stored seeds as applicable." The shared
disposition pair is the coherence signal for relating exactly these two.
...
**One field deliberately outside this decision.** `human_review_id = "HR-0004"`
sits in the `APPROVAL` inventory projection (goal L435-436) and is decided by
this component's `APPROVAL` review.
