# Inventory review — DISP-M-1 / APPROVAL / r0
...
```json
{"approval_records":[],"human_review_id":["HR-0004"],"required_approvals":[{"actor":null,"approval_id":"APR-DISP-M-1-01","approval_type":"DELEGATED_ARTIFACT_APPROVAL","evidence_ref_ids":[],"matched_record_id":null,"required_authority":"Delegated fresh Sol xhigh specification reviewer","scope":"M-1 under S05","status":"UNRESOLVED","timestamp":null},{"actor":null,"approval_id":"APR-DISP-M-1-02","approval_type":"ANALYST_ACCEPTANCE","evidence_ref_ids":[],"matched_record_id":null,"required_authority":"Responsible analyst","scope":"M-1 analyst acceptance","status":"UNRESOLVED","timestamp":null}],"security_exception_ids":[]}
```
...
- `reviewed_inventory_sha256` (pre-record): `eb550066744729436d40bebfb773b5855ed7cb9091b5e9c466ff0eb404c52963`
...
|---|---|---|---|---|
| `APR-DISP-M-1-01` | `DELEGATED_ARTIFACT_APPROVAL` | `Delegated fresh Sol xhigh specification reviewer` | `M-1 under S05` | `UNRESOLVED` |
| `APR-DISP-M-1-02` | `ANALYST_ACCEPTANCE` | `Responsible analyst` | `M-1 analyst acceptance` | `UNRESOLVED` |
...
required-authority table and by `validate_ledger_structural.py:2587`
(`REQUIRED_AUTHORITY_VOCABULARY`); the stored value matches exactly, which matters
because a satisfying record's `authority` must equal it byte for byte (goal
...
Report L122-128 contains one explicit authorization verb: "**Approve and version
it** before the three later assisted updates", referring to the analyst-authored
bootstrap coverage thesis. The clause identifies the actor by describing the
artifact as "analyst-authored", and the related register `A-11` states the same
obligation ("manually written, **approved**, versioned, and available before
Quarter 1") and itself carries `ANALYST_ACCEPTANCE` / `Responsible analyst`. The
ledger enumerates exactly that requirement here, with a component-local scope
("M-1 analyst acceptance") distinct from the register's scope, so the two are not
one approval doing double duty — which goal L~594 forbids. It is also mirrored by
the `TYPED_APPROVAL` evidence item `REQ-DISP-M-1-ANALYST_ACCEPTANCE-02`, whose
`approval_ids` names `APR-DISP-M-1-02`.

Candidate examined and rejected: `PRODUCT_OWNER_DECISION` / `Product owner
authorized to activate deferred blueprint scope`, suggested by the clause's closing
sentences "Do not expand Phase 0.5 into a full initiation product" and "Full
company initiation remains deferred". Rejected because that authority governs
*activating* deferred scope, and this clause requires the opposite — that the scope
stay deferred. An unrequested activation approval would be a padded obligation, not
a completeness fix.

No other verb of authorization appears, and the clause names no phase gate,
transition, or fail-closed boundary introducing one.
...
human authority beyond the delegated artifact approval, and that authority —
`ANALYST_ACCEPTANCE` / `Responsible analyst` — is enumerated with the correct type,
the exact pinned authority literal, and a component-local scope.
...
authority of any kind (goal L~624-626). It records only that `DISP-M-1`'s
