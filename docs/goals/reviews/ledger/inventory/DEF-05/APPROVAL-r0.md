# Inventory review — DEF-05 / APPROVAL / r0
...
```json
{"approval_records":[],"human_review_id":["HR-0004"],"required_approvals":[{"actor":null,"approval_id":"APR-DEF-05-01","approval_type":"DELEGATED_ARTIFACT_APPROVAL","evidence_ref_ids":[],"matched_record_id":null,"required_authority":"Delegated fresh Sol xhigh specification reviewer","scope":"DEF-05 under S24","status":"UNRESOLVED","timestamp":null}],"security_exception_ids":[]}
```
...
- `reviewed_inventory_sha256` (pre-record): `190b83b0dd425f6003db02a42bc67ac20f23e22139375ad4f5c7c61ee98ba276`
...
`text_digest` recomputed over the normalized L179-179 span →
`4cc2271bfacc75af7216017f1e2d19dca384b993ab9dc937981ff3b33dfa64b2`, matching the stored value.
...
**A capacity commitment, tested and rejected.** Of all thirteen bullets this is
the one whose subject is literally throughput, and the ledger does carry
`CAPACITY_COMMITMENT` (`Capacity owner`) on six rows, including `REG-C-18`
('Validate results-season throughput') and `REG-E-02` ('Add stress-test
companies'). The candidate is therefore real. It fails because a capacity
commitment is an owner's undertaking that a load *will be carried*; this clause
is the statement that the load is not taken on. The five authorities that would
be needed to run full-universe daily research are inventoried on `REG-E-04` —
activate-deferred `PRODUCT_OWNER_DECISION`, `DATA_RIGHTS_APPROVAL`,
`BUDGET_APPROVAL`, `NAMED_OWNER_COMMITMENT` (`Event-monitoring owner`), and the
delegated artifact approval — and not one of them transfers to the exclusion.

**The enumerated obligation.** One requirement, `APR-DEF-05-01`, type
`DELEGATED_ARTIFACT_APPROVAL`, authority "Delegated fresh Sol xhigh
specification reviewer", scope "DEF-05 under S24", `UNRESOLVED`. It is the delegated
...
| `PRODUCT_OWNER_DECISION` (incl. `Product owner authorized to activate deferred blueprint scope`) | `REG-E-04` ('Add event monitoring', `Deferred`) carries the activate-deferred authority together with four other typed approvals — the densest approval set in the ledger. All five attach to switching daily monitoring on. |
...
| `DATA_RIGHTS_APPROVAL`, `LEGAL_REVIEW`, `REGULATORY_REVIEW` | `REG-E-04` carries a `DATA_RIGHTS_APPROVAL` (`Data-rights authority`), because monitoring a wide universe daily consumes provider data under licence terms. This clause consumes none: excluding the capability acquires no data and triggers no rights, legal, or regulatory obligation. |
...
`required_approvals`. `DEF-05` is linked to `HR-0004` only.
...
`required_approvals` for `DEF-05` is complete at the input bytes pinned above: the
source clause demands exactly one typed approval — the delegated artifact
approval already enumerated as `APR-DEF-05-01` — and no authority whose
