# Inventory review — DEF-05 / SCOPE / r0
...
- `text_digest` recomputed over the normalized L179-179 span
  ...
  `4cc2271bfacc75af7216017f1e2d19dca384b993ab9dc937981ff3b33dfa64b2`,
  equal to the stored `text_digest`.
- `required_acceptance_text` is `"full-universe daily research"`, which is
  that span with the list marker and terminal punctuation removed — the exact
  clause content.

## Reasoning

**Kind.** Bullet 5 of register §G (L179), excluding full-universe daily
research. This is a *scale* exclusion — universe breadth and cadence — which
made `scale_trigger` worth testing explicitly. It fails: `scale_trigger` is
pinned to §H's storage and workflow reconsideration bullets (eight rows,
`SCALE-SQLITE-01..04` and `SCALE-WORKFLOW-01..04`, goal L143), each of which
states a condition under which to *reconsider infrastructure*. This bullet states
a research-scope exclusion with no reconsideration condition and sits in §G.
`first_release_deferral` is correct.
...
**Related register IDs.** `[]`, contract-forced. Two register neighbourhoods
bound this territory and neither is a related ID this row may carry:
`REG-C-01` ("Expand to two or three core non-financial companies", `Open`) and
`REG-C-18` ("Validate results-season throughput", `Open`) fix the *actual*
universe as a handful of companies, and `REG-E-04` ("Add event monitoring",
`Deferred`) holds the daily-cadence capability that this bullet's spec, S24
("conditional event monitoring"), also serves. `E-04` being `Deferred` with its
own five typed approvals is where the dormancy is proven; this row proves only
that the exclusion is active now.

**Source anchor and span.** `source_anchor` is `DEF-05`, a clause ordinal.
...
**Applicable review slot.** `DEF-05` is a non-register canonical row, so
...
only that `DEF-05`'s scope derivation is correct at the input bytes pinned above.
