# Inventory review — DOC-03 / APPROVAL / r0
...
### Reading line 471 against the closed vocabulary

"- **`funda-blueprint-implementation-decision-register-v2.md`** — authoritative live decisions
and gates;" designates a document as authoritative. This is the `DOC` clause most likely to be
mistaken for an approval-bearing statement, because "authoritative" and "gates" both sound like
authority language, so I tested it directly.

### Where this designation's authority actually sits

Not on this row. The goal itself pins the file at L72-77 with SHA-256 `26d51b31…`, and goal
L196 states "The register remains canonical; the ledger mirrors it and never overrides it."
The register's Status cells are the register's own; the approved `HR-0004` transaction scope
states explicitly that it "changes no pinned blueprint bytes or register Status cells". So the
authority relationship is established by the goal's pin and its approval, above the component
layer. A `required_approvals` entry here would assert that the ledger row itself needs a
sign-off to designate the register authoritative, which inverts the direction of authority the
contract sets.

Nor do the "gates" in the clause create an obligation here: the phase gates are inventoried as
the 35 `phase_gate_clause` rows and referenced from `register_row.gate_refs` (39 of 60 register
rows carry them). `DOC-03` carries `gate_refs: []`, correctly, so no gate flows an approval
obligation into this row under goal L535-536.

### Result

Line 471 names no authority whose sign-off is required for this component.
`required_approvals: []` is an affirmed determination.
