# Inventory review — PG-0A-03 / APPROVAL / r0
...
"are selected", which asserts a state of an artifact, not a sign-off. §F draws that
...
point, and it is mechanical, not stylistic.** `REG-A-02` carries `APR-REG-A-02-02`, a `PRODUCT_OWNER_DECISION` held by the Product owner, covering the register decision "Select one discovery company and four consecutive quarters". That is the authority behind the selection this gate observes. A reader could
...
duplicate requirement on this gate could not be satisfied by the same product-owner decision that satisfies `APR-REG-A-02-02`
...
**No other authority is hiding in the clause.** I read A-02's acceptance column
in full — "Quarter 0 is reserved for the manual baseline and bootstrap thesis;
Quarters 1–3 are reserved for three assisted incremental updates; source package
exists for all quarters and at least one management commitment can be tracked
across periods" — looking for any analyst, domain, rights, legal, budget,
capacity, owner, or distribution authority. There is none: every element is a
factual property of the selection record. The quarters' *use* does involve
analyst approvals, but those obligations sit on `A-03` and `A-11` and are gated
by `PG-05-01` and `PG-05-02`, both of which correctly carry
`ANALYST_ACCEPTANCE`.
...
`PG-0A-03` regardless of kind, and it is the slot this review is written against.
...
`docs/goals/reviews/ledger/equity-os-blueprint-approval-inventory-r0.md`
examined every canonical row for "explicit approval or acceptance authority"
with no corresponding non-delegated requirement, and its Important finding 2
named nine rows — of which the phase-gate members are `PG-05-01`, `PG-05-02`,
`PG-05-05`, `PG-1-06`, `PG-1-09`, and `PG-2-05`. `PG-0A-03` is not among them.
That review ran against an earlier ledger (167 canonical rows versus 169 now), so
I did not rely on it; the reading above is independent and the two agree.
