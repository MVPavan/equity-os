# Inventory review — DEF-03 / SCOPE / r0
...
- `text_digest` recomputed over the normalized L177-177 span
  ...
  `ef21f118b363ef3bc2bc4a7e1f342accc884b18f7a9dbe27dcc7f02229185c9a`,
  equal to the stored `text_digest`.
- `required_acceptance_text` is `"generalized autonomous planning"`, which is
  that span with the list marker and terminal punctuation removed — the exact
  clause content.

## Reasoning

**Kind.** Bullet 3 of register §G (L177), excluding generalized autonomous
planning. `first_release_deferral` is right and the alternative worth testing was
`scale_trigger`: §H (L191-207) also produces workflow-shaped rows. It fails on
both the source and the semantics — §H rows are "operating notes, not Phase 0.5
blockers" (register L193) that say *when to reconsider* an engine, and are
inventoried as the eight `SCALE-*` rows; this bullet says a capability is out of
the release and names no reconsideration condition.
...
**Related register IDs.** `[]`, contract-forced and semantically right. No
register row proposes autonomous planning in any status, so there is no dormant
decision this exclusion shadows. The register's positive commitment is the
opposite architecture: `REG-B-01` ("Implement fixed, resumable earnings-review
workflow", Status `Open`, `REQUIRED_NOW`) whose acceptance names state
definitions, allowed transitions, failure states, immutable step outputs,
idempotent retries, and resume behaviour — a *fixed* workflow. This bullet makes
the exclusion of the generalized alternative authoritative rather than merely
unstated, which is precisely the function goal L255-257 assigns to
`PROGRAM_WIDE_ACTIVE_CONTROL`.

**Source anchor and span.** `source_anchor` is `DEF-03`, a clause ordinal.
...
**Applicable review slot.** `DEF-03` is a non-register canonical row, so
...
only that `DEF-03`'s scope derivation is correct at the input bytes pinned above.
