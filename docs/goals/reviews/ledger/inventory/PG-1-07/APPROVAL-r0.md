# Inventory review — PG-1-07 / APPROVAL / r0
...
`primary_spec` is `null` on every phase-gate row. The artifact approval for this clause's subject matter is `REG-C-10`'s, under S15.
...
**Applying it to this clause.** "…are auditable" is a property of the system,
established by inspection at gate time. It names no approving authority and no
act of acceptance. The near-miss is real and worth stating plainly: `C-10`'s
*own* acceptance text does say "canonical promotion is separately approved", and
`REG-C-10` correspondingly carries the ledger's single `MEMORY_PROMOTION`
requirement (authority "Responsible analyst", paired with the `ANALYST`-typed
`REQ-REG-C-10-MEMORY_PROMOTION`). `PG-1-07` does not repeat that requirement. It
requires that promotion — including the record of its approval — be *auditable*.
That is an evidence property about a record, not a sign-off the gate itself
demands. Duplicating `MEMORY_PROMOTION` here would assert a second, gate-local
promotion authority that §F does not name, and would be exactly the
manufactured obligation that goal L551-554 and L613-615 are written against.

**Sweep of the closed non-delegated vocabulary.** Goal L562-575 and
`REQUIRED_AUTHORITY_VOCABULARY` (`validate_ledger_structural.py:2570-2613`)
close the set of typed approvals that a `required_approvals` entry can carry.
Checked one by one against this clause:

| Type | Why it is not demanded here |
|---|---|
| `MEMORY_PROMOTION` | The tempting near-miss, argued above. Its one instance in the whole ledger is `REG-C-10`'s, for `C-10`'s own "separately approved" phrase. `PG-1-07` asks for auditability of the promotion record, not for the promotion decision. |
| `ANALYST_ACCEPTANCE` | The clause approves no thesis, narrative, output, or measurement. Contrast `PG-1-06`, whose text contains the object "the approved narrative"; `PG-1-07`'s objects are operations, not artifacts awaiting an analyst's acceptance. |
| `DOMAIN_EXPERT_ACCEPTANCE` | No calculation, data, entity, or vocabulary judgment is being signed. All 6 sit on `REG-A-10`, `REG-B-03`, `REG-B-07`, `REG-B-12`, `REG-C-17`, and `PG-05-05`. |
| `PRODUCT_OWNER_DECISION` | Activates no deferred blueprint scope and adopts no memory approach. `C-10` is `Open`, so no activation authority is in play at all. |
| `BUDGET_APPROVAL`, `CAPACITY_COMMITMENT`, `NAMED_OWNER_COMMITMENT` | The clause commits no spend, no capacity, and no named owner; it constrains how existing workflow records must behave. |
