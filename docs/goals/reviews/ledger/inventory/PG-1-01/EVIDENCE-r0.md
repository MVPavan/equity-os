# Inventory review — PG-1-01 / EVIDENCE / r0
...
```json
{"evidence_refs":[{"captured_at":"2026-08-13T02:49:11Z","content_sha256":"196d501ba87f349f96e0aacfd891de2f6b74a122fe07fecea2cf0668a56b1b2a","digest_mode":"UTF8_LINE_SPAN","end_line":150,"evidence_ref_id":"EV-PG-1-01-SOURCE","path":"docs/blueprint/funda-blueprint-implementation-decision-register-v2.md","scope":"Exact authoritative source occurrence for PG-1-01","start_line":150}],"required_evidence":[{"approval_ids":[],"description":"Current proof satisfying: all numerical claims classified as material under A-10 resolve to a fact or calculation trace","evidence_id":"REQ-PG-1-01-ACCEPTANCE","evidence_ref_ids":[],"evidence_type":"ARTIFACT","proof_mode":"CONTENT_HASH","scope":"PG-1-01 acceptance and delivery scope","status":"UNRESOLVED"}],"verification_command":{"commands":[],"mode":"UNRESOLVED","not_applicable_review":null}}
```
...
- `reviewed_input_sha256` (pre-record): `3dac22636ead36b07d7d4054f6ed7721d9a0317993edd022e54143b1839354bc`
- `reviewed_inventory_sha256` (pre-record): `80f642b7a8e91835d104c04b2aa6ec47d3b1852439433984dde774b55c32382c`
...
`docs/blueprint/funda-blueprint-implementation-decision-register-v2.md` line 150,
under `### Phase 1 may exit only when` (register v2 L148), inside `## F. Phase-gate scorecard` (L122); `source_title` "Phase 1 gate clause 1",
`source_anchor` `F-1-01`:
...
- `text_digest` over the normalized L150-150 span (`"\n".join(lines[149:150]).strip(" \t\n\r\f\v")`,
  the exact rule at `validate_ledger_structural.py:174-176`) → `196d501ba87f349f96e0aacfd891de2f6b74a122fe07fecea2cf0668a56b1b2a`,
  equal to the stored value.
- `EV-PG-1-01-SOURCE.content_sha256` recomputed over the same span → `196d501ba87f349f96e0aacfd891de2f6b74a122fe07fecea2cf0668a56b1b2a`,
  equal to the stored value.
- `required_acceptance_text` equals that span with the `- ` list marker and the
  terminal `;` removed, byte for byte.
- Anchor uniqueness holds ledger-wide: `(funda-blueprint-implementation-decision-register-v2.md, F-1-01)` occurs once
  across all 213 rows, and so does the span `(150, 150)` — both recomputed this round
...
|---|---|---|---|---|---|
| `REQ-PG-1-01-ACCEPTANCE` | `ARTIFACT` | `CONTENT_HASH` | `UNRESOLVED` | `[]` | `[]` |
...
**Clause-by-clause coverage.** One obligation: every numerical claim classified
as material under A-10 resolves to a fact or a calculation trace. The proof is
the produced evidence package — the claims, their materiality classifications,
and the fact or trace each resolves to — whose bytes can be hashed. Register C-04
states the same obligation as an implementation requirement ("Material
observed/computed claims require direct source or calculation support …
contradiction and materiality reasoning are visible"), and register A-10 supplies
the classification policy. `ARTIFACT`/`CONTENT_HASH` is the right classification
for a property of a produced artifact.

**The strongest argument for an additional item, stated and answered.** "All …
resolve to …" is a universally quantified property over a claim set, and
universally quantified properties are the usual shape of a mechanical check, so a
`COMMAND` item is a live candidate. I do not adopt it, on three grounds. (i) The
clause names no test, replay, or demonstration, and neither does C-04's
acceptance column — contrast C-08 ("pass tests and fail closed") and C-15 ("tests
insert and reject post-cutoff records"), the two Phase 1 registers whose gates
`PG-1-04` and `PG-1-05` do carry `COMMAND` items. (ii) The mechanical obligation
that *does* exist in this clause's neighbourhood is A-10's "validator test cases
approved", and it is inventoried where its source demands it, as
`REQ-REG-A-10-COMMAND-PROOF` on `REG-A-10`; goal L439 keeps evidence
component-local, so it is not also owed here. (iii) The determination of what
"material" means is a policy classification, not an executable predicate at this
stage — which is exactly why A-10's test cases are *approved* rather than merely
run. The clause's gate is that the resolution holds in the produced work product,
and that is artifact-shaped.

**No `TYPED_APPROVAL` item is owed.** `required_approvals` is empty on this row,
so a typed item would name nothing (goal L485-487). `REG-A-10` carries a
`DOMAIN_EXPERT_ACCEPTANCE`; that authority is inventoried there, and this row's
emptiness is affirmed by its own `APPROVAL` review.
...
**`evidence_refs`.** One object, `EV-PG-1-01-SOURCE`, a
`UTF8_LINE_SPAN` binding to register v2 L150-150 with scope "Exact authoritative source occurrence for PG-1-01".
I re-hashed it against current bytes: `196d501ba87f349f96e0aacfd891de2f6b74a122fe07fecea2cf0668a56b1b2a`, equal to
...
evidence-inventory review's Critical finding 2 listed the §F clauses with
"explicit test/replay/demonstration obligations" as `PG-05-08`, `PG-1-04`–
`PG-1-06`, and `PG-2-03`–`PG-2-04`. `PG-1-01` is deliberately absent from a list
that includes its immediate neighbours, and `human_review_id` is `null` on this
row while it is `HR-0004` on `PG-1-04`, `PG-1-05`, and `PG-1-06` — the three that
received command items under that reconciliation. Two independent signals, same
conclusion as the reading above.
