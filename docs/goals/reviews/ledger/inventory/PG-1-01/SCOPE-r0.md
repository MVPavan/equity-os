# Inventory review — PG-1-01 / SCOPE / r0
...
**`related_register_ids` = `["A-10", "C-04"]`, and one half of it is quoted, not
inferred.** This clause and `PG-1-02` are the only two bullets in the whole of §F
that name a register ID inline: "classified as material under **A-10**". So
`A-10` is not a derivation judgment at all — it is a citation in the source text,
and omitting it would be the error. The second element is a judgment: `C-04`
(register L75) is "Implement materiality- and epistemic-class-aware claim
validation", acceptance "Material observed/computed claims require direct source
or **calculation support**". The clause's "resolve to a fact or calculation trace"
is that acceptance clause restated as a gate.

**Contrary candidate, considered and rejected.** `C-08` ("Implement minimum
deterministic calculations … reconciliation **traces** pass tests and fail
closed") is the strongest alternative, because it is the register decision that
*produces* a calculation trace. It is rejected: this clause's obligation is that a
material claim **resolves to** a trace, which is a validation property (C-04),
not the implementation of the calculation (C-08). C-08 has its own two gates,
`PG-1-04` and `PG-1-06`, both of which are about the calculations themselves.
Adding C-08 here would infer the producer from the product, which goal L233-235
forbids.

**Back-reference verified.** `REG-A-10.gate_refs == ["PG-0A-05", "PG-1-01",
"PG-1-02"]` and `REG-C-04.gate_refs == ["PG-1-01", "PG-1-02"]`; the `gate_map`
equality holds on both.

**An identical `reviewed_inventory_sha256` with `PG-1-02`, checked rather than
assumed.** The `SCOPE` inventory digest of this row,
`85509081a84a92c83b768150e6fe13acdf47527cfd2f5aa1060f3c2864f2a0e5`, is
byte-identical to `PG-1-02`'s. That is expected and is not a collision or a
copy-paste defect: the `SCOPE` projection (goal L431-432) carries no component
identity, and the two clauses genuinely share rule, related set, `disposition_refs`,
`gate_refs`, and `activation_predicate`. The two rows remain distinct where it
matters — their `reviewed_input_sha256` values differ
(`3dac2263…` here versus `9e206d7c…`), because the input projection binds
`component_id`, `source_anchor`, the span, `text_digest`, and
`required_acceptance_text` (goal L417-424). I confirmed both digests by
recomputation.

**Aggregation → `REQUIRED_NOW`.** Both `REG-A-10` and `REG-C-04` are
`REQUIRED_NOW` (`source_status=Open`), so the aggregate is `REQUIRED_NOW` under
goal L248-250. Stored values agree.
...
clause.
Both registers carry `G-5` ("Undefined materiality", disposition report L102) and
`6.2` ("Materiality is not only a financial-statement threshold", L359), which is
the coherence signal for pairing exactly these two on a materiality-resolution
gate.
