# Inventory review — PG-1-01 / APPROVAL / r0
...
"resolve to", which asserts a state of an artifact, not a sign-off. §F draws that
...
**The clause is a universally quantified property, not a determination.** "All
numerical claims classified as material under A-10 resolve to a fact or
calculation trace" asserts something that either holds of the produced work or
does not. There is no party whose signature could make it true, which is the
test goal L619-622 sets when it asks this review to check "the exact source
acceptance text … and fail-closed boundaries". Contrast `PG-1-06` two bullets
below, whose text says "the **approved** narrative" — there a person's approval
is part of the asserted state, and that row correctly carries
`APR-PG-1-06-01`.

**Why the related register's authority is not also owed here — the decisive
point, and it is mechanical, not stylistic.** `REG-A-10` carries `APR-REG-A-10-02`, a `DOMAIN_EXPERT_ACCEPTANCE` held by an "Equity-research domain expert", covering acceptance of the claim materiality policy this clause invokes by name. `REG-C-04` carries only the delegated specification approval and no non-delegated authority. A reader could
...
duplicate requirement on this gate could not be satisfied by the same domain-expert acceptance of the materiality policy
...
**And note what the clause does with A-10.** It cites A-10 as the *classifier*
("classified as material under A-10"), i.e. it consumes A-10's output. Consuming
a policy does not re-open the authority that accepted the policy; goal L188's
"one approval never implies another" forecloses deriving a second requirement
from the citation.
...
`PG-1-01` regardless of kind, and it is the slot this review is written against.
...
**Corroboration, treated as corroboration only.** `PG-1-01` is absent from
Important finding 2 of
`docs/goals/reviews/ledger/equity-os-blueprint-approval-inventory-r0.md`, which
named `PG-1-06` and `PG-1-09` from the same Phase 1 block. Independent reading
