# Inventory review — DEF-08 / SCOPE / r0
...
- `text_digest` recomputed over the normalized L182-182 span
  ...
  `8b5125037698336de9e01dc48e213e09ccf21fd0a6d99cb874e960826b127f1d`,
  equal to the stored `text_digest`.
- `required_acceptance_text` is `"portfolio construction"`, which is
  that span with the list marker and terminal punctuation removed — the exact
  clause content.

## Reasoning

**Kind.** Bullet 8 of register §G (L182), excluding portfolio construction.
`first_release_deferral` by the §G normalization rule. Worth noting that this is
the shortest acceptance text in the batch — "portfolio construction" — and that
brevity is not a defect: goal L182 requires `required_acceptance_text` to be the
exact occurrence text, and the bullet is exactly that phrase.
...
**Related register IDs.** `[]`, contract-forced and — unusually for this batch —
also true in the strongest sense: **no register row proposes portfolio
construction in any status**. I checked all 60. The nearest is `REG-E-05`
("Begin controlled quant validation", `Deferred`), which is *validation of
signals against point-in-time data*, not construction of portfolios, and which
this row's sibling `DEF-09` shadows on the alpha-claims side.

That absence is exactly why this row must exist and must be `REQUIRED_NOW`. A
capability the register never proposes is otherwise merely unmentioned; §G makes
the omission authoritative. `PROGRAM_WIDE_ACTIVE_CONTROL` with no related
register IDs is the only derivation that expresses "excluded, and not parked
against any decision".

**Distinct from `DEF-09`.** Both rows own spec S25 ("quant validation and
historical leakage"). They are different bullets at different lines with
different digests: `DEF-08` excludes *constructing* portfolios, `DEF-09` excludes
*claiming historical alpha*. Sharing a spec artifact is ownership, not identity.

**Source anchor and span.** `source_anchor` is `DEF-08`, a clause ordinal.
...
**Applicable review slot.** `DEF-08` is a non-register canonical row, so
...
only that `DEF-08`'s scope derivation is correct at the input bytes pinned above.
