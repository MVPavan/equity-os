# Inventory review — DEF-08 / EVIDENCE / r0
...
```json
{"evidence_refs":[{"captured_at":"2026-08-13T02:49:11Z","content_sha256":"8b5125037698336de9e01dc48e213e09ccf21fd0a6d99cb874e960826b127f1d","digest_mode":"UTF8_LINE_SPAN","end_line":182,"evidence_ref_id":"EV-DEF-08-SOURCE","path":"docs/blueprint/funda-blueprint-implementation-decision-register-v2.md","scope":"Exact authoritative source occurrence for DEF-08","start_line":182},{"captured_at":"2026-08-15T07:13:28Z","content_sha256":"3b66cb90a76ab8f62eef203de2beabff5171c556146071974cc48e926374bbd2","digest_mode":"FILE_BYTES","end_line":null,"evidence_ref_id":"EV-DEF-08-SPEC-DRAFT","path":"docs/specs/equity-os-s25-quant-validation-historical-leakage.md","scope":"Current draft specification bytes for DEF-08","start_line":null}],"required_evidence":[{"approval_ids":[],"description":"Current proof satisfying: portfolio construction","evidence_id":"REQ-DEF-08-ACCEPTANCE","evidence_ref_ids":[],"evidence_type":"ARTIFACT","proof_mode":"CONTENT_HASH","scope":"DEF-08 acceptance and delivery scope","status":"UNRESOLVED"},{"approval_ids":[],"description":"Persisted clean fresh Sol xhigh review of the current specification bytes","evidence_id":"REQ-DEF-08-SPEC-REVIEW","evidence_ref_ids":[],"evidence_type":"REVIEW","proof_mode":"CONTENT_HASH","scope":"DEF-08 under S25","status":"UNRESOLVED"},{"approval_ids":[],"description":"Current negative proof that the deferred scope has no implementation in the current bytes","evidence_id":"REQ-DEF-08-NO-IMPLEMENTATION","evidence_ref_ids":[],"evidence_type":"ARTIFACT","proof_mode":"CONTENT_HASH","scope":"DEF-08 current no-implementation proof","status":"UNRESOLVED"}],"verification_command":{"commands":[],"mode":"UNRESOLVED","not_applicable_review":null}}
```
...
- `reviewed_input_sha256` (pre-record): `166dd02239c53984e328e2123420716548314bda6daa5e29932a744eab9f22e6`
- `reviewed_inventory_sha256` (pre-record): `18606685a9c6e431e95a47a8a87a8e7f355ef929047e8af25f7e5f0830854f81`
...
182, inside `## G. Explicitly deferred from the first release` (L173):

> - portfolio construction;

`text_digest` and `EV-DEF-08-SOURCE.content_sha256` both recomputed over the
normalized L182-182 span → `8b5125037698336de9e01dc48e213e09ccf21fd0a6d99cb874e960826b127f1d`, matching the
stored values. `required_acceptance_text` is `"portfolio construction"`.

## Reasoning

**What the source clause actually demands, and whether it is covered.** The
clause is the batch's shortest — *portfolio construction* — and the shortness
made me check hardest whether a two-word clause can carry a complete obligation
list. It can, and does: the clause names a capability and places it outside the
first release, so the whole of what it demands is proof of continued absence.
`REQ-DEF-08-NO-IMPLEMENTATION` is that obligation. `REQ-DEF-08-SPEC-REVIEW` binds
S25 ("quant validation and historical leakage"), the spec that owns the quant
territory where portfolio construction would otherwise appear.

**No register row proposes portfolio construction in any status** — verified
across all 60. That absence matters for completeness in a specific way: where a
deferral shadows a `Deferred` register row (as `DEF-02`, `DEF-04`, `DEF-06`,
`DEF-09`, `DEF-10`, and `DEF-11` do), one could argue the register row's own
typed approvals hint at proof obligations the deferral should mirror. Here there
is no such row to mirror, so the three-item list is not merely sufficient but
exhaustive of anything the program has ever said about this capability.
...
|---|---|---|---|
| `REQ-DEF-08-ACCEPTANCE` | `ARTIFACT` | `CONTENT_HASH` | the clause's own acceptance scope |
| `REQ-DEF-08-SPEC-REVIEW` | `REVIEW` | `CONTENT_HASH` | a persisted clean review of the current specification bytes |
| `REQ-DEF-08-NO-IMPLEMENTATION` | `ARTIFACT` | `CONTENT_HASH` | current negative proof that the deferred scope has no implementation in the current bytes |
...
whose `program_disposition` is `REJECTED_ACCOUNTED` (`:2734-2736`). `DEF-08` is
...
`REQ-DEF-08-NO-IMPLEMENTATION`. Its future satisfaction is governed by the
...
- *`TYPED_APPROVAL`.* Goal L484-487 requires a `TYPED_APPROVAL` item to name
  ...
  requirement is `APR-DEF-08-01`, of type `DELEGATED_ARTIFACT_APPROVAL`. Swept all
...
- *`REVIEW` — present, and checked against the known defect population.* Of the
  ...
  ("All 32 disposition components omit required review evidence"). `DEF-08` is in
...
Two references: `EV-DEF-08-SOURCE` (UTF8_LINE_SPAN over `docs/blueprint/funda-blueprint-implementation-decision-register-v2.md`, L182-182, captured `2026-08-13T02:49:11Z`); `EV-DEF-08-SPEC-DRAFT` (FILE_BYTES over `docs/specs/equity-os-s25-quant-validation-historical-leakage.md`, captured `2026-08-15T07:13:28Z`). Every one resolved and re-digested during the structural validation
...
1. **The `ACCEPTANCE` item's description is still positively framed.**
   `REQ-DEF-08-ACCEPTANCE.description` reads "Current proof satisfying:
   portfolio construction" — a deferral clause demands no proof that its
...
2. **A vendor model lane survives in `REQ-DEF-08-SPEC-REVIEW.description`**
...
`required_evidence` for `DEF-08` is complete at the input bytes pinned above: the
