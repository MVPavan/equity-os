# Inventory review — DEF-08 / APPROVAL / r0
...
```json
{"approval_records":[],"human_review_id":["HR-0004"],"required_approvals":[{"actor":null,"approval_id":"APR-DEF-08-01","approval_type":"DELEGATED_ARTIFACT_APPROVAL","evidence_ref_ids":[],"matched_record_id":null,"required_authority":"Delegated fresh Sol xhigh specification reviewer","scope":"DEF-08 under S25","status":"UNRESOLVED","timestamp":null}],"security_exception_ids":[]}
```
...
- `reviewed_inventory_sha256` (pre-record): `df9337ba7cffae579a818291ecf769f31591be2490f868ab3e8632bb37df42d5`
...
`text_digest` recomputed over the normalized L182-182 span →
`8b5125037698336de9e01dc48e213e09ccf21fd0a6d99cb874e960826b127f1d`, matching the stored value.
...
**The unusual case: an exclusion with no register counterpart.** Six of the
thirteen §G bullets shadow a `Deferred` register row, and for those the approval
question is answered by pointing at where the activation authority already sits.
Here there is no such row. That makes the completeness question sharper, not
easier, because there is no neighbouring inventory to absorb an obligation this
row might have dropped.

I resolved it by direction of travel. Every one of the twelve types in the closed
required-authority table authorises *doing* something — accepting an output,
spending, committing capacity, taking data rights, crossing a boundary,
distributing. None authorises *not* doing something, and the contract offers no
"approval to abstain". A clause whose entire content is that a capability stays
out of the release therefore has an empty non-delegated obligation set as its
correct and complete state, exactly as goal L188 describes: "a completed,
evidenced determination that no approval is required, not an unknown inventory."

**The enumerated obligation.** One requirement, `APR-DEF-08-01`, type
`DELEGATED_ARTIFACT_APPROVAL`, authority "Delegated fresh Sol xhigh
specification reviewer", scope "DEF-08 under S25", `UNRESOLVED`. It is the delegated
...
| `PRODUCT_OWNER_DECISION` (incl. `Product owner authorized to activate deferred blueprint scope`) | No register row proposes portfolio construction in any status — verified across all 60 — so the activate-deferred authority has no row to sit on and nothing here to activate. Introducing portfolio construction would require a new register decision under the register's own Authority rule (register L23), inventoried as `AUTH-REG-001`. |
...
`required_approvals`. `DEF-08` is linked to `HR-0004` only.
...
`required_approvals` for `DEF-08` is complete at the input bytes pinned above: the
source clause demands exactly one typed approval — the delegated artifact
approval already enumerated as `APR-DEF-08-01` — and no authority whose
