# Inventory review — PG-2-05 / EVIDENCE / r0
...
```json
{"evidence_refs":[{"captured_at":"2026-08-13T02:49:11Z","content_sha256":"29dbd4151b7e3b554d28eaab54625e2ecfdb0b6b774bf730a23aef5950444764","digest_mode":"UTF8_LINE_SPAN","end_line":168,"evidence_ref_id":"EV-PG-2-05-SOURCE","path":"docs/blueprint/funda-blueprint-implementation-decision-register-v2.md","scope":"Exact authoritative source occurrence for PG-2-05","start_line":168}],"required_evidence":[{"approval_ids":[],"description":"Current proof satisfying: operational burden is acceptable","evidence_id":"REQ-PG-2-05-ACCEPTANCE","evidence_ref_ids":[],"evidence_type":"ARTIFACT","proof_mode":"CONTENT_HASH","scope":"PG-2-05 acceptance and delivery scope","status":"UNRESOLVED"}],"verification_command":{"commands":[],"mode":"UNRESOLVED","not_applicable_review":null}}
```
...
- `reviewed_inventory_sha256` (pre-record): `83b8c38ce84cd534fdc6d677bf80739c170a8c8c146f2e09c0242980c55c646d`
...
**What the clause actually demands.** One judgment — that operational burden is
acceptable — resting on a measurement. `REQ-PG-2-05-ACCEPTANCE`
(`ARTIFACT`/`CONTENT_HASH`, description quoting the clause verbatim, scope
"PG-2-05 acceptance and delivery scope") is the single enumerated obligation,
and it binds the burden measurement artifact whose fields `AP-PG-2-05` reads
(`operator_minutes_per_month`, `incidents_per_100_runs`, `p95_recovery_minutes`
and their three `precommitted_max_*` counterparts).
...
**The one real completeness question on this row: the unpaired
`PRODUCT_OWNER_DECISION`.** This row carries `APR-PG-2-05-01`,
`PRODUCT_OWNER_DECISION` / "Product owner", and **no** `TYPED_APPROVAL` evidence
item names it. Six other rows in the ledger pair an approval requirement with a
typed item, so the absence has to be justified rather than waved through.

It is correct, for a reason in the closed vocabularies rather than in
convention. Goal L487-490 lists the evidence categories that "always uses
`TYPED_APPROVAL`": "Analyst, domain, provider, rights, legal, regulatory,
budget, capacity, owner, production, distribution, security, and external". Each
name in that list corresponds to a member of the closed `evidence_type`
vocabulary at goal L479-482 — `ANALYST`, `DOMAIN`, `PROVIDER`, `DATA_RIGHTS`,
`LEGAL`, `REGULATORY`, `BUDGET`, `CAPACITY`, `NAMED_OWNER`, `PRODUCTION`,
`DISTRIBUTION`, `SECURITY`, `EXTERNAL_COORDINATION`. **There is no
product-owner evidence type.** A `TYPED_APPROVAL` item for
`APR-PG-2-05-01` would therefore have to be typed as something it is not.

I verified the consequence holds ledger-wide instead of assuming it: of the 23
`PRODUCT_OWNER_DECISION` requirements in the ledger, **zero** are paired with a
`TYPED_APPROVAL` item — not one exception across `PG-2-05`, `REG-A-01`,
`REG-A-02`, `REG-A-04`, `REG-A-13`, `REG-C-13`, `REG-C-14`, `REG-D-02`,
`REG-D-03`, `REG-D-04`, `REG-D-05` (×2), `REG-E-01`…`REG-E-10`. The same holds
for the single `EXECUTION_TRUST_DOMAIN_APPROVAL` (`REG-E-09`) and for all 123
`DELEGATED_ARTIFACT_APPROVAL` requirements. Meanwhile every requirement whose
type *does* have an evidence-category counterpart is paired 1:1 — 13
`ANALYST_ACCEPTANCE`, 6 `DOMAIN_EXPERT_ACCEPTANCE`, 6 `CAPACITY_COMMITMENT`, 6
`BUDGET_APPROVAL`, 5 `DATA_RIGHTS_APPROVAL`, 5 `LEGAL_REVIEW`, 3
`NAMED_OWNER_COMMITMENT`, and one each of `MEMORY_PROMOTION` (typed `ANALYST`,
matching its "Responsible analyst" authority), `DISTRIBUTION_APPROVAL`, and
`REGULATORY_REVIEW`. The rule is exact, the ledger obeys it without exception,
and `PG-1-09` in this same batch shows the pairing side working: its
`CAPACITY_COMMITMENT` does carry the `CAPACITY`-typed item.

So the product owner's decision is proved through the approval path — an
`approval_records` entry bound to a canonical human resolution (goal L593-608) —
not through a `required_evidence` item. Nothing is unenumerated.
...
`PG-1-04`, `PG-1-05`, `PG-1-06`, `PG-2-03`, and `PG-2-04`. `PG-2-05` is not among
...
Independently, an acceptability judgment is exactly the case goal L487-490
excludes from command proof: it is decided by an authority against pre-committed
ceilings, "never a fabricated shell command". The mechanical evaluation of those
ceilings is `AP-PG-2-05`'s job, and it is present.
...
`validate_ledger_structural.py:2553-2562`. `PG-2-05` is `CONDITIONAL_UNACTIVATED` and carries the predicate `AP-PG-2-05`, so the sentence — about a clause left with **no** predicate by aggregation — does not describe this row. The prose and the
...
artifact to have reviewed or delegated-approved. The spec-review and delegated-approval obligations sit on `REG-D-05` under S20.
...
**`evidence_refs`.** One reference, re-verified: `EV-PG-2-05-SOURCE`,
`UTF8_LINE_SPAN` over register v2 L168-168, digest
`29dbd4151b7e3b554d28eaab54625e2ecfdb0b6b774bf730a23aef5950444764`, captured
...
`required_evidence` for `PG-2-05` is complete at the input bytes pinned above.
