# Inventory review — PG-05-10 / SCOPE / r0
...
```json
{"activation_predicate":null,"disposition_refs":[],"gate_refs":[],"related_register_ids":["A-08","B-08"],"scope_derivation":{"authority_effect":null,"derived_program_disposition":"REQUIRED_NOW","related_register_ids":["A-08","B-08"],"rule":"RELATED_REGISTER_SCOPE"}}
```
...
- `reviewed_inventory_sha256` (pre-record): `69375f04dedfbc1e880008c7eb3489265a1c62aabe6447cfcbceef815a77f22e`
...
**Related register IDs — `["A-08", "B-08"]`.** The clause has a subject and a
disjunctive predicate, and the two registers cover them:

- "the first golden cases" → A-08 (register L38), "Appoint golden-test-set owner", whose
  acceptance includes "**first twenty labeled cases**, including
  prompt-injection/source-confusion cases". "First golden cases" is that noun phrase.
- "automated or consistently reviewable" → A-08's "**review cadence**" for the reviewable
  branch, and B-08 (register L58), "Record failure taxonomy", which supplies the
  categories — "Extraction, reconciliation, source, unit, period, calculation, citation,
  inference, review, cutoff leakage, source-confusion, and document-as-instruction
  failures categorized" — against which cases are labeled *consistently*. Without a
  taxonomy, "consistently reviewable" has no referent.

B-08's backlink is decisive on whether it is padding: `REG-B-08.gate_refs ==
["PG-05-10"]`, its only gate. A-08's is `["PG-0A-08", "PG-05-10"]`, and the split between
those two gates is exactly right — `PG-0A-08` is "the golden-set owner and initial cases
exist" (existence, Phase 0A) and this clause is about those cases being automated or
consistently reviewable (quality of use, Phase 0.5). One register, two clauses, two
distinct demands: this is the clearest case in the batch that `related_register_ids`
records source semantics rather than a whole-row claim.

**Aggregated disposition.** A-08 and B-08 are both `REQUIRED_NOW`; the aggregation
...
**Last bullet of the list.** The occurrence at register L146 ends with `.` rather than
`;`, being the final item under the Phase 0.5 heading. `required_acceptance_text` strips
it correctly, and `text_digest` recomputes over the span *with* the terminator, matching
the stored value — checked because an off-by-one in terminator handling on the last bullet
of a list is a plausible generator error and would invalidate the content binding.
