# Inventory review — `REG-A-06` — `EVIDENCE` — `r0`
  ...
  "evidence_refs": [
    {
      ...
      "content_sha256": "fe4ab447a3f815971ef8182e3b94e602e80a85be61b283c53042992e2cde128e",
      "digest_mode": "UTF8_LINE_SPAN",
      "end_line": 36,
      "evidence_ref_id": "EV-REG-A-06-SOURCE",
      "path": "docs/blueprint/funda-blueprint-implementation-decision-register-v2.md",
      "scope": "Exact authoritative source occurrence for REG-A-06",
      "start_line": 36
    },
    {
      ...
      "end_line": null,
      "evidence_ref_id": "EV-REG-A-06-SPEC-DRAFT",
      "path": "docs/specs/equity-os-s09-filing-ingestion-point-in-time-capture.md",
      "scope": "Current draft specification bytes for REG-A-06",
      "start_line": null
    },
    {
      ...
      "digest_mode": "FILE_BYTES",
      "end_line": null,
      "evidence_ref_id": "EV-REG-A-06-S09-R3-N1-CURRENT-S09",
      "path": "docs/specs/equity-os-s09-filing-ingestion-point-in-time-capture.md",
      "scope": "Exact current S09 bytes adjudicated for S09-r3-N1 on REG-A-06",
      "start_line": null
    },
    {
      "captured_at": "2026-08-13T04:29:50Z",
      ...
      "digest_mode": "FILE_BYTES",
      "end_line": null,
      "evidence_ref_id": "EV-REG-A-06-S09-R3-N1-R4",
      "path": "docs/goals/reviews/specs/equity-os-s07-s09-r4.md",
      "scope": "Final ordinary r4 review report retaining S09-r3-N1 for REG-A-06",
      "start_line": null
    },
    {
      "captured_at": "2026-08-13T04:29:50Z",
      ...
      "digest_mode": "FILE_BYTES",
      "end_line": null,
      "evidence_ref_id": "EV-REG-A-06-S09-R3-N1-ADJUDICATION",
      "path": "docs/goals/reviews/specs/equity-os-s07-s09-adjudication.md",
      "scope": "Post-cap adjudication upholding S09-r3-N1 and its exact cone for REG-A-06",
      "start_line": null
    }
  ],
  "required_evidence": [
    {
      "approval_ids": [],
      "description": "Current proof satisfying: Coverage matrix by company, quarter, filing channel, taxonomy/version, statement, segment, note, ownership/share count, restatement behavior, mapping stability, and reconciliation effort",
      "evidence_id": "REQ-REG-A-06-ACCEPTANCE",
      "evidence_ref_ids": [],
      "evidence_type": "ARTIFACT",
      "proof_mode": "CONTENT_HASH",
      "scope": "REG-A-06 acceptance and delivery scope",
      "status": "UNRESOLVED"
    },
    {
      "approval_ids": [],
      "description": "Persisted clean fresh Sol xhigh review of the current specification bytes",
      "evidence_id": "REQ-REG-A-06-SPEC-REVIEW",
      "evidence_ref_ids": [],
...
**What the source clause demands.** Register row `A-06` (v2 line 36) is a
*spike*: "Run filing-channel-aware XBRL-versus-PDF spike", whose acceptance
cell demands one deliverable — a coverage matrix spanning eleven named axes
...
effort). The demanded proof is a document, not an executed check: the clause
contains no verb of verification (`tested`, `pass tests`, `replay`,
`reproducible`, `fail closed`). It is a measurement report about two ingestion
channels.

**Against the enumerated inventory.** Two items are declared.
`REQ-REG-A-06-ACCEPTANCE` (`ARTIFACT` / `CONTENT_HASH`) carries the acceptance
cell byte-for-byte in its description — I recomputed
`"Current proof satisfying: " + required_acceptance_text` and it equals the
stored description exactly — so all eleven axes are inside one hashed artifact
obligation and none is dropped. `REQ-REG-A-06-SPEC-REVIEW` (`REVIEW` /
`CONTENT_HASH`) is the persisted-review proof that pairs with the row's single
`DELEGATED_ARTIFACT_APPROVAL`; without it that approval would have no evidence
surface at all.

**Is a command proof missing?** No. `REG-A-06` is deliberately absent from the
contract's pinned `EXPECTED_COMMAND_PROOF_COMPONENTS`
(`validate_ledger_structural.py:2635-2649`), and the textual test I applied
independently reaches the same answer: every one of the twenty-five pinned
components has an explicit executed-verification demand in its own clause,
and `A-06`'s has none. Declaring a `COMMAND_RESULT` item here would in fact
fail structural validation at `:2649`, so its absence is not an oversight but a
contract-fixed classification.

**Do the linked authorities add an unlisted proof?** Gate `PG-0A-04` ("the
filing-channel-aware XBRL/PDF spike is complete") holds only its own
`ARTIFACT`/`CONTENT_HASH` acceptance and no approval, so it introduces no
proof kind here. Disposition `R-2` sharpens the spike — distinguish exchange
quarterly-result XBRL, annual channels, issuer documents, and taxonomy/version
changes, and "measure mapping stability, not merely field coverage" — and both
of those requirements are already *inside* the acceptance text I hashed
("filing channel, taxonomy/version" and "mapping stability"), so `R-2` adds
sharpness, not a new obligation type. Disposition `M-9`
(untrusted-document surface) reaches `A-06` only as a spec-level
`disposition_refs` tag; its register scope is `A-08` and `B-08`, and its
operational controls are carried as command proof on `DISP-M-9`, not here.

**Open finding.** `S09-r3-N1` is `OPEN_BLOCKING` and load-bearing on this row,
but it is an approval-proof defect in the S09 specification, adjudicated
`UPHELD` with fix `NOT_AUTHORIZED`. It changes what must be *fixed*; it names
no acceptance proof that `required_evidence` fails to enumerate.
