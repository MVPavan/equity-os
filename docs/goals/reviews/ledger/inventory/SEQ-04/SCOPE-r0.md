# Inventory review — SEQ-04 / SCOPE / r0
...
```json
{"activation_predicate":null,"disposition_refs":[],"gate_refs":[],"related_register_ids":[],"scope_derivation":{"applicable_spec_ids":["S06","S08"],"authority_effect":null,"derived_program_disposition":"REQUIRED_NOW","related_register_ids":[],"rule":"PROGRAM_WIDE_ACTIVE_CONTROL","source_register_ids":["A-10","A-13"]}}
```
...
- `reviewed_inventory_sha256` (pre-record): `0aa6c1234fe16fcfcbbe9b6041f6a641379df2c0693e4ffcba0f69b48f0389cd`
...
**`source_register_ids` = `["A-10","A-13"]`, verified against the clause.**
"define materiality" is `A-10`; "and measurement methods" is `A-13`. The trailing
"before collecting the baseline" is the ordering constraint the clause exists to
impose — it is sequencing content, not a third register reference, and correctly
adds no ID. `validate_ledger_structural.py:2483` pins the same pair.

**`applicable_spec_ids` = `["S06","S08"]`, resolved independently.** `A-10` is
inventoried on `REG-A-10`, approval scope
`"A-10 under S06: Define claim materiality policy"`, spec draft
`docs/specs/equity-os-s06-output-materiality-falsifiers.md` ⇒ `S06`. `A-13` is
inventoried on `REG-A-13`, approval scope
`"A-13 under S08: Freeze success-metric contract"`, spec draft
`docs/specs/equity-os-s08-success-metrics-budgets-capacity.md` ⇒ `S08`. Closure
`{S06, S08}` matches the stored list. The split is real: materiality policy is
output-contract scope, measurement methods are success-metric scope.

**The derivation is right; the downstream inventory does not follow it.** Only
`S06` is covered by an enumerated obligation on this row. That is outside the
`SCOPE` projection and is recorded in this component's `EVIDENCE` and `APPROVAL`
reviews, neither of which is clean.

**Source anchor and span.** `SEQUENCE-04` at L454-454. `text_digest` recomputed
over the normalized span → `8a721afc…`, matching; `required_acceptance_text`
...
**Blocked state checked against this projection, and it does not enter it.** This
row is `delivery_status: REVIEW_BLOCKED` at `review_round: 4` with one
`OPEN_BLOCKING`, load-bearing, Important finding (`S06-I7`, "Cross-record digest
cycle", `UPHELD` on adjudication) and a `blocked_scope` entry naming bead
`eqos-0xb.6`. I checked whether any of that should alter the scope derivation and
it should not: `open_findings`, `blocked_scope`, `delivery_status`, and
`review_round` are all outside the `SCOPE` inventory projection, and none of them
is a `scope_derivation` input under goal L226-235. The blocker constrains delivery,
not derivation. It is covered by `reviewed_input_sha256`, so a change to it
correctly stales this review.

**Cross-check.** `derived_program_disposition` = `program_disposition` =
`REQUIRED_NOW` — note that `REVIEW_BLOCKED` delivery does not downgrade the
program disposition, and should not: the component is still required.
...
only that `SEQ-04`'s scope derivation is correct at the input bytes pinned
above.
