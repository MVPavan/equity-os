# Inventory review — DEF-02 / SCOPE / r0
...
- `source_hash` `26d51b31…` recomputed over the whole register file → matches
  the stored value.
- `text_digest` recomputed over the normalized L176-176 span
  (`\n`-joined, surrounding ASCII whitespace trimmed) →
  `a25c877de6325fe15f0aba062fc45e7a6bed29159b644df454df05fafc511fa1`,
  equal to the stored `text_digest`.
- `required_acceptance_text` is `"GBrain as a mandatory dependency"`, which is
  that span with the list marker and terminal punctuation removed — the exact
  clause content.

## Reasoning

**Kind.** Bullet 2 of register §G (L176). The clause excludes GBrain **as a
mandatory dependency** — a dependency-status exclusion, not a decision to
evaluate GBrain. That distinction is what makes `first_release_deferral` right
rather than the row being a restatement of a §D register decision: §D already
carries three GBrain rows (`D-02` benchmark, `D-04` repository/dependency
posture, `D-05` adoption decision), all Status `Deferred`, all inventoried
separately as `register_row`s. Those decide *whether to adopt*; this bullet fixes
that adoption may not be assumed by the first release regardless of how they
resolve.
...
**Related register IDs.** `[]`, contract-forced. Here the semantic relation is
real — `REG-D-02`, `REG-D-04`, and `REG-D-05` are all GBrain rows — so I checked
where the relation is carried instead of on this row, and it is carried
soundly: each of the three is derived `CONDITIONAL_UNACTIVATED` from its own
`Deferred` activation and current status (goal L215-216), and each carries a
`PRODUCT_OWNER_DECISION` requirement with authority `Product owner authorized to
activate deferred blueprint scope`. The dormancy of GBrain is therefore proven by
those rows' own derivations, not by a ref array here. Goal L232-234 is explicit
that `related_register_ids` is source semantics and "may not be padded or
inferred"; naming D-02/D-04/D-05 here would be exactly that inference, and would
additionally break `:1547-1549`.

Checked against `DISP-R-1`, the ledger's one `REJECTED_ACCOUNTED` row, which
also concerns `D-02` dormancy ("Current S20 draft preserves D-02 as dormant and
contains no implementation claim") and shares spec S20. It is a distinct
occurrence in the disposition report with its own `authority_effect`
(`REJECTED_PROPOSAL`) and its own rejection record. Two rows for two occurrences
in two documents is correct, not duplication.

**Source anchor and span.** `source_anchor` is `DEF-02`, a clause ordinal.
...
**Applicable review slot.** `DEF-02` is a non-register canonical row, so
...
only that `DEF-02`'s scope derivation is correct at the input bytes pinned above.
