# Inventory review — DEF-02 / APPROVAL / r0
...
```json
{"approval_records":[],"human_review_id":["HR-0004"],"required_approvals":[{"actor":null,"approval_id":"APR-DEF-02-01","approval_type":"DELEGATED_ARTIFACT_APPROVAL","evidence_ref_ids":[],"matched_record_id":null,"required_authority":"Delegated fresh Sol xhigh specification reviewer","scope":"DEF-02 under S20","status":"UNRESOLVED","timestamp":null}],"security_exception_ids":[]}
```
...
- `reviewed_inventory_sha256` (pre-record): `c42ce7285f85c6fb4c1f89a7d00d70925d9a6daa9f3dc54e16eb98a2b5a4e5c9`
...
`a25c877de6325fe15f0aba062fc45e7a6bed29159b644df454df05fafc511fa1`, matching the stored value.
...
**A dependency-licence authority, tested and rejected.** The tempting candidate
here is `LEGAL_REVIEW` with authority `Competent dependency-license reviewer`,
which exists in this ledger on three rows — `REG-D-04`, `REG-E-06`, `REG-E-07` —
all of which concern taking on external code. `REG-D-04` is the GBrain one, and
it shares this row's subject. It is not this row's obligation: a licence review
proves that a dependency *may be used*, and this clause is the commitment not to
require it. The asymmetry is the point — declining a dependency needs no
authority, while adopting one needs several, which is why `D-04` carries the
review and this row does not.

**The enumerated obligation.** One requirement, `APR-DEF-02-01`, type
`DELEGATED_ARTIFACT_APPROVAL`, authority "Delegated fresh Sol xhigh
specification reviewer", scope "DEF-02 under S20", `UNRESOLVED`. It is the delegated
...
| `PRODUCT_OWNER_DECISION` (incl. `Product owner authorized to activate deferred blueprint scope`) | `REG-D-02`, `REG-D-04`, and `REG-D-05` — the three GBrain register rows, all `Deferred` — each carry the activate-deferred authority. That is where a decision to depend on GBrain would be approved. Excluding it as mandatory needs no counter-approval. |
...
| `DATA_RIGHTS_APPROVAL`, `LEGAL_REVIEW`, `REGULATORY_REVIEW` | No data is acquired and no regulated activity performed. `REG-D-04` ('Verify GBrain repository and dependency posture') does carry a `LEGAL_REVIEW` (`Competent dependency-license reviewer`) — because *adopting* a dependency requires a licence review. Not adopting one requires none. |
...
`required_approvals`. `DEF-02` is linked to `HR-0004` only.
...
`required_approvals` for `DEF-02` is complete at the input bytes pinned above: the
source clause demands exactly one typed approval — the delegated artifact
approval already enumerated as `APR-DEF-02-01` — and no authority whose
