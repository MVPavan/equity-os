# Inventory review — DEF-02 / EVIDENCE / r0
...
```json
{"evidence_refs":[{"captured_at":"2026-08-13T02:49:11Z","content_sha256":"a25c877de6325fe15f0aba062fc45e7a6bed29159b644df454df05fafc511fa1","digest_mode":"UTF8_LINE_SPAN","end_line":176,"evidence_ref_id":"EV-DEF-02-SOURCE","path":"docs/blueprint/funda-blueprint-implementation-decision-register-v2.md","scope":"Exact authoritative source occurrence for DEF-02","start_line":176},{"captured_at":"2026-08-15T07:13:28Z","content_sha256":"4948d0f84240cd58b107da1272ed2b90a386bc3fb9ebb6228faac8b98be9c483","digest_mode":"FILE_BYTES","end_line":null,"evidence_ref_id":"EV-DEF-02-SPEC-DRAFT","path":"docs/specs/equity-os-s20-memory-benchmark-gbrain.md","scope":"Current draft specification bytes for DEF-02","start_line":null}],"required_evidence":[{"approval_ids":[],"description":"Current proof satisfying: GBrain as a mandatory dependency","evidence_id":"REQ-DEF-02-ACCEPTANCE","evidence_ref_ids":[],"evidence_type":"ARTIFACT","proof_mode":"CONTENT_HASH","scope":"DEF-02 acceptance and delivery scope","status":"UNRESOLVED"},{"approval_ids":[],"description":"Persisted clean fresh Sol xhigh review of the current specification bytes","evidence_id":"REQ-DEF-02-SPEC-REVIEW","evidence_ref_ids":[],"evidence_type":"REVIEW","proof_mode":"CONTENT_HASH","scope":"DEF-02 under S20","status":"UNRESOLVED"},{"approval_ids":[],"description":"Current negative proof that the deferred scope has no implementation in the current bytes","evidence_id":"REQ-DEF-02-NO-IMPLEMENTATION","evidence_ref_ids":[],"evidence_type":"ARTIFACT","proof_mode":"CONTENT_HASH","scope":"DEF-02 current no-implementation proof","status":"UNRESOLVED"}],"verification_command":{"commands":[],"mode":"UNRESOLVED","not_applicable_review":null}}
```
...
- `reviewed_input_sha256` (pre-record): `85e6977a19cc0a34bac4d0566cee8c3b9723cb06c102a7465f7d55547513f6f2`
- `reviewed_inventory_sha256` (pre-record): `296dbf85c4a182ffb8a4d8a8882742d9defa5308ae2cb76deb0cf34e3fb8836d`
...
176, inside `## G. Explicitly deferred from the first release` (L173):

> - GBrain as a mandatory dependency;

`text_digest` and `EV-DEF-02-SOURCE.content_sha256` both recomputed over the
normalized L176-176 span → `a25c877de6325fe15f0aba062fc45e7a6bed29159b644df454df05fafc511fa1`, matching the
stored values. `required_acceptance_text` is `"GBrain as a mandatory dependency"`.

## Reasoning

**What the source clause actually demands, and whether it is covered.** The
clause excludes *GBrain as a mandatory dependency*. The demanded proof is
narrower than "GBrain is absent" and I checked that the enumerated item is not
mis-scoped against it: `REQ-DEF-02-NO-IMPLEMENTATION` says "the deferred scope
has no implementation in the current bytes", and the deferred scope here is
*mandatory dependency status*, not GBrain evaluation — which `REG-D-02` keeps
alive as a `Deferred` benchmark. The item's wording tracks the row's own deferred
scope rather than the capability name, so it covers the right thing.
`REQ-DEF-02-SPEC-REVIEW` binds S20 ("memory benchmark, GBrain"), the exact spec
where a mandatory dependency would appear if it appeared anywhere.

**Adjacency to `DISP-R-1`, checked deliberately.** `DISP-R-1` is the ledger's one
row with a mapped, mechanized no-implementation predicate, and its requirement
description is "Current S20 draft preserves D-02 as dormant and contains no
implementation claim" — same spec, same register decision. The two obligations do
not merge: `DISP-R-1`'s is a *rejection*-record obligation with a closed
validator predicate, `DEF-02`'s is a §G release-scope obligation with none. Each
row carries its own, and neither is missing.
...
|---|---|---|---|
| `REQ-DEF-02-ACCEPTANCE` | `ARTIFACT` | `CONTENT_HASH` | the clause's own acceptance scope |
| `REQ-DEF-02-SPEC-REVIEW` | `REVIEW` | `CONTENT_HASH` | a persisted clean review of the current specification bytes |
| `REQ-DEF-02-NO-IMPLEMENTATION` | `ARTIFACT` | `CONTENT_HASH` | current negative proof that the deferred scope has no implementation in the current bytes |
...
whose `program_disposition` is `REJECTED_ACCOUNTED` (`:2734-2736`). `DEF-02` is
...
`REQ-DEF-02-NO-IMPLEMENTATION`. Its future satisfaction is governed by the
...
- *`TYPED_APPROVAL`.* Goal L484-487 requires a `TYPED_APPROVAL` item to name
  ...
  requirement is `APR-DEF-02-01`, of type `DELEGATED_ARTIFACT_APPROVAL`. Swept all
...
- *`REVIEW` — present, and checked against the known defect population.* Of the
  ...
  ("All 32 disposition components omit required review evidence"). `DEF-02` is in
...
Two references: `EV-DEF-02-SOURCE` (UTF8_LINE_SPAN over `docs/blueprint/funda-blueprint-implementation-decision-register-v2.md`, L176-176, captured `2026-08-13T02:49:11Z`); `EV-DEF-02-SPEC-DRAFT` (FILE_BYTES over `docs/specs/equity-os-s20-memory-benchmark-gbrain.md`, captured `2026-08-15T07:13:28Z`). Every one resolved and re-digested during the structural validation
...
1. **The `ACCEPTANCE` item's description is still positively framed.**
   `REQ-DEF-02-ACCEPTANCE.description` reads "Current proof satisfying:
   GBrain as a mandatory dependency" — a deferral clause demands no proof that its
...
2. **A vendor model lane survives in `REQ-DEF-02-SPEC-REVIEW.description`**
...
`required_evidence` for `DEF-02` is complete at the input bytes pinned above: the
