# Inventory review — DEF-11 / SCOPE / r0
...
- `text_digest` recomputed over the normalized L185-185 span
  ...
  `26a7689b0e491988e66664a011909b7d2f32d717532828156940f860d9190bee`,
  equal to the stored `text_digest`.
- `required_acceptance_text` is `"live execution"`, which is
  that span with the list marker and terminal punctuation removed — the exact
  clause content.

## Reasoning

**Kind.** Bullet 11 of register §G (L185), excluding live execution — the
highest-consequence exclusion in §G. `first_release_deferral` is correct by the
normalization rule, and the kind matters more here than elsewhere: an
`ACTIVE_NEGATIVE_CONTROL` `phase_gate_clause` would also be a "prove it stays
out" control, but that rule is allowed only for `phase_gate_clause` (goal L259)
and requires nonempty exact `related_register_ids`, which this kind is forbidden
to carry. The §G source section settles it.
...
**Related register IDs.** `[]`, contract-forced. The register keeps live
execution behind **two** Critical `Deferred` rows: `REG-E-09` ("Keep execution
in a separate trust domain", acceptance naming deterministic limits, approvals,
a kill switch, and reconciliation) and `REG-E-08` ("Gate paid/public/personalized
research on current legal review"), which `E-09` depends on. Both are dormant,
and between them they carry the `EXECUTION_TRUST_DOMAIN_APPROVAL`,
`LEGAL_REVIEW`, `REGULATORY_REVIEW`, and `DISTRIBUTION_APPROVAL` requirements
that any live-execution activation would have to satisfy.

That two-row depth is why `[]` here is not a loss of safety: the exclusion this
row asserts is redundantly backed by two register rows whose own derivations keep
the capability `CONDITIONAL_UNACTIVATED`. This row adds the release-scope
statement that live execution is out *now*, independent of how E-08/E-09 later
resolve.

**Source anchor and span.** `source_anchor` is `DEF-11`, a clause ordinal.
...
**Applicable review slot.** `DEF-11` is a non-register canonical row, so
...
only that `DEF-11`'s scope derivation is correct at the input bytes pinned above.
