# Inventory review — DEF-11 / APPROVAL / r0
...
```json
{"approval_records":[],"human_review_id":["HR-0004"],"required_approvals":[{"actor":null,"approval_id":"APR-DEF-11-01","approval_type":"DELEGATED_ARTIFACT_APPROVAL","evidence_ref_ids":[],"matched_record_id":null,"required_authority":"Delegated fresh Sol xhigh specification reviewer","scope":"DEF-11 under S04","status":"UNRESOLVED","timestamp":null}],"security_exception_ids":[]}
```
...
- `reviewed_inventory_sha256` (pre-record): `3e9ffe09a535e9b8db5e8a738250d1a4e209e0365ded71fb7be111dad58de995`
...
`text_digest` recomputed over the normalized L185-185 span →
`26a7689b0e491988e66664a011909b7d2f32d717532828156940f860d9190bee`, matching the stored value.
...
**The highest-consequence exclusion, checked exhaustively rather than by
analogy.** Live execution is where an omitted approval would do the most damage,
so I enumerated every authority the register attaches anywhere to the execution
path and confirmed each has a home that is not this row:

| Authority | Type | Where it sits | Why not here |
|---|---|---|---|
| `Execution-boundary owner` | `EXECUTION_TRUST_DOMAIN_APPROVAL` | `REG-E-09` (only row) | Authorises crossing the boundary under controls; this clause does not cross it |
| `Competent legal reviewer` | `LEGAL_REVIEW` | `REG-E-08` (only row) | Attaches to the intended distribution mode, which this clause does not enter |
| `Competent regulatory reviewer` | `REGULATORY_REVIEW` | `REG-E-08` (only row) | Same |
| `Distribution owner` | `DISTRIBUTION_APPROVAL` | `REG-E-08` (only row) | Nothing is distributed |
| `Product owner authorized to activate deferred blueprint scope` | `PRODUCT_OWNER_DECISION` | `REG-E-08`, `REG-E-09` | Activates dormant scope; this row is not dormant scope |
| `Product owner` | `PRODUCT_OWNER_DECISION` | `REG-A-01` | Fixes the audience and execution-linkage boundary; this clause reports a consequence |

Six candidate authorities, six homes, none of them this row. The result is not
that live execution is lightly governed — it is governed by more approvals than
anything else in the register — but that all of that governance attaches to
doing it.

**The enumerated obligation.** One requirement, `APR-DEF-11-01`, type
`DELEGATED_ARTIFACT_APPROVAL`, authority "Delegated fresh Sol xhigh
specification reviewer", scope "DEF-11 under S04", `UNRESOLVED`. It is the delegated
...
| `PRODUCT_OWNER_DECISION` (incl. `Product owner authorized to activate deferred blueprint scope`) | `REG-E-09` and, upstream of it, `REG-E-08` — both `Deferred`, both Critical — carry the activate-deferred authority. Live execution activates through that pair or not at all. |
...
| `DATA_RIGHTS_APPROVAL`, `LEGAL_REVIEW`, `REGULATORY_REVIEW` | No data is acquired and no regulated activity performed by an exclusion. `REG-E-08` holds `LEGAL_REVIEW` (`Competent legal reviewer`) and `REGULATORY_REVIEW` (`Competent regulatory reviewer`) — the ledger's only instance of each — precisely because *performing* regulated research and execution needs them. |
| `DISTRIBUTION_APPROVAL`, `EXECUTION_TRUST_DOMAIN_APPROVAL` | `DISTRIBUTION_APPROVAL` (`Distribution owner`, one row: `REG-E-08`) and `EXECUTION_TRUST_DOMAIN_APPROVAL` (`Execution-boundary owner`, one row: `REG-E-09`) are the two authorities a live-execution capability would most obviously need. Both sit on the `Deferred` register rows that would perform the activity. Neither is demanded by a clause that keeps it out of the release. |
...
`required_approvals`. `DEF-11` is linked to `HR-0004` only.
...
`required_approvals` for `DEF-11` is complete at the input bytes pinned above: the
source clause demands exactly one typed approval — the delegated artifact
approval already enumerated as `APR-DEF-11-01` — and no authority whose
