# Inventory review — PG-05-07 / SCOPE / r0
...
```json
{"activation_predicate":null,"disposition_refs":[],"gate_refs":[],"related_register_ids":["B-05","B-06"],"scope_derivation":{"authority_effect":null,"derived_program_disposition":"REQUIRED_NOW","related_register_ids":["B-05","B-06"],"rule":"RELATED_REGISTER_SCOPE"}}
```
...
- `reviewed_inventory_sha256` (pre-record): `56d84265d2891acd597a679a753d906cd34c3607c53bad555b641c98c1999140`
...
**Related register IDs — `["B-05", "B-06"]`.** The clause names two schemas and one
provenance condition, and the registers split cleanly:

- "minimum fact … schemas" → B-05 (register L55), "Derive minimum source and fact schemas
  from actual use".
- "claim schemas" → B-06 (register L56), "Derive minimum typed claim schema".
- "based on actual workflow evidence" → B-05's own "**from actual use**", the same
  provenance requirement in the register's words.

Both register titles begin with "Derive … minimum", matching the clause's "minimum … are
based on"; the correspondence is lexical, not inferred.

Backlinks: `REG-B-05.gate_refs == ["PG-05-07", "PG-1-03"]` — B-05 also feeds the Phase 1
explicitness gate, which is why its ID appearing here is not sufficient on its own and I
checked the clause text rather than the backlink alone. `REG-B-06.gate_refs ==
["PG-05-07"]`, a singleton.

B-12 is the near-miss: B-06's acceptance requires a "registered predicate", so claim
schemas depend on the registries. But dependency is not source semantics (goal L233-235),
and B-12's gate is `PG-05-06` in this same batch.

**Aggregated disposition.** B-05 and B-06 are both `REQUIRED_NOW`; the aggregation
...
**Phase placement checked.** `blueprint_phase == "0.5"` and the occurrence is the
seventh bullet under the Phase 0.5 heading at register L135, so phase, anchor
(`F-0.5-07`), and line all agree. This matters because B-05 is also cited by a Phase 1
gate; a phase mismatch here would be the first sign of a mis-anchored occurrence.
