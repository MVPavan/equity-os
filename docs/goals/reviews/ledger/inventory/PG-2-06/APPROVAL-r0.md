# Inventory review — PG-2-06 / APPROVAL / r0
...
`primary_spec` is `null` on every phase-gate row. The artifact approvals sit on `REG-D-02` and `REG-D-05`, both under S20.
...
**Applying it to this clause.** "…are **recorded**" is a documentation verb, and
the ledger's most consistent signal: every gate clause whose verb is
"documented", "recorded", or "versioned" carries an empty approval list,
including `PG-0A-01`, `PG-0A-07`, `PG-0A-05`, and `PG-05-03`. Nothing here is
approved, accepted, agreed, or decided.

**The in-batch contrast that makes this concrete.** `PG-2-05` and `PG-2-06`
share the same related register `D-05` and differ only in what they assert about
it. `PG-2-05` says operational burden "**is acceptable**" and carries
`PRODUCT_OWNER_DECISION` / "Product owner". `PG-2-06` says triggers "**are
recorded**" and carries nothing. If the approval inventory were being derived
from the register cone rather than from the clause text, the two rows would
carry the same list. They do not, which is the pattern working as goal L535-537
specifies: derivation from "exact source acceptance text" first.

**The near-miss, checked explicitly: "regardless of the current engine
decision".** The clause names the engine decision, and that decision *is* an
approval — `REG-D-05` carries `PRODUCT_OWNER_DECISION` / "Product owner for
memory adoption" for it. One could argue this clause therefore depends on a
product-owner sign-off. I rejected it: the clause's grammar makes the decision a
**quantified-over variable**, not a required act. "Regardless of X" asserts that
the obligation holds for every value of X, including the value where no adoption
occurred; it is the opposite of requiring X to be decided a particular way. The
mechanization confirms the reading — `AP-PG-2-06`'s `COMPARE_METRICS` leaf
requires only that `current_engine_decision_id` *equal* the decision the trigger
policy was written against, whatever that decision was, with no comparison to
any approved value.
...
|---|---|
| `PRODUCT_OWNER_DECISION` | The near-miss, argued above. The adoption and activation authorities sit on `REG-D-05` and `REG-D-02`; goal L349-352 locates activation authority on the register transition, and this clause requires a record, not a decision. |
| `MEMORY_PROMOTION` | Nothing is promoted to canonical memory; the clause concerns a policy document. |
| `ANALYST_ACCEPTANCE` | No thesis, narrative, or analytic output is accepted. A trigger threshold is an operating parameter, not an analytic judgment. |
| `DOMAIN_EXPERT_ACCEPTANCE` | Corpus size, graph-query volume, and retrieval miss rate are operational thresholds, not calculation, data, entity, or vocabulary rulings. |
| `BUDGET_APPROVAL`, `CAPACITY_COMMITMENT`, `NAMED_OWNER_COMMITMENT` | Recording a trigger commits no spend, capacity, or owner; the commitments would arise only if a trigger later fired and re-evaluation were activated, and those would be `D-02`/`D-05` obligations at that time. |
