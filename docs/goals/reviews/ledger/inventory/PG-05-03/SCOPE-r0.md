# Inventory review — PG-05-03 / SCOPE / r0
...
```json
{"activation_predicate":null,"disposition_refs":[],"gate_refs":[],"related_register_ids":["A-03","B-04"],"scope_derivation":{"authority_effect":null,"derived_program_disposition":"REQUIRED_NOW","related_register_ids":["A-03","B-04"],"rule":"RELATED_REGISTER_SCOPE"}}
```
...
- `reviewed_inventory_sha256` (pre-record): `f61384d66f5e3c7278bbc2a9ebf80a6d2b4af153fa3e34450ce3a0321552787d`
...
**Related register IDs — `["A-03", "B-04"]`.** The clause names two recorded
quantities, and each has one owner:

- "the manual baseline … time" → A-03 (register L33), which requires Quarter 0 to be
  "completed manually with time-stamped reading …" and that "the same lightweight
  instrumentation is used in manual and assisted workflows and **its overhead is
  recorded**".
- "all three report-level review times are recorded" → B-04 (register L54), "Measure
  analyst review economics without invalid percentiles", whose first acceptance term is
  "Record each report's total review time".

The near-miss here is B-02, which produces the three reports whose review times this
clause counts. B-02 is the *production* obligation and is gated at `PG-05-02`; this
clause is about the *measurement record*. Distinguishing the two is the whole point of
having both gates, and conflating them would make `PG-05-03` an unmotivated restatement
of `PG-05-02`.

Backlinks: `REG-A-03.gate_refs == ["PG-05-02", "PG-05-03"]` — A-03 is coupled to both
gates, exactly matching the two different things this clause and the previous one draw
from it — and `REG-B-04.gate_refs == ["PG-05-03", "PG-05-04", "PG-1-08"]`.

**Aggregated disposition.** A-03 and B-04 are both `REQUIRED_NOW`, so the aggregation
...
**No human-review link, and that is correct.** `human_review_id` is `null` and this row
carries a single transition, the bootstrap `ACTIVATION_SNAPSHOT` `TR-PG-05-03-000`. It is
one of the eight rows in this batch that the HR-0004 transaction did not touch, and
direct lookup in the canonical human-review artifact returns zero occurrences of
`PG-05-03`. That is outside the `SCOPE` projection but confirms the scope object here has
never been reconciled or amended since bootstrap.
