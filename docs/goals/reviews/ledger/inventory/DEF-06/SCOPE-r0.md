# Inventory review — DEF-06 / SCOPE / r0
...
- `text_digest` recomputed over the normalized L180-180 span
  ...
  `3996aafc6a7302db1e5065c5f606382e7bcb02a65b825e0e6103b16cc17b9465`,
  equal to the stored `text_digest`.
- `required_acceptance_text` is `"broad sector-pack library"`, which is
  that span with the list marker and terminal punctuation removed — the exact
  clause content.

## Reasoning

**Kind.** Bullet 6 of register §G (L180), excluding a broad sector-pack library.
`first_release_deferral` is correct: it is a §G bullet, it names a capability
breadth rather than a phase-exit condition, and it has no finding ordinal or
register ID. The word "broad" is doing the work — the exclusion is of *library
breadth*, not of sector handling as such, and I confirmed the register keeps
narrow sector work alive elsewhere rather than excluding it here.
...
**Related register IDs.** `[]`, contract-forced. The register's sector work lives
inside `REG-E-01` ("Add model-grade financial compute", Status `Deferred`), whose
acceptance text names "sector definitions" alongside statement tie-outs,
DCF/SOTP/WACC, and sensitivities, and whose spec S21 ("conditional model-grade
compute") is this row's spec too. `E-01` is dormant and carries a
`NAMED_OWNER_COMMITMENT` to a `Model-grade compute owner`, a
`CAPACITY_COMMITMENT`, a `BUDGET_APPROVAL`, and the activate-deferred
`PRODUCT_OWNER_DECISION` — five authorities for *building* it. None of that
transfers to the exclusion, and naming `E-01` here would be the inferred padding
goal L232-234 forbids.

**Source anchor and span.** `source_anchor` is `DEF-06`, a clause ordinal.
...
**Applicable review slot.** `DEF-06` is a non-register canonical row, so
...
only that `DEF-06`'s scope derivation is correct at the input bytes pinned above.
