# Inventory review — DEF-06 / EVIDENCE / r0
...
```json
{"evidence_refs":[{"captured_at":"2026-08-13T02:49:11Z","content_sha256":"3996aafc6a7302db1e5065c5f606382e7bcb02a65b825e0e6103b16cc17b9465","digest_mode":"UTF8_LINE_SPAN","end_line":180,"evidence_ref_id":"EV-DEF-06-SOURCE","path":"docs/blueprint/funda-blueprint-implementation-decision-register-v2.md","scope":"Exact authoritative source occurrence for DEF-06","start_line":180},{"captured_at":"2026-08-15T07:13:28Z","content_sha256":"85d3f7fd2b6cc48b415772d11db84ce6b4ed8845b8a5104a7503f16dbd14ab75","digest_mode":"FILE_BYTES","end_line":null,"evidence_ref_id":"EV-DEF-06-SPEC-DRAFT","path":"docs/specs/equity-os-s21-conditional-model-grade-compute.md","scope":"Current draft specification bytes for DEF-06","start_line":null}],"required_evidence":[{"approval_ids":[],"description":"Current proof satisfying: broad sector-pack library","evidence_id":"REQ-DEF-06-ACCEPTANCE","evidence_ref_ids":[],"evidence_type":"ARTIFACT","proof_mode":"CONTENT_HASH","scope":"DEF-06 acceptance and delivery scope","status":"UNRESOLVED"},{"approval_ids":[],"description":"Persisted clean fresh Sol xhigh review of the current specification bytes","evidence_id":"REQ-DEF-06-SPEC-REVIEW","evidence_ref_ids":[],"evidence_type":"REVIEW","proof_mode":"CONTENT_HASH","scope":"DEF-06 under S21","status":"UNRESOLVED"},{"approval_ids":[],"description":"Current negative proof that the deferred scope has no implementation in the current bytes","evidence_id":"REQ-DEF-06-NO-IMPLEMENTATION","evidence_ref_ids":[],"evidence_type":"ARTIFACT","proof_mode":"CONTENT_HASH","scope":"DEF-06 current no-implementation proof","status":"UNRESOLVED"}],"verification_command":{"commands":[],"mode":"UNRESOLVED","not_applicable_review":null}}
```
...
- `reviewed_input_sha256` (pre-record): `502dd9b1b98ea17bc2b06d39cb9ad3806354de41cc8807fd9764822e87743418`
- `reviewed_inventory_sha256` (pre-record): `24dc01f84b6dacdaba9a6182cd108260933657bbbc110e6bb733d844d0d80e85`
...
180, inside `## G. Explicitly deferred from the first release` (L173):

> - broad sector-pack library;

`text_digest` and `EV-DEF-06-SOURCE.content_sha256` both recomputed over the
normalized L180-180 span → `3996aafc6a7302db1e5065c5f606382e7bcb02a65b825e0e6103b16cc17b9465`, matching the
stored values. `required_acceptance_text` is `"broad sector-pack library"`.

## Reasoning

**What the source clause actually demands, and whether it is covered.** The
clause excludes a *broad sector-pack library*. The demanded proof is about
breadth: that current bytes do not ship a general library of sector packs. That
is `REQ-DEF-06-NO-IMPLEMENTATION`, bound through `REQ-DEF-06-SPEC-REVIEW` to S21
("conditional model-grade compute"), whose register counterpart `REG-E-01`
carries "sector definitions" inside its own deferred acceptance text.

**A named-owner obligation, tested and rejected.** `REG-E-01` carries a
`NAMED_OWNER_COMMITMENT` to a `Model-grade compute owner`, and goal L487-490
requires owner evidence to travel as `TYPED_APPROVAL`. That obligation attaches
to *building* model-grade compute with sector definitions, which is what `E-01`
would do on activation. A library that is not built needs no owner, so no
`NAMED_OWNER` evidence item is demanded on this row. Checked rather than assumed,
because the shared spec S21 makes the two rows easy to conflate.
...
|---|---|---|---|
| `REQ-DEF-06-ACCEPTANCE` | `ARTIFACT` | `CONTENT_HASH` | the clause's own acceptance scope |
| `REQ-DEF-06-SPEC-REVIEW` | `REVIEW` | `CONTENT_HASH` | a persisted clean review of the current specification bytes |
| `REQ-DEF-06-NO-IMPLEMENTATION` | `ARTIFACT` | `CONTENT_HASH` | current negative proof that the deferred scope has no implementation in the current bytes |
...
whose `program_disposition` is `REJECTED_ACCOUNTED` (`:2734-2736`). `DEF-06` is
...
`REQ-DEF-06-NO-IMPLEMENTATION`. Its future satisfaction is governed by the
...
- *`TYPED_APPROVAL`.* Goal L484-487 requires a `TYPED_APPROVAL` item to name
  ...
  requirement is `APR-DEF-06-01`, of type `DELEGATED_ARTIFACT_APPROVAL`. Swept all
...
- *`REVIEW` — present, and checked against the known defect population.* Of the
  ...
  ("All 32 disposition components omit required review evidence"). `DEF-06` is in
...
Two references: `EV-DEF-06-SOURCE` (UTF8_LINE_SPAN over `docs/blueprint/funda-blueprint-implementation-decision-register-v2.md`, L180-180, captured `2026-08-13T02:49:11Z`); `EV-DEF-06-SPEC-DRAFT` (FILE_BYTES over `docs/specs/equity-os-s21-conditional-model-grade-compute.md`, captured `2026-08-15T07:13:28Z`). Every one resolved and re-digested during the structural validation
...
1. **The `ACCEPTANCE` item's description is still positively framed.**
   `REQ-DEF-06-ACCEPTANCE.description` reads "Current proof satisfying:
   broad sector-pack library" — a deferral clause demands no proof that its
...
2. **A vendor model lane survives in `REQ-DEF-06-SPEC-REVIEW.description`**
...
`required_evidence` for `DEF-06` is complete at the input bytes pinned above: the
