# Inventory review — DEF-06 / APPROVAL / r0
...
```json
{"approval_records":[],"human_review_id":["HR-0004"],"required_approvals":[{"actor":null,"approval_id":"APR-DEF-06-01","approval_type":"DELEGATED_ARTIFACT_APPROVAL","evidence_ref_ids":[],"matched_record_id":null,"required_authority":"Delegated fresh Sol xhigh specification reviewer","scope":"DEF-06 under S21","status":"UNRESOLVED","timestamp":null}],"security_exception_ids":[]}
```
...
- `reviewed_inventory_sha256` (pre-record): `17f30b151cf47265f4e72386d169a62c98e1dcc72365eded8b859b3e73f566e7`
...
`text_digest` recomputed over the normalized L180-180 span →
`3996aafc6a7302db1e5065c5f606382e7bcb02a65b825e0e6103b16cc17b9465`, matching the stored value.
...
**A named-owner commitment, tested and rejected.** `NAMED_OWNER_COMMITMENT`
exists on exactly three rows with three distinct authorities — `Golden-set owner`
(`REG-A-08`), `Event-monitoring owner` (`REG-E-04`), and `Model-grade compute
owner` (`REG-E-01`). The third shares this row's spec, S21, and a sector-pack
library is exactly the kind of artefact that needs a named maintainer, so this is
the candidate to test. It fails on what the type means here: a named-owner
commitment is a person undertaking to own a thing that exists. A library that is
excluded from the release has nothing to own. Note also that two of those three
authorities (`Event-monitoring owner`, `Model-grade compute owner`) are recorded
in `AUTHORIZED_AUTHORITY_ADDITIONS` at
`validate_ledger_structural.py:2614-2617` as deliberate additions — the
vocabulary is tightly held, and adding an entry here would be a contract change,
not an inventory fix.

**The enumerated obligation.** One requirement, `APR-DEF-06-01`, type
`DELEGATED_ARTIFACT_APPROVAL`, authority "Delegated fresh Sol xhigh
specification reviewer", scope "DEF-06 under S21", `UNRESOLVED`. It is the delegated
...
| `PRODUCT_OWNER_DECISION` (incl. `Product owner authorized to activate deferred blueprint scope`) | `REG-E-01` ('Add model-grade financial compute', `Deferred`), whose acceptance text carries the register's sector-definition work, holds the activate-deferred authority. Building a broad sector-pack library would activate through it, not through this row. |
...
`required_approvals`. `DEF-06` is linked to `HR-0004` only.
...
`required_approvals` for `DEF-06` is complete at the input bytes pinned above: the
source clause demands exactly one typed approval — the delegated artifact
approval already enumerated as `APR-DEF-06-01` — and no authority whose
