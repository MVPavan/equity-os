# Inventory review — DISP-M-7 / EVIDENCE / r0
...
```json
{"evidence_refs":[{"captured_at":"2026-08-13T02:49:11Z","content_sha256":"90270f7df30c0860334933a72cf82cdc3e45ee2a28fe87cd601fad6114a486aa","digest_mode":"UTF8_LINE_SPAN","end_line":238,"evidence_ref_id":"EV-DISP-M-7-SOURCE","path":"docs/blueprint/funda-third-order-review-disposition-report.md","scope":"Exact authoritative source occurrence for DISP-M-7","start_line":226},{"captured_at":"2026-08-15T07:13:28Z","content_sha256":"dbb6b8600de771e9ae668208a9893394321ce67fb366c706c2d9c98985ee85aa","digest_mode":"FILE_BYTES","end_line":null,"evidence_ref_id":"EV-DISP-M-7-SPEC-DRAFT","path":"docs/specs/equity-os-s17-entity-security-master-actions.md","scope":"Current draft specification bytes for DISP-M-7","start_line":null}],"required_evidence":[{"approval_ids":[],"description":"Current proof satisfying: ### M-7 — Entity and security master authority\n\n**Disposition: Accept, but do not use ISIN as the internal primary key.**\n\nFunda should use stable internal `company_id` and `security_id` values. ISIN, exchange symbol, CIN, LEI, and other identifiers are versioned external mappings with valid-time and knowledge-time intervals.\n\nThe decision must name:\n\n- source hierarchy for each identifier type;\n- conflict-resolution rule;\n- symbol and listing changes;\n- corporate-action handling;\n- one real test case involving an identifier change.","evidence_id":"REQ-DISP-M-7-ACCEPTANCE","evidence_ref_ids":[],"evidence_type":"ARTIFACT","proof_mode":"CONTENT_HASH","scope":"DISP-M-7 acceptance and delivery scope","status":"UNRESOLVED"},{"approval_ids":[],"description":"Reproducible command result proving the current DISP-M-7 acceptance obligation","evidence_id":"REQ-DISP-M-7-COMMAND-PROOF","evidence_ref_ids":[],"evidence_type":"COMMAND_RESULT","proof_mode":"COMMAND","scope":"DISP-M-7 command proof","status":"UNRESOLVED"}],"verification_command":{"commands":[],"mode":"UNRESOLVED","not_applicable_review":null}}
```
...
- `reviewed_inventory_sha256` (pre-record): `42b8f7e4af05fd917134ce1fcfe2bdec7212675e09e534baee3a4ec56956a66e`
...
|---|---|---|---|---|---|
| `REQ-DISP-M-7-ACCEPTANCE` | `ARTIFACT` | `CONTENT_HASH` | `UNRESOLVED` | `[]` | `[]` |
| `REQ-DISP-M-7-COMMAND-PROOF` | `COMMAND_RESULT` | `COMMAND` | `UNRESOLVED` | `[]` | `[]` |
...
**Clause-by-clause coverage.** The clause's decision content — use stable internal
`company_id` and `security_id`; treat ISIN, exchange symbol, CIN and LEI as
versioned external mappings with valid-time and knowledge-time intervals; and name
the source hierarchy, the conflict-resolution rule, symbol and listing changes, and
corporate-action handling — is provable by hashing the approved S17 specification
bytes, hence the `ARTIFACT`/`CONTENT_HASH` item.

The clause's fifth "must name" item is different in kind: "**one real test case
involving an identifier change**". That is an executable obligation, and it is
carried by `REQ-DISP-M-7-COMMAND-PROOF`, typed `COMMAND_RESULT`/`COMMAND` with
scope "DISP-M-7 command proof". The register control agrees — `C-17`'s acceptance
ends "one real identifier-change case **tested**" — and `REG-C-17` carries its own
`REQ-REG-C-17-COMMAND-PROOF`, so the obligation is classified consistently on both
the disposition occurrence and the register decision it controls.
...
`EV-DISP-M-7-SOURCE` (`UTF8_LINE_SPAN`, disposition report L226-238) and
`EV-DISP-M-7-SPEC-DRAFT` (`FILE_BYTES`, S17 draft).
...
must become `COMMANDS` with a command binding this requirement before terminal use.
Recorded as a forward obligation, not a finding.
...
**What is missing.** This row's `required_approvals` contains `APR-DISP-M-7-01`,
`DELEGATED_ARTIFACT_APPROVAL`, `Delegated fresh Sol xhigh specification reviewer`,
scope `M-7 under S17`. Under goal L598-600 a record satisfying that requirement
...
`REVIEWER`-role review**". The persisted review of S17's current
...
`EV-DISP-M-7-SPEC-DRAFT` is a `FILE_BYTES` reference to `docs/specs/equity-os-s17-entity-security-master-actions.md` — the exact
...
2. **The reviewed generator makes the pairing unconditional.**
   ...
   emits `REQ-DISP-M-7-SPEC-REVIEW` for this component. The canonical ledger contains
   no such item.
...
4. **The proof cannot be borrowed from a sibling row.** Goal L439 requires review
   evidence to be "current and **component-local**". S17 owns register rows
   C-06, C-07, C-17, each of which carries its own `REQ-REG-*-SPEC-REVIEW`; none of them
...
`REQ-DISP-M-7-SPEC-REVIEW`, `evidence_type` `REVIEW`, `proof_mode` `CONTENT_HASH`,
`status` `UNRESOLVED`, `evidence_ref_ids` `[]`, `approval_ids` `[]`, `scope`
`M-7 under S17`, description "Persisted clean fresh Sol xhigh review of the
...
recording design r2 §5.4 the `EVIDENCE` review on `DISP-M-7` therefore stays
`PENDING`, this artifact is the durable record of the finding, and the finding
belongs in `open_findings` on `DISP-M-7` with severity, load-bearing status, artifact
...
(recording design r2 §3.4), no review on `DISP-M-7` is recordable while this finding
stands.
