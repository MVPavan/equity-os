# Inventory review — DISP-M-7 / APPROVAL / r0
...
```json
{"approval_records":[],"human_review_id":["HR-0004"],"required_approvals":[{"actor":null,"approval_id":"APR-DISP-M-7-01","approval_type":"DELEGATED_ARTIFACT_APPROVAL","evidence_ref_ids":[],"matched_record_id":null,"required_authority":"Delegated fresh Sol xhigh specification reviewer","scope":"M-7 under S17","status":"UNRESOLVED","timestamp":null}],"security_exception_ids":[]}
```
...
- `reviewed_inventory_sha256` (pre-record): `fdb946415f7138b8727de33d89d9068c1d8b2b51600c48624ec2935d18b10437`
...
|---|---|---|---|---|
| `APR-DISP-M-7-01` | `DELEGATED_ARTIFACT_APPROVAL` | `Delegated fresh Sol xhigh specification reviewer` | `M-7 under S17` | `UNRESOLVED` |
...
Report L226-238 states a decision — use stable internal `company_id` and
`security_id`, treat ISIN and other external identifiers as versioned mappings —
and then lists five things "**The decision must name**": source hierarchy for each
identifier type; conflict-resolution rule; symbol and listing changes;
corporate-action handling; one real test case involving an identifier change. Every
one of the five is a required *content* of the decision document. None is an act
requiring authorization, and the clause contains no verb of approval, acceptance,
commitment, or ownership.

Near-miss examined and rejected: `DOMAIN_EXPERT_ACCEPTANCE` / `Entity-data
authority`, which the related register `REG-C-17` carries. This is the strongest
candidate in the batch after `DISP-M-3`, because `C-17` and this clause are near
restatements of one another and identifier authority is genuinely a data-domain
judgement. It is nevertheless rejected, on the distinction the goal itself draws
between a clause that *names an approval* and a clause that *names required
content*: `C-17` carries the sign-off because the register decision must be
accepted by a data-domain authority, whereas this occurrence constrains what that
decision must say. The contrast with `DISP-M-3` in this same batch is the
discriminator — `M-3`'s clause requires "a human approval rule for additions", an
approval obligation in the clause's own words, and its `APPROVAL` review returned
`ISSUES_FOUND` for exactly that reason. `M-7`'s clause requires naming
corporate-action handling and a conflict-resolution rule; naming is not approving.

Sibling check: `DISP-6-3`, the narrower ISIN occurrence under the same spec and the
same related register, likewise carries only the delegated approval.
...
authority of any kind (goal L~624-626). It records only that `DISP-M-7`'s
