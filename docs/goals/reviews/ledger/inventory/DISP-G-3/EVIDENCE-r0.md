# Inventory review — DISP-G-3 / EVIDENCE / r0
...
```json
{"evidence_refs":[{"captured_at":"2026-08-13T02:49:11Z","content_sha256":"ae5b642d33d770cb932dc756db0b063455ba57c1e449022c40be7290acaa1ea4","digest_mode":"UTF8_LINE_SPAN","end_line":86,"evidence_ref_id":"EV-DISP-G-3-SOURCE","path":"docs/blueprint/funda-third-order-review-disposition-report.md","scope":"Exact authoritative source occurrence for DISP-G-3","start_line":75},{"captured_at":"2026-08-15T07:13:28Z","content_sha256":"6b59d6ef082ccca047ec119bc60331894ab1b752fd50e810634da317b0a78631","digest_mode":"FILE_BYTES","end_line":null,"evidence_ref_id":"EV-DISP-G-3-SPEC-DRAFT","path":"docs/specs/equity-os-s18-universe-review-economics-throughput.md","scope":"Current draft specification bytes for DISP-G-3","start_line":null}],"required_evidence":[{"approval_ids":[],"description":"Current proof satisfying: ### G-3 — Cross-company economics comparison\n\n**Disposition: Accept.**\n\nComparing assisted work on unfamiliar Phase 1 companies with a manual baseline from the now-familiar discovery company confounds company complexity, analyst familiarity, and tooling effect.\n\nThe gate should use:\n\n- a manual baseline for each Phase 1 company or a matched historical quarter;\n- normalized operational measures such as verification time per material claim, source-locate time, and correction time;\n- explicit complexity descriptors such as document count, page count, claim count, and number of reconciliation exceptions;\n- total report time retained as a business metric, but not treated as a portable causal measure by itself.","evidence_id":"REQ-DISP-G-3-ACCEPTANCE","evidence_ref_ids":[],"evidence_type":"ARTIFACT","proof_mode":"CONTENT_HASH","scope":"DISP-G-3 acceptance and delivery scope","status":"UNRESOLVED"}],"verification_command":{"commands":[],"mode":"UNRESOLVED","not_applicable_review":null}}
```
...
- `reviewed_inventory_sha256` (pre-record): `1544ee60970c5c36fdff3847bcd8a83cd1e9c3dbe2f368ffb882f0be5cb4e333`
...
|---|---|---|---|---|---|
| `REQ-DISP-G-3-ACCEPTANCE` | `ARTIFACT` | `CONTENT_HASH` | `UNRESOLVED` | `[]` | `[]` |

Its `description` was recomputed this round and is byte-equal to
`"Current proof satisfying: "` concatenated with the row's exact
`required_acceptance_text`, so the acceptance item carries the whole clause, not a
paraphrase.

**Clause-by-clause coverage.** The clause prescribes what the Phase 1 economics
gate "should use": a per-company or matched-quarter manual baseline; normalized
operational measures; explicit complexity descriptors; and total report time
retained as a business metric only. All four are *design content of the owning
specification*, provable by hashing the approved spec bytes — which is exactly
what one `ARTIFACT`/`CONTENT_HASH` item asserts. No sentence demands a reproducible
command: contrast `DISP-M-4`, whose clause says "tests deliberately insert
post-cutoff records and verify that retrieval excludes them" and which
consequently carries a `COMMAND_RESULT`/`COMMAND` item. The absence of a command
obligation here is a correct classification, not a gap, and it agrees with the
program-level evidence-inventory review, whose list of components with unclassified
mechanical obligations names `DISP-G-1`, `DISP-M-4`–`M-7`, `DISP-M-9`, `DISP-6-6`,
`DISP-6-9` and `SEQ-09` — not `DISP-G-3`.

No typed-approval evidence item is demanded, because the clause names no
authority; that is established separately in this component's `APPROVAL` review.

**`evidence_refs`.** Two objects, both recomputed against current bytes this round
and both matching: `EV-DISP-G-3-SOURCE` (`UTF8_LINE_SPAN`, disposition report
L75-86) and `EV-DISP-G-3-SPEC-DRAFT` (`FILE_BYTES`, S18 draft
`docs/specs/equity-os-s18-universe-review-economics-throughput.md`).

**`verification_command`.** `{"mode": "UNRESOLVED", "commands": [],
"not_applicable_review": null}`. Valid: goal L~504-506 permits `UNRESOLVED` during
initial ledger construction, structural validation exits `0` at these bytes, and
the mode is consistent with this row carrying no `COMMAND`-classified requirement.
...
**What is missing.** This row's `required_approvals` contains `APR-DISP-G-3-01`,
`DELEGATED_ARTIFACT_APPROVAL`, `Delegated fresh Sol xhigh specification reviewer`,
scope `G-3 under S18`. Under goal L598-600 a record satisfying that requirement
...
`REVIEWER`-role review**". The persisted review of S18's current
...
`EV-DISP-G-3-SPEC-DRAFT` is a `FILE_BYTES` reference to `docs/specs/equity-os-s18-universe-review-economics-throughput.md` — the exact
...
2. **The reviewed generator makes the pairing unconditional.**
   ...
   emits `REQ-DISP-G-3-SPEC-REVIEW` for this component. The canonical ledger contains
   no such item.
...
4. **The proof cannot be borrowed from a sibling row.** Goal L439 requires review
   evidence to be "current and **component-local**". S18 owns register rows
   B-04, C-01, C-12, C-18, each of which carries its own `REQ-REG-*-SPEC-REVIEW`; none of them
...
`REQ-DISP-G-3-SPEC-REVIEW`, `evidence_type` `REVIEW`, `proof_mode` `CONTENT_HASH`,
`status` `UNRESOLVED`, `evidence_ref_ids` `[]`, `approval_ids` `[]`, `scope`
`G-3 under S18`, description "Persisted clean fresh Sol xhigh review of the
...
recording design r2 §5.4 the `EVIDENCE` review on `DISP-G-3` therefore stays
`PENDING`, this artifact is the durable record of the finding, and the finding
belongs in `open_findings` on `DISP-G-3` with severity, load-bearing status, artifact
...
(recording design r2 §3.4), no review on `DISP-G-3` is recordable while this finding
stands.
