# Inventory review — PG-1-11 / APPROVAL / r0
...
**The clause contains the word "approved", and the emptiness is still correct.**
This is the only row in the batch whose source text names approval explicitly —
"…remain outside the release **unless separately approved**" — so the emptiness
has to be argued from the grammar of the clause, not from the absence of an
approval word.

The phrase is an **exception condition**, not a requirement. The gate is
satisfied by the four capabilities remaining outside; a separate approval is
what would *end* that state, not what is needed to establish it. Reading it as a
requirement would invert the clause: it would say the gate cannot pass until
someone signs off on keeping capabilities out, when in fact the gate fails the
moment someone signs off on letting them in.

The contract states this directly for this rule. Goal L262-264, on
`ACTIVE_NEGATIVE_CONTROL`: "Its gate proof is invalidated by any related register
state, activation, rejection, **approval**, or no-implementation-proof change.
It activates no capability: it proves that named capabilities stay dormant or
rejected." So an approval is modelled as an *invalidator* of this gate's proof,
arriving from the related register rows — the exact opposite of an obligation the
gate carries. A `required_approvals` entry here would additionally have to be
`SATISFIED` for the component to reach `VERIFIED` (goal L198), i.e. the gate
proving that execution stays out could only be completed by approving execution.

**Where the "separately approved" authorities actually live — enumerated.** Each
is present on the related register row, so nothing is lost by this row's
emptiness:

| Capability | Register row | Approval carrying the exception |
|---|---|---|
| GBrain (benchmark) | `REG-D-02` | `PRODUCT_OWNER_DECISION` / "Product owner authorized to activate deferred blueprint scope" |
| GBrain (adoption) | `REG-D-05` | the same activation authority, plus `PRODUCT_OWNER_DECISION` / "Product owner for memory adoption" |
| debate | `REG-E-03` | the activation authority, plus `BUDGET_APPROVAL` and a retention `PRODUCT_OWNER_DECISION` |
| backtesting | `REG-E-05` | the activation authority, plus `BUDGET_APPROVAL` |
| execution | `REG-E-09` | the activation authority, plus `EXECUTION_TRUST_DOMAIN_APPROVAL` / "Execution-boundary owner" |

Goal L613-615 forbids inferring that one decision covers two obligations. That
is satisfied structurally here rather than by duplication: each activation has
its own explicit requirement on its own row, and goal L262-263 wires this gate's
proof to those rows' state without copying their obligations.
...
`primary_spec` is `null` on every phase-gate row. Each of the five related rows carries its own delegated artifact approval under S20, S23, S25, or S04.
...
`PG-1-11` fits that measured pattern: the approval its text mentions is not an
act the gate observes, it is the act that would falsify the gate.

**Sweep of the closed non-delegated vocabulary.**

| Type | Why it is not demanded here |
|---|---|
| `PRODUCT_OWNER_DECISION` | The activation authority, and the tempting one. It belongs on the row being activated — all 5 related registers carry it — because activation is a change to *that* row's controlled state. On this row it would mean "a product owner must approve that these stay out", which the clause does not say. |
| `EXECUTION_TRUST_DOMAIN_APPROVAL` | The ledger's single instance sits on `REG-E-09`, where the execution boundary is actually defined. Keeping execution out of the release crosses no boundary and needs no boundary owner's signature. |
| `SECURITY_EXCEPTION` | Goal L536 lists "any approved security exception" among the derivation inputs. There is none: `security_exception_ids` is `[]` on this row and on all 213 rows. Keeping execution out is the fail-closed default, not an exception to it. |
| `ANALYST_ACCEPTANCE`, `MEMORY_PROMOTION` | Nothing is accepted or promoted; the clause is about scope exclusion. |
| `DOMAIN_EXPERT_ACCEPTANCE` | No calculation, data, entity, or vocabulary judgment. |
| `BUDGET_APPROVAL`, `CAPACITY_COMMITMENT`, `NAMED_OWNER_COMMITMENT` | Excluding scope commits no spend, capacity, or owner. `REG-E-03` and `REG-E-05` carry the budget obligations that would arise on activation. |
| `DATA_RIGHTS_APPROVAL`, `LEGAL_REVIEW`, `REGULATORY_REVIEW` | No acquisition, licence, or regulated activity is undertaken by not shipping something. |
...
**Residuals.** None. The "unless separately approved" question is recorded above
as a resolved judgment grounded in goal L262-264, not as an open doubt.
