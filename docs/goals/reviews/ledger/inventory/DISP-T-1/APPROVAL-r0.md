# Inventory review — DISP-T-1 / APPROVAL / r0
...
```json
{"approval_records":[],"human_review_id":["HR-0004"],"required_approvals":[{"actor":null,"approval_id":"APR-DISP-T-1-01","approval_type":"DELEGATED_ARTIFACT_APPROVAL","evidence_ref_ids":[],"matched_record_id":null,"required_authority":"Delegated fresh Sol xhigh specification reviewer","scope":"T-1 under S08","status":"UNRESOLVED","timestamp":null}],"security_exception_ids":[]}
```
...
- `reviewed_inventory_sha256` (pre-record): `8a2d531f2bcd0b8052412c7bd6560d6cc3a510e0f8f27617400f28201e76cb75`
...
**Declared.** One requirement: `APR-DISP-T-1-01`, `DELEGATED_ARTIFACT_APPROVAL`,
authority "Delegated fresh Sol xhigh specification reviewer", scope "T-1 under
S08".

**Is any authority missing?** This is the batch's strongest candidate for an
omission, because the clause names budgets and capacity directly — weekly builder
capacity, monthly provider/model/infrastructure ceilings, analyst-review capacity,
maintenance burden — and delegation explicitly excludes budget and capacity
commitments. I resolved it on the difference between *creating a place to record*
a budget and *committing* one. This clause demands the former: "Add a separate row
for …". The commitments themselves are declared on the row it created — REG-A-12
carries both `BUDGET_APPROVAL` ("Budget owner") and `CAPACITY_COMMITMENT`
("Capacity owner"), each with paired `TYPED_APPROVAL` evidence. Declaring them
here as well would record one real-world decision against two components, which
the goal forbids as directly as it forbids inferring coverage. `NAMED_OWNER_COMMITMENT`
was also considered and is not owed: the clause names no owner role, only field
families.

**Scope string.** "T-1 under S08" matches this row's single `applicable_spec_ids`
entry and its `primary_spec` object; S08 is REG-A-12's owning spec.
...
`null` → string growth. It confers no approval on this component.
...
only that `DISP-T-1`'s `required_approvals` inventory is correct at the input bytes pinned above.
