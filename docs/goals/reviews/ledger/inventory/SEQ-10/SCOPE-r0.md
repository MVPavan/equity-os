# Inventory review — SEQ-10 / SCOPE / r0
...
```json
{"activation_predicate":null,"disposition_refs":[],"gate_refs":[],"related_register_ids":[],"scope_derivation":{"applicable_spec_ids":["S14"],"authority_effect":null,"derived_program_disposition":"REQUIRED_NOW","related_register_ids":[],"rule":"PROGRAM_WIDE_ACTIVE_CONTROL","source_register_ids":["B-02"]}}
```
...
- `reviewed_inventory_sha256` (pre-record): `c746551fe769110f7b70746d2cbd6192251dc1e3c07de5ea38f555f0d5878bbf`
...
**`source_register_ids` = `["B-02"]`, verified against the clause — including the
"onward".** The lead is "**B-02 onward:**". The word "onward" is the one place in
this section where an open-ended reading could silently pad the derivation: it
could be taken to sweep in every later `B-*` row. The derivation correctly does
not, and I agree — goal L233-235 forbids inferring one axis from the other and
forbids padding, and an open-ended "onward" names no further ID with the
exactness the contract requires of a source reference. The clause's own body
confirms the narrow reading: "produce the three assisted incremental updates" is
precisely `B-02` ("Produce three real incremental earnings updates"), and "refine
the remaining schema from real failures" is a method qualifier on that work, not a
second register obligation. `validate_ledger_structural.py:2489` pins
`SEQ-10: (["B-02"], ["S14"])`.

**`applicable_spec_ids` = `["S14"]`, resolved independently.** `B-02` is
inventoried on `REG-B-02`, approval scope
`"B-02 under S14: Produce three real incremental earnings updates"`, spec draft
`docs/specs/equity-os-s14-earnings-review-workflow-rework.md` ⇒ `S14`. Single
register, single owning spec; closure `{S14}`.

**Source anchor and span.** `SEQUENCE-10` at L460-460. `text_digest` recomputed
over the normalized span → `0b6d45a3…`, matching; `required_acceptance_text`
equals that span byte for byte, including the leading "10." ordinal. Anchor and
span unique within the source path.
...
only that `SEQ-10`'s scope derivation is correct at the input bytes pinned
above.
