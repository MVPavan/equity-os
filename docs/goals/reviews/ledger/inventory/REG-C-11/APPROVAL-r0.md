# Inventory review — `REG-C-11` — `APPROVAL` — `r0`
...
**What the source clause demands.** "Prohibit product dependence on raw model
scratchpads", accepted when "Stored records are evidence, tool traces,
structured decisions, QA results, and concise rationales". A prohibition plus a
closed list of permitted record classes. No approval verb, no acceptance
language, no named authority.

**Against the enumerated inventory.** One requirement is declared:
`APR-REG-C-11-01`, `DELEGATED_ARTIFACT_APPROVAL`, the universal
specification-review obligation, scope "C-11 under S10: Prohibit product
dependence on raw model scratchpads".

**Derivation sources checked one by one.**

- *Acceptance text*: the five permitted classes are record *types*, not
  approval surfaces. "structured decisions" is the one that could be misread as
  implying a decision authority; it does not — it describes the form in which
  decisions are stored (structured records rather than free-text scratchpad),
  not who decides.
- *Phase gates*: `gate_refs` is `[]`, and the reverse scan over all thirty-five
  gate clauses found none whose `related_register_ids` contains `C-11`. The
  empty list is corroborated from the gate side.
- *Dependencies*: `dependencies` is `[]` — this row inherits no upstream
  determination at all, so the dependency axis contributes nothing and does not
  need the inheritance reasoning that `C-02` and `B-09` required.
- *Dispositions*: `T-3` and `R-5` are S10 spec-level tags. `T-3` is an
  authority-*placement* instruction (the implementation register, not the
  consolidated review, owns live gate wording) and `R-5` is explicitly retained
  as an operational note rather than a decision. `DISP-T-3` and `DISP-R-5` each
  carry only the delegated specification-review approval, so neither disposition
  is itself a source of a business authority that could flow here.
- *Fail-closed boundaries / security exceptions*: `security_exception_ids` is
  empty; `approval_records` is empty.

**Contrast within the batch.** `B-03`, which shares this row's owning spec
(S10) and its two disposition tags, *does* carry a second requirement, because
its cell says "**Approved** authority table". `C-11`'s cell says nothing of the
kind. The two rows being tagged identically at spec level while differing in
`required_approvals` is the correct outcome, not an inconsistency: the
obligation derives from the cell, not from the spec tag.

**Human-review links.** `["HR-0003","HR-0004"]`, both bidirectionally verified.
`HR-0003` carries `GOAL_OR_PROCESS_AUTHORIZATION`, a human-resolution decision
type that appears zero times in `required_approvals` ledger-wide.
