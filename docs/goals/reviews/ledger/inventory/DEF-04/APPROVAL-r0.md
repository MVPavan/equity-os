# Inventory review — DEF-04 / APPROVAL / r0
...
```json
{"approval_records":[],"human_review_id":["HR-0004"],"required_approvals":[{"actor":null,"approval_id":"APR-DEF-04-01","approval_type":"DELEGATED_ARTIFACT_APPROVAL","evidence_ref_ids":[],"matched_record_id":null,"required_authority":"Delegated fresh Sol xhigh specification reviewer","scope":"DEF-04 under S23","status":"UNRESOLVED","timestamp":null}],"security_exception_ids":[]}
```
...
- `reviewed_inventory_sha256` (pre-record): `c7e1d7c3c850fbad1f246df024d1351a567d93dd89304d0a7720d83602fb0c3c`
...
`text_digest` recomputed over the normalized L178-178 span →
`39aec8c101fa0d69c49e3880201d9dcb923ebf6ace576e496df638333e6f883c`, matching the stored value.
...
**A budget authority, tested and rejected.** `REG-E-03`'s acceptance text makes
cost explicit — "retain only if incremental valid issue detection justifies cost"
— and the row carries a `BUDGET_APPROVAL` (`Budget owner`) backed by
`REQ-REG-E-03-BUDGET_APPROVAL-03`. A multi-agent committee is the most obviously
expensive thing in §G, so a budget obligation is the candidate worth testing. It
fails cleanly: `BUDGET_APPROVAL` in this ledger authorises *spending* (six rows,
all committing model, provider, or infrastructure cost). Excluding the committee
spends nothing, and there is no such thing as approval to not spend.

**The enumerated obligation.** One requirement, `APR-DEF-04-01`, type
`DELEGATED_ARTIFACT_APPROVAL`, authority "Delegated fresh Sol xhigh
specification reviewer", scope "DEF-04 under S23", `UNRESOLVED`. It is the delegated
...
| `PRODUCT_OWNER_DECISION` (incl. `Product owner authorized to activate deferred blueprint scope`) | `REG-E-03` ('Evaluate bull/bear and forensic review', `Deferred`) carries both the activate-deferred authority and a plain `Product owner` decision, alongside a `BUDGET_APPROVAL`. Those attach to retaining an evaluated capability. The exclusion retains nothing. |
...
`required_approvals`. `DEF-04` is linked to `HR-0004` only.
...
`required_approvals` for `DEF-04` is complete at the input bytes pinned above: the
source clause demands exactly one typed approval — the delegated artifact
approval already enumerated as `APR-DEF-04-01` — and no authority whose
