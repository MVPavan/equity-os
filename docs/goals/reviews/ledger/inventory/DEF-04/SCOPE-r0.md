# Inventory review — DEF-04 / SCOPE / r0
...
- `text_digest` recomputed over the normalized L178-178 span
  ...
  `39aec8c101fa0d69c49e3880201d9dcb923ebf6ace576e496df638333e6f883c`,
  equal to the stored `text_digest`.
- `required_acceptance_text` is `"multi-agent investment committee"`, which is
  that span with the list marker and terminal punctuation removed — the exact
  clause content.

## Reasoning

**Kind.** Bullet 4 of register §G (L178), excluding a multi-agent investment
committee. Correct as `first_release_deferral`; not a `disposition_item`, since
those are the 32 numbered findings of the third-order disposition report
(`G-1`…`G-5`, `M-1`…`M-9`, `T-1`…`T-4`, `R-1`…`R-5`, `6.1`…`6.9`, goal L144) and
live in a different document with `AUTHORITATIVE_OCCURRENCE` derivation. This
occurrence is register prose and carries no finding ordinal.
...
**Related register IDs.** `[]`, contract-forced. The nearest register row is
`REG-E-03` ("Evaluate bull/bear and forensic review", Status `Deferred`), whose
acceptance is "Compare with single senior-reviewer baseline; retain only if
incremental valid issue detection justifies cost" — and the row's own spec, S23,
is "conditional bull-bear forensic review". I checked whether this bullet is a
restatement of `E-03` and concluded it is not: `E-03` keeps a *conditional
evaluation* alive against a single-reviewer baseline; the §G bullet excludes the
*committee form* from the first release outright. The two coexist without
contradiction — you may evaluate adversarial review later without ever shipping a
committee — and each is separately inventoried under its own normalization rule
(goal L140 for A-01…E-10, L142 for §G bullets).

**Source anchor and span.** `source_anchor` is `DEF-04`, a clause ordinal.
...
**Applicable review slot.** `DEF-04` is a non-register canonical row, so
...
only that `DEF-04`'s scope derivation is correct at the input bytes pinned above.
