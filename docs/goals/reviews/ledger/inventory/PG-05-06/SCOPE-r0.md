# Inventory review — PG-05-06 / SCOPE / r0
...
```json
{"activation_predicate":null,"disposition_refs":[],"gate_refs":[],"related_register_ids":["B-11","B-12"],"scope_derivation":{"authority_effect":null,"derived_program_disposition":"REQUIRED_NOW","related_register_ids":["B-11","B-12"],"rule":"RELATED_REGISTER_SCOPE"}}
```
...
- `reviewed_inventory_sha256` (pre-record): `60d3b49f2a217f0ae9f850cd9731b9565e6e6ad3d48a521bbe9fedde29587965`
...
**Related register IDs — `["B-11", "B-12"]`.** The clause is a two-term conjunction
and each term has exactly one register owner:

- "fact identity/revision rules" → B-11 (register L61), "Specify fact identity,
  revision-family, and correction semantics".
- "metric/predicate registries" → B-12 (register L62), "Establish versioned metric and
  predicate registries".

The term-to-row mapping is near-verbatim in both directions, and neither row's title
overlaps the other's clause term. Backlinks confirm exclusivity: `REG-B-11.gate_refs ==
["PG-05-06"]` and `REG-B-12.gate_refs == ["PG-05-06"]` — both singletons pointing here,
so this clause is the sole gate for both rows.

I checked B-05 and B-06 as candidates, since fact and claim schemas are adjacent subject
matter. They are the owners of the *schemas*, gated at `PG-05-07`, not of identity
semantics or registries. Adding them would pad.

**Aggregated disposition.** B-11 and B-12 are both `REQUIRED_NOW`; the aggregation
...
**"in use" is scope, not a predicate.** The clause's operative words are "are in use" —
an operational state. Under goal L288-290 a `REQUIRED_NOW` component "including one that
became `REQUIRED_NOW` by related-register aggregation" carries `activation_predicate =
null`, and this row does. The temptation to encode "in use" as a measurable predicate is
explicitly foreclosed: L292-294 says no "phase-gate exemption, or kind exemption to this
rule is authorized without its own reviewed design and approval".
