# Inventory review — PG-0A-06 / APPROVAL / r0
...
"exists", which asserts a state of an artifact, not a sign-off. §F draws that
...
**The trap on this row is in the register, and I checked it deliberately.**
`REG-A-04` carries two non-delegated authorities — `APR-REG-A-04-02`
(`PRODUCT_OWNER_DECISION`, Product owner) and `APR-REG-A-04-03`
(`ANALYST_ACCEPTANCE`, Responsible analyst) — and A-04's acceptance column ends
by requiring an "**approval record**". Read as a whole, A-04 looks like a
decision this gate could not observe without a sign-off. But A-04's acceptance is
explicitly two-staged: "A provisional v0 exists **before baseline**; **final
contract after baseline** includes … and approval record." The approval record
attaches to the final contract, whose dependency cell reads "A-03 for final
freeze". This clause binds only the provisional stage, and §F contains no bullet
for the final contract in any phase block — so the approval obligation is real,
is not source-required by *this* clause, and is inventoried on `REG-A-04` where
its own source text demands it.

**Why the related register's authority is not also owed here — the decisive
point, and it is mechanical, not stylistic.** `REG-A-04`'s two non-delegated authorities cover the final output contract, not the provisional v0 this clause observes. A reader could
...
duplicate requirement on this gate could not be satisfied by the product-owner decision or analyst acceptance recorded for A-04
...
argue that "the provisional output contract" is still a *contract*, and that a
contract implies an approving party. I do not adopt it: A-04 draws the line
itself, giving the provisional stage the single condition "exists" and reserving
the approval record for the final stage. A review that added an approval
requirement here would be reading A-04's second sentence onto a clause that
...
`PG-0A-06` regardless of kind, and it is the slot this review is written against.
...
`docs/goals/reviews/ledger/equity-os-blueprint-approval-inventory-r0.md` swept §F
for exactly this failure mode and did not name `PG-0A-06`, having read the same
A-04 text. Independent reading and prior review agree.
