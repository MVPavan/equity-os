# Inventory review — DISP-M-5 / EVIDENCE / r0
...
```json
{"evidence_refs":[{"captured_at":"2026-08-13T02:49:11Z","content_sha256":"a4ec04abb27baa34f607b4d4bca27f3e5178064f583edec0f3c870159c91e8ef","digest_mode":"UTF8_LINE_SPAN","end_line":210,"evidence_ref_id":"EV-DISP-M-5-SOURCE","path":"docs/blueprint/funda-third-order-review-disposition-report.md","scope":"Exact authoritative source occurrence for DISP-M-5","start_line":197},{"captured_at":"2026-08-15T07:13:28Z","content_sha256":"b9515d9b6fe92fb735f9ab8121dec2c7d2ba8566828896f1dc5386d6fb801912","digest_mode":"FILE_BYTES","end_line":null,"evidence_ref_id":"EV-DISP-M-5-SPEC-DRAFT","path":"docs/specs/equity-os-s14-earnings-review-workflow-rework.md","scope":"Current draft specification bytes for DISP-M-5","start_line":null}],"required_evidence":[{"approval_ids":[],"description":"Current proof satisfying: ### M-5 — Human-feedback rework transitions\n\n**Disposition: Accept.**\n\n“Resumable” must include correction after human review, not only restart after a crash. The workflow needs:\n\n- immutable step outputs;\n- idempotent step re-entry;\n- evidence-package versioning;\n- dependency-aware invalidation;\n- partial revalidation when only a subset changes;\n- a clear path from rejected claim to source correction, re-extraction, recalculation, redrafting, and reapproval.\n\nSQLite plus explicit state and attempt tables is sufficient for Phase 0.5. A durable workflow platform should be adopted only after observed rework/concurrency complexity justifies it.","evidence_id":"REQ-DISP-M-5-ACCEPTANCE","evidence_ref_ids":[],"evidence_type":"ARTIFACT","proof_mode":"CONTENT_HASH","scope":"DISP-M-5 acceptance and delivery scope","status":"UNRESOLVED"},{"approval_ids":["APR-DISP-M-5-02"],"description":"Typed ANALYST_ACCEPTANCE proof for M-5 analyst acceptance","evidence_id":"REQ-DISP-M-5-ANALYST_ACCEPTANCE-02","evidence_ref_ids":[],"evidence_type":"ANALYST","proof_mode":"TYPED_APPROVAL","scope":"M-5 analyst acceptance","status":"UNRESOLVED"},{"approval_ids":[],"description":"Reproducible command result proving the current DISP-M-5 acceptance obligation","evidence_id":"REQ-DISP-M-5-COMMAND-PROOF","evidence_ref_ids":[],"evidence_type":"COMMAND_RESULT","proof_mode":"COMMAND","scope":"DISP-M-5 command proof","status":"UNRESOLVED"}],"verification_command":{"commands":[],"mode":"UNRESOLVED","not_applicable_review":null}}
```
...
- `reviewed_inventory_sha256` (pre-record): `aae4339e6b3d8c5681e3ddf156faf72d0ac21f00fea51743b2adb8d9af80e460`
...
`required_evidence` enumerates three items — the largest inventory in this batch:

| `evidence_id` | `evidence_type` | `proof_mode` | `status` | refs | `approval_ids` |
|---|---|---|---|---|---|
| `REQ-DISP-M-5-ACCEPTANCE` | `ARTIFACT` | `CONTENT_HASH` | `UNRESOLVED` | `[]` | `[]` |
| `REQ-DISP-M-5-ANALYST_ACCEPTANCE-02` | `ANALYST` | `TYPED_APPROVAL` | `UNRESOLVED` | `[]` | `["APR-DISP-M-5-02"]` |
| `REQ-DISP-M-5-COMMAND-PROOF` | `COMMAND_RESULT` | `COMMAND` | `UNRESOLVED` | `[]` | `[]` |
...
**Clause-by-clause coverage.** The clause's obligations fall into three proof modes
and all three are present:

- Design content — immutable step outputs; idempotent step re-entry;
  evidence-package versioning; dependency-aware invalidation; and the judgement
  that "SQLite plus explicit state and attempt tables is sufficient for Phase 0.5"
  — is provable by hashing the approved S14 specification bytes
  (`ARTIFACT`/`CONTENT_HASH`).
- "**reapproval**", the terminus of the clause's rejected-claim path, is a human
  approval obligation, carried as `ANALYST`/`TYPED_APPROVAL` naming the
  component-local requirement `APR-DISP-M-5-02` (`ANALYST_ACCEPTANCE` /
  `Responsible analyst`) — the shape goal L~487-491 prescribes.
- "partial revalidation when only a subset changes" and the end-to-end rework path
  are demonstrable, and the register control `B-14` says so explicitly ("partial
  revalidation and reapproval **succeed**"). That is carried by
  `REQ-DISP-M-5-COMMAND-PROOF`, typed `COMMAND_RESULT`/`COMMAND`.

This is one of the eight disposition components the program-level
evidence-inventory review named as having unclassified mechanical obligations; that
finding is remediated on this row and re-verified this round.

**`evidence_refs`.** Two objects, both recomputed and current:
`EV-DISP-M-5-SOURCE` (`UTF8_LINE_SPAN`, disposition report L197-210) and
`EV-DISP-M-5-SPEC-DRAFT` (`FILE_BYTES`, S14 draft).

**`verification_command`.** `UNRESOLVED` while a `COMMAND`-classified requirement
exists. Permitted now (goal L~504-506; structural validation exits `0`), but a
forward obligation: the mode must become `COMMANDS` with a command binding this
requirement before terminal use. Recorded as an obligation, not a finding.
...
**What is missing.** This row's `required_approvals` contains `APR-DISP-M-5-01`,
`DELEGATED_ARTIFACT_APPROVAL`, `Delegated fresh Sol xhigh specification reviewer`,
scope `M-5 under S14`. Under goal L598-600 a record satisfying that requirement
...
`REVIEWER`-role review**". The persisted review of S14's current
...
`EV-DISP-M-5-SPEC-DRAFT` is a `FILE_BYTES` reference to `docs/specs/equity-os-s14-earnings-review-workflow-rework.md` — the exact
...
2. **The reviewed generator makes the pairing unconditional.**
   ...
   emits `REQ-DISP-M-5-SPEC-REVIEW` for this component. The canonical ledger contains
   no such item.
...
4. **The proof cannot be borrowed from a sibling row.** Goal L439 requires review
   evidence to be "current and **component-local**". S14 owns register rows
   B-01, B-02, B-14, each of which carries its own `REQ-REG-*-SPEC-REVIEW`; none of them
...
`REQ-DISP-M-5-SPEC-REVIEW`, `evidence_type` `REVIEW`, `proof_mode` `CONTENT_HASH`,
`status` `UNRESOLVED`, `evidence_ref_ids` `[]`, `approval_ids` `[]`, `scope`
`M-5 under S14`, description "Persisted clean fresh Sol xhigh review of the
...
recording design r2 §5.4 the `EVIDENCE` review on `DISP-M-5` therefore stays
`PENDING`, this artifact is the durable record of the finding, and the finding
belongs in `open_findings` on `DISP-M-5` with severity, load-bearing status, artifact
...
(recording design r2 §3.4), no review on `DISP-M-5` is recordable while this finding
stands.
