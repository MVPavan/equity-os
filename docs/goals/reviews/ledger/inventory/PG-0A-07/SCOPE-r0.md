# Inventory review — PG-0A-07 / SCOPE / r0
...
```json
{"activation_predicate":null,"disposition_refs":[],"gate_refs":[],"related_register_ids":["A-12"],"scope_derivation":{"authority_effect":null,"derived_program_disposition":"REQUIRED_NOW","related_register_ids":["A-12"],"rule":"RELATED_REGISTER_SCOPE"}}
```
...
- `reviewed_inventory_sha256` (pre-record): `ad68cdecfa80cd0bb7c9e1b6fcda746aa9a9fb6a1fae58b26a3b27efea4317c2`
...
**`related_register_ids` = `["A-12"]`, checked against a real near-miss.** A-12
(register L42) is "Define operating calendar, standing budget, and capacity",
with acceptance "Weekly builder/analyst capacity, target phase dates, monthly
provider/model/infrastructure ceilings, maintenance allowance, and expected
company coverage documented". Both nouns in the clause — "operating capacity" and
"standing budget" — and its verb, "documented", appear verbatim in A-12's decision
and acceptance columns. One register covers the whole clause.

**Contrary candidate, considered and rejected — this is the one place this row is
easy to get wrong.** `A-07` is "Define initial per-workflow budgets", acceptance
"Ceilings or measurement rules for model cost, tool calls, latency, document
volume, retries, and analyst minutes". It shares the word "budget", and a reader
skimming for that word would add it. It is the wrong granularity: A-07 is a
*per-workflow* ceiling, A-12 is the *standing* budget and operating capacity, and
the clause says "standing budget". A-07's gate home is `PG-1-10` ("cost, latency,
failures, and retries are visible"), which is precisely the per-workflow
telemetry A-07 defines. Adding A-07 here would be inference from a shared word,
which goal L233-235 forbids; and it would also make `REG-A-07.gate_refs` contain
two gates whose subjects differ.

**Back-reference verified.** `REG-A-12.gate_refs == ["PG-0A-07"]` and
`REG-A-07.gate_refs == ["PG-1-10"]` — the two budget-adjacent registers are
cleanly partitioned across two gates, with no overlap. The `gate_map` equality
holds on both.

**Aggregation → `REQUIRED_NOW`.** `REG-A-12` is `REQUIRED_NOW`
...
related registers rather than duplicating it: here `A-12` → `M-8`, `T-1`, `T-2`. That inheritance
...
clause. T-1 (disposition report L268,
"Operating budget and calendar disappeared") is the finding that restored this
register decision, and it is carried on A-12 rather than on the gate.
