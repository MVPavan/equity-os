# Inventory review — PG-0A-05 / SCOPE / r0
...
```json
{"activation_predicate":null,"disposition_refs":[],"gate_refs":[],"related_register_ids":["A-10","A-13"],"scope_derivation":{"authority_effect":null,"derived_program_disposition":"REQUIRED_NOW","related_register_ids":["A-10","A-13"],"rule":"RELATED_REGISTER_SCOPE"}}
```
...
- `reviewed_inventory_sha256` (pre-record): `62bf33ee1e66920e8b2df2b8739baf1d14dc08ce9046ed367c319652b5d93c3a`
...
**`related_register_ids` = `["A-10", "A-13"]`, one register per conjunct.** This
clause is a two-conjunct gate — "materiality … contracts" and "success-metric
contracts" — and the derivation resolves each conjunct to exactly one register
decision:

- "materiality" → `A-10` (register L40), "Define claim materiality policy",
  acceptance "**Versioned** policy combining quantitative magnitude, always-material
  categories, thesis relevance, …".
- "success-metric" → `A-13` (register L43), "Freeze success-metric contract",
  acceptance "**Versioned** definitions and measurement methods for factual
  accuracy, citation correctness, …".

The clause's operative verb, "are versioned", is the word both acceptance columns
use. Neither register is inferred; both are the unique register decision naming
its conjunct's subject.

**Contrary candidate, considered and rejected.** `A-07` ("Define initial
per-workflow budgets") is the nearest miss because it is a Phase 0A contract-like
artifact, but it concerns budgets, not materiality or success metrics, and its
gate home is `PG-1-10` (register L159, "cost, latency, failures, and retries are
visible") paired with `A-13`. Including it would be inference.

**Back-reference verified.** `REG-A-10.gate_refs == ["PG-0A-05", "PG-1-01",
"PG-1-02"]` and `REG-A-13.gate_refs == ["PG-0A-05", "PG-1-10"]` — both contain
this component, and the `gate_map` equality holds on both. A register claimed by
several gates at different phases is expected and is not double-booking: `A-10`
is gated once here for *versioning the policy* and twice in Phase 1 for
*applying* it.

**Aggregation → `REQUIRED_NOW`.** Both `REG-A-10` and `REG-A-13` are
`REQUIRED_NOW` (`source_status=Open`), so the "any related row is `REQUIRED_NOW`"
branch of goal L248-250 fires on the first element and the aggregate is
`REQUIRED_NOW`. Stored values agree.
...
related registers rather than duplicating it: here `A-10` → `G-1`, `G-5`, `R-4`, `6.2`; `A-13` → `M-8`, `T-1`, `T-2`. That inheritance
...
clause.
For this component that inheritance is unusually direct: T-2's closing sentence
"All phase gates should reference this contract" (disposition report L291) names
the success-metric contract, and this is one of the two gates that reference it.
