# Inventory review — DISP-M-6 / EVIDENCE / r0
...
```json
{"evidence_refs":[{"captured_at":"2026-08-13T02:49:11Z","content_sha256":"0c0e2cc1bb9541af1001e4f18f65d15f5a564e3dd7197af113b334628a58ff4b","digest_mode":"UTF8_LINE_SPAN","end_line":224,"evidence_ref_id":"EV-DISP-M-6-SOURCE","path":"docs/blueprint/funda-third-order-review-disposition-report.md","scope":"Exact authoritative source occurrence for DISP-M-6","start_line":212},{"captured_at":"2026-08-15T07:13:28Z","content_sha256":"5da8bf5f31e29833885d7e0bf74ecfc2a550fb9c24415ef5123fa47c938e2957","digest_mode":"FILE_BYTES","end_line":null,"evidence_ref_id":"EV-DISP-M-6-SPEC-DRAFT","path":"docs/specs/equity-os-s07-golden-set-failure-reviewer-controls.md","scope":"Current draft specification bytes for DISP-M-6","start_line":null}],"required_evidence":[{"approval_ids":[],"description":"Current proof satisfying: ### M-6 — Reviewer and builder are the same person\n\n**Disposition: Accept with safeguards.**\n\n“Accepted unchanged” is not a standalone quality metric because careless review can maximize it. Add:\n\n- edit/reject accuracy on known golden cases;\n- false-accept and false-reject categories, stratified by materiality and epistemic class;\n- periodic seeded-error drills in a **shadow copy or test-mode report only**;\n- seeded errors that cover wrong period, unit, source, unsupported claim, and fabricated citation;\n- occasional external spot review where practical.\n\nNever inject a known falsehood into the artifact that can be promoted or published.","evidence_id":"REQ-DISP-M-6-ACCEPTANCE","evidence_ref_ids":[],"evidence_type":"ARTIFACT","proof_mode":"CONTENT_HASH","scope":"DISP-M-6 acceptance and delivery scope","status":"UNRESOLVED"},{"approval_ids":[],"description":"Reproducible command result proving the current DISP-M-6 acceptance obligation","evidence_id":"REQ-DISP-M-6-COMMAND-PROOF","evidence_ref_ids":[],"evidence_type":"COMMAND_RESULT","proof_mode":"COMMAND","scope":"DISP-M-6 command proof","status":"UNRESOLVED"}],"verification_command":{"commands":[],"mode":"UNRESOLVED","not_applicable_review":null}}
```
...
- `reviewed_inventory_sha256` (pre-record): `725f0e11fc0f95c1c39cffa45e62394377d47560de3fd7e2fa6c11c80a0372e9`
...
|---|---|---|---|---|---|
| `REQ-DISP-M-6-ACCEPTANCE` | `ARTIFACT` | `CONTENT_HASH` | `UNRESOLVED` | `[]` | `[]` |
| `REQ-DISP-M-6-COMMAND-PROOF` | `COMMAND_RESULT` | `COMMAND` | `UNRESOLVED` | `[]` | `[]` |
...
**Clause-by-clause coverage.** The clause opens with a measurement judgement
("'Accepted unchanged' is not a standalone quality metric because careless review
can maximize it") and then adds five safeguards plus one prohibition:

- Edit/reject accuracy on known golden cases, false-accept and false-reject
  categories stratified by materiality and epistemic class, and periodic
  seeded-error drills are **measurable outcomes over a golden set**, and are
  carried by `REQ-DISP-M-6-COMMAND-PROOF` (`COMMAND_RESULT`/`COMMAND`, scope
  "DISP-M-6 command proof").
- The confinement of drills to "a **shadow copy or test-mode report only**", the
  seeded-error coverage list (wrong period, unit, source, unsupported claim,
  fabricated citation), and the closing prohibition — "Never inject a known
  falsehood into the artifact that can be promoted or published" — are control
  design, carried by the `CONTENT_HASH` acceptance item.
- "occasional external spot review where practical" is explicitly conditional, and
  its register control `B-13` records it as an "**optional** external spot review
  procedure defined". A conditional, optional practice creates no unconditional
  proof obligation, so its absence from `required_evidence` as a separate item is
  correct; the requirement that the procedure be *defined* is design content
  already inside the acceptance item.
...
`EV-DISP-M-6-SOURCE` (`UTF8_LINE_SPAN`, disposition report L212-224) and
`EV-DISP-M-6-SPEC-DRAFT` (`FILE_BYTES`, S07 draft).

**`verification_command`.** `UNRESOLVED` while a `COMMAND`-classified requirement
exists. Permitted now (goal L~504-506; structural validation exits `0`); the mode
must become `COMMANDS` before terminal use. Recorded as a forward obligation, not a
finding.
...
**What is missing.** This row's `required_approvals` contains `APR-DISP-M-6-01`,
`DELEGATED_ARTIFACT_APPROVAL`, `Delegated fresh Sol xhigh specification reviewer`,
scope `M-6 under S07`. Under goal L598-600 a record satisfying that requirement
...
`REVIEWER`-role review**". The persisted review of S07's current
...
`EV-DISP-M-6-SPEC-DRAFT` is a `FILE_BYTES` reference to `docs/specs/equity-os-s07-golden-set-failure-reviewer-controls.md` — the exact
...
2. **The reviewed generator makes the pairing unconditional.**
   ...
   emits `REQ-DISP-M-6-SPEC-REVIEW` for this component. The canonical ledger contains
   no such item.
...
4. **The proof cannot be borrowed from a sibling row.** Goal L439 requires review
   evidence to be "current and **component-local**". S07 owns register rows
   A-08, B-08, B-13, each of which carries its own `REQ-REG-*-SPEC-REVIEW`; none of them
...
`REQ-DISP-M-6-SPEC-REVIEW`, `evidence_type` `REVIEW`, `proof_mode` `CONTENT_HASH`,
`status` `UNRESOLVED`, `evidence_ref_ids` `[]`, `approval_ids` `[]`, `scope`
`M-6 under S07`, description "Persisted clean fresh Sol xhigh review of the
...
recording design r2 §5.4 the `EVIDENCE` review on `DISP-M-6` therefore stays
`PENDING`, this artifact is the durable record of the finding, and the finding
belongs in `open_findings` on `DISP-M-6` with severity, load-bearing status, artifact
...
(recording design r2 §3.4), no review on `DISP-M-6` is recordable while this finding
stands.
