# Inventory review — DEF-12 / SCOPE / r0
...
- `text_digest` recomputed over the normalized L186-186 span
  ...
  `a7595dd8e141fbe63dfda149d579a5915d8c7a2552fb98cacb57be49363d7503`,
  equal to the stored `text_digest`.
- `required_acceptance_text` is `"local-model optimization without a Funda-specific benchmark"`, which is
  that span with the list marker and terminal punctuation removed — the exact
  clause content.

## Reasoning

**Kind.** Bullet 12 of register §G (L186), excluding local-model optimization
**without a Funda-specific benchmark**. `first_release_deferral` by the §G rule.
Like `DEF-07` and `DEF-13`, the clause is conditional in form, so I checked the
conditional reading: the exclusion lapses only once a Funda-specific benchmark
exists, and building that benchmark is `REG-D-02`'s obligation ("Run
current-scale three-arm memory benchmark"), not this row's. The row states the
standing default, which is active now.
...
**Related register IDs.** `[]`, contract-forced. `REG-D-02` (`Deferred`) is the
benchmark decision whose completion would dissolve the "without a benchmark"
condition; `REG-E-06` ("Evaluate OpenBB deployment", `Deferred`) is the nearest
out-of-process-tooling row. Neither may be named here, and neither needs to be:
both are `CONDITIONAL_UNACTIVATED` on their own derivations.

**Two `DEF-12`-only facts, recorded because a reader comparing the thirteen rows
will notice them, and neither changes this review.** First, `DEF-12` is the only
§G row whose `primary_spec` is `null`, and that null is pinned by the goal-derived
validator itself (`validate_ledger_structural.py:2654`,
`assert by_id["DEF-12"]["primary_spec"] is None`). Goal L184 permits `null` only
when "`scope_derivation` explicitly supplies program-wide or related-register
ownership; null never means inactive" — satisfied exactly, since
`PROGRAM_WIDE_ACTIVE_CONTROL` is that explicit program-wide supply. Second,
`DEF-12` is the only row in the entire ledger carrying an authorized delivery
reset (`authorized_delivery_reset = ("DEF-12", "SPEC_DRAFT", "INVENTORIED")` at
`:1819`), which is why its `delivery_status` is `INVENTORIED` while the other
twelve are `SPEC_DRAFT`. Neither `primary_spec` nor `delivery_status` is in the
`SCOPE` inventory projection, so neither is within this review's decided
question; both are verified consistent with the contract and recorded here.

**Source anchor and span.** `source_anchor` is `DEF-12`, a clause ordinal.
...
**Applicable review slot.** `DEF-12` is a non-register canonical row, so
...
only that `DEF-12`'s scope derivation is correct at the input bytes pinned above.
