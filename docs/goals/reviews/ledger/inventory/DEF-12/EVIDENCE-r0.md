# Inventory review — DEF-12 / EVIDENCE / r0
...
```json
{"evidence_refs":[{"captured_at":"2026-08-13T02:49:11Z","content_sha256":"a7595dd8e141fbe63dfda149d579a5915d8c7a2552fb98cacb57be49363d7503","digest_mode":"UTF8_LINE_SPAN","end_line":186,"evidence_ref_id":"EV-DEF-12-SOURCE","path":"docs/blueprint/funda-blueprint-implementation-decision-register-v2.md","scope":"Exact authoritative source occurrence for DEF-12","start_line":186},{"captured_at":"2026-08-15T07:13:28Z","content_sha256":"4948d0f84240cd58b107da1272ed2b90a386bc3fb9ebb6228faac8b98be9c483","digest_mode":"FILE_BYTES","end_line":null,"evidence_ref_id":"EV-DEF-12-SPEC-DRAFT","path":"docs/specs/equity-os-s20-memory-benchmark-gbrain.md","scope":"Current draft specification bytes for DEF-12","start_line":null}],"required_evidence":[{"approval_ids":[],"description":"Current proof satisfying: local-model optimization without a Funda-specific benchmark","evidence_id":"REQ-DEF-12-ACCEPTANCE","evidence_ref_ids":[],"evidence_type":"ARTIFACT","proof_mode":"CONTENT_HASH","scope":"DEF-12 acceptance and delivery scope","status":"UNRESOLVED"},{"approval_ids":[],"description":"Persisted clean fresh Sol xhigh review of the current specification bytes","evidence_id":"REQ-DEF-12-SPEC-REVIEW","evidence_ref_ids":[],"evidence_type":"REVIEW","proof_mode":"CONTENT_HASH","scope":"DEF-12 under S20","status":"UNRESOLVED"},{"approval_ids":[],"description":"Current negative proof that the deferred scope has no implementation in the current bytes","evidence_id":"REQ-DEF-12-NO-IMPLEMENTATION","evidence_ref_ids":[],"evidence_type":"ARTIFACT","proof_mode":"CONTENT_HASH","scope":"DEF-12 current no-implementation proof","status":"UNRESOLVED"}],"verification_command":{"commands":[],"mode":"UNRESOLVED","not_applicable_review":null}}
```
...
- `reviewed_inventory_sha256` (pre-record): `35895a9d8be149435da28e1782e55b2ae419c13db2d4d23a1f5d97c4179fc0c3`
...
`text_digest` and `EV-DEF-12-SOURCE.content_sha256` both recomputed over the
normalized L186-186 span → `a7595dd8e141fbe63dfda149d579a5915d8c7a2552fb98cacb57be49363d7503`, matching the
stored values. `required_acceptance_text` is `"local-model optimization without a Funda-specific benchmark"`.

## Reasoning

**What the source clause actually demands, and whether it is covered.** The
clause excludes *local-model optimization without a Funda-specific benchmark* —
conditional in form, like `DEF-07`'s. The proof demanded is that current bytes
contain no local-model optimization work predicated on the absent benchmark.
`REQ-DEF-12-NO-IMPLEMENTATION` carries it, and the benchmark whose existence
would dissolve the condition is `REG-D-02`'s own obligation ("Run current-scale
three-arm memory benchmark", `Deferred`), inventoried there with its own
acceptance evidence. No benchmark-construction proof is demanded on this row, and
enumerating one would duplicate `D-02`.

**The `primary_spec: null` seam, examined for a missing binding.** This is the
only §G row with no `primary_spec` — a null the goal-derived validator pins
directly (`validate_ledger_structural.py:2654`). Yet its requirement `scope`
strings read "DEF-12 under S20" and `EV-DEF-12-SPEC-DRAFT` points at
`docs/specs/equity-os-s20-memory-benchmark-gbrain.md`. So the spec binding is
carried by the requirement scope strings and the evidence reference rather than
by `primary_spec`. I checked whether that leaves an obligation unbound and it
does not: `REQ-DEF-12-SPEC-REVIEW`'s scope names S20 explicitly, so the review
obligation has a named artifact; goal L184 permits the null precisely when
`scope_derivation` supplies program-wide ownership, which
`PROGRAM_WIDE_ACTIVE_CONTROL` does; and goal L185 adds that "`primary_spec` never
determines whether a component is active." Recorded as a structural
observation, not a completeness gap.
...
|---|---|---|---|
| `REQ-DEF-12-ACCEPTANCE` | `ARTIFACT` | `CONTENT_HASH` | the clause's own acceptance scope |
| `REQ-DEF-12-SPEC-REVIEW` | `REVIEW` | `CONTENT_HASH` | a persisted clean review of the current specification bytes |
| `REQ-DEF-12-NO-IMPLEMENTATION` | `ARTIFACT` | `CONTENT_HASH` | current negative proof that the deferred scope has no implementation in the current bytes |
...
whose `program_disposition` is `REJECTED_ACCOUNTED` (`:2734-2736`). `DEF-12` is
...
`REQ-DEF-12-NO-IMPLEMENTATION`. Its future satisfaction is governed by the
...
- *`TYPED_APPROVAL`.* Goal L484-487 requires a `TYPED_APPROVAL` item to name
  ...
  requirement is `APR-DEF-12-01`, of type `DELEGATED_ARTIFACT_APPROVAL`. Swept all
...
- *`REVIEW` — present, and checked against the known defect population.* Of the
  ...
  ("All 32 disposition components omit required review evidence"). `DEF-12` is in
...
Two references: `EV-DEF-12-SOURCE` (UTF8_LINE_SPAN over `docs/blueprint/funda-blueprint-implementation-decision-register-v2.md`, L186-186, captured `2026-08-13T02:49:11Z`); `EV-DEF-12-SPEC-DRAFT` (FILE_BYTES over `docs/specs/equity-os-s20-memory-benchmark-gbrain.md`, captured `2026-08-15T07:13:28Z`). Every one resolved and re-digested during the structural validation
...
1. **The `ACCEPTANCE` item's description is still positively framed.**
   `REQ-DEF-12-ACCEPTANCE.description` reads "Current proof satisfying:
   local-model optimization without a Funda-specific benchmark" — a deferral clause demands no proof that its
...
2. **A vendor model lane survives in `REQ-DEF-12-SPEC-REVIEW.description`**
...
`required_evidence` for `DEF-12` is complete at the input bytes pinned above: the
