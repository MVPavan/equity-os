# Inventory review — PG-1-02 / APPROVAL / r0
...
**"Correct" is not an approval word, and that is the question worth asking on
this row.** The clause requires factual claims to "resolve to the **correct**
source location". Correctness invites the thought that someone must adjudicate
it, and claim-level review is indeed a real activity in this programme (register
C-05, "Build claim-level review UI/workflow"). But correctness of a citation is a
verifiable fact about where the cited passage is, not a discretionary
determination; a reviewer who checks it is reading, not authorising. Goal
L487-489's list of evidence classes that must route through the typed approval
path — analyst, domain, provider, rights, legal, regulatory, budget, capacity,
owner, production, distribution, security, external — is a list of *competences*,
and citation correctness needs none of them to be exercised as an authority.
Consistently, `REG-C-05` is claimed by no gate in §F at all.

**Why the related register's authority is not also owed here — the decisive
point, and it is mechanical, not stylistic.** `REG-A-10` carries `APR-REG-A-10-02` (`DOMAIN_EXPERT_ACCEPTANCE`, "Equity-research domain expert") for acceptance of the materiality policy this clause cites; `REG-C-04` carries only the delegated specification approval. A reader could
...
and both have empty approval inventories, so the natural worry is that this row
was derived by copying its neighbour. The affirmation here rests on a different
clause: this one turns on whether *citation correctness* is a determination
(answered above), while `PG-1-01` turns on whether *trace resolution* is. Both
answers are no, but they are separately reasoned, and the two rows'
...
`PG-1-02` regardless of kind, and it is the slot this review is written against.
...
**Corroboration, treated as corroboration only.** `PG-1-02` is absent from
