# Inventory review — PG-05-09 / SCOPE / r0
...
```json
{"activation_predicate":null,"disposition_refs":[],"gate_refs":[],"related_register_ids":["B-09"],"scope_derivation":{"authority_effect":null,"derived_program_disposition":"REQUIRED_NOW","related_register_ids":["B-09"],"rule":"RELATED_REGISTER_SCOPE"}}
```
...
- `reviewed_inventory_sha256` (pre-record): `ff649a6feb07b3061b0cfcd643e742bb076dea8edc4dd57309982957407521f1`
...
**Related register IDs — `["B-09"]`, and only B-09.** The clause is four words of
subject and one of state: "point-in-time capture has started". B-09 (register L59) is
"**Start point-in-time capture**" — the same verb and the same noun phrase, the closest
title-to-clause match in this batch. Its acceptance is "Daily/event jobs persist approved
membership/security changes, prices, announcements, corporate actions, shareholding
changes, hashes, first-seen times, and capture failures".

A-05 is the candidate I checked and rejected: its acceptance includes "point-in-time
availability" per source, and B-09's declared dependency is A-05. But A-05's term is about
whether a *provider* offers point-in-time data — a rights-register attribute — while this
clause is about the capture *jobs* running. A-05 is gated at `PG-0A-02`, also in this
batch, which keeps the two cleanly separated.

Backlink: `REG-B-09.gate_refs == ["PG-05-09"]`, a singleton — one-to-one in both
directions.

**Aggregated disposition.** `REG-B-09.program_disposition == "REQUIRED_NOW"`, so the
...
**Prior human-review lineage checked, and correctly not inherited.** `REG-B-09` is
listed under `HR-0002` in `EXPECTED_PRIOR_HR_LINKS`
(`validate_ledger_structural.py:2781`). This gate's `human_review_id` is `null` and it has
zero occurrences in the canonical human-review artifact. That is right: a human-review
link attaches to the component whose state the resolution changed, and the resolution
under `HR-0002` concerned the register row, not this clause. A mis-propagated link would
have shown up here as a scope-adjacent anomaly, so I checked it explicitly.
