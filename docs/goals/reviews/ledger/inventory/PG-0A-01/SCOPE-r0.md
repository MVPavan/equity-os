# Inventory review — PG-0A-01 / SCOPE / r0
...
```json
{"activation_predicate":null,"disposition_refs":[],"gate_refs":[],"related_register_ids":["A-01"],"scope_derivation":{"authority_effect":null,"derived_program_disposition":"REQUIRED_NOW","related_register_ids":["A-01"],"rule":"RELATED_REGISTER_SCOPE"}}
```
...
- `reviewed_inventory_sha256` (pre-record): `ef4fd804726c7bda295826ee8f7d7a8465cd5ae81c6bf1f81a8b02d51bfa8e56`
...
**Related register IDs — `["A-01"]`, and only A-01.** "the initial product boundary"
is A-01's subject: register L31, "Freeze initial user and distribution boundary", with
acceptance "Written statement covering private/internal use, public or paid distribution,
personalization, execution linkage, and intended future boundary; document does not claim
legal sufficiency". "Initial … boundary" is A-01's own phrase, and "documented" is its
"Written statement".

A-12 is the near-miss, since "operating capacity and standing budget are documented" is
the parallel Phase 0A clause and A-01 is one of A-12's dependencies. But that clause is
`PG-0A-07`, with `related_register_ids == ["A-12"]` — checked directly. The two clauses
partition their subject matter cleanly and neither borrows the other's register.

Backlink: `REG-A-01.gate_refs == ["PG-0A-01"]`, a singleton — one-to-one in both
directions.

**Aggregated disposition.** `REG-A-01.program_disposition == "REQUIRED_NOW"`
(activation `Open`, current `Open`), so the one-element aggregation yields `REQUIRED_NOW`,
matching derivation and stored value.
...
**Phase 0A anchoring checked.** `blueprint_phase == "0A"`, `source_anchor ==
"F-0A-01"`, and the occurrence is the first bullet under `### Phase 0A may exit only when`
at register L124 — a different heading and a different anchor prefix from the ten Phase
0.5 rows in this batch, and all three agree. Note the clause's subject is also what
`PG-0A-02` refers back to as "that boundary", making this the anchor row for the Phase 0A
boundary pair; a mis-anchored occurrence here would silently move that referent.
