# Inventory review — DEF-09 / EVIDENCE / r0
...
```json
{"evidence_refs":[{"captured_at":"2026-08-13T02:49:11Z","content_sha256":"07c843b73d2139dbcaffbbca91a6e26460e7251f2adb0eeed4a236e20b63224a","digest_mode":"UTF8_LINE_SPAN","end_line":183,"evidence_ref_id":"EV-DEF-09-SOURCE","path":"docs/blueprint/funda-blueprint-implementation-decision-register-v2.md","scope":"Exact authoritative source occurrence for DEF-09","start_line":183},{"captured_at":"2026-08-15T07:13:28Z","content_sha256":"3b66cb90a76ab8f62eef203de2beabff5171c556146071974cc48e926374bbd2","digest_mode":"FILE_BYTES","end_line":null,"evidence_ref_id":"EV-DEF-09-SPEC-DRAFT","path":"docs/specs/equity-os-s25-quant-validation-historical-leakage.md","scope":"Current draft specification bytes for DEF-09","start_line":null}],"required_evidence":[{"approval_ids":[],"description":"Current proof satisfying: historical LLM alpha claims","evidence_id":"REQ-DEF-09-ACCEPTANCE","evidence_ref_ids":[],"evidence_type":"ARTIFACT","proof_mode":"CONTENT_HASH","scope":"DEF-09 acceptance and delivery scope","status":"UNRESOLVED"},{"approval_ids":[],"description":"Persisted clean fresh Sol xhigh review of the current specification bytes","evidence_id":"REQ-DEF-09-SPEC-REVIEW","evidence_ref_ids":[],"evidence_type":"REVIEW","proof_mode":"CONTENT_HASH","scope":"DEF-09 under S25","status":"UNRESOLVED"},{"approval_ids":[],"description":"Current negative proof that the deferred scope has no implementation in the current bytes","evidence_id":"REQ-DEF-09-NO-IMPLEMENTATION","evidence_ref_ids":[],"evidence_type":"ARTIFACT","proof_mode":"CONTENT_HASH","scope":"DEF-09 current no-implementation proof","status":"UNRESOLVED"}],"verification_command":{"commands":[],"mode":"UNRESOLVED","not_applicable_review":null}}
```
...
- `reviewed_input_sha256` (pre-record): `59570beef087c29336b2b1ab7e9cbc42d7e12211b1700e681c432f2f26c94d0b`
- `reviewed_inventory_sha256` (pre-record): `131421c52a6faf65e085e95db99cc9f9deac1ff9dc6b27ed46c59dc17c343b0f`
...
183, inside `## G. Explicitly deferred from the first release` (L173):

> - historical LLM alpha claims;

`text_digest` and `EV-DEF-09-SOURCE.content_sha256` both recomputed over the
normalized L183-183 span → `07c843b73d2139dbcaffbbca91a6e26460e7251f2adb0eeed4a236e20b63224a`, matching the
stored values. `required_acceptance_text` is `"historical LLM alpha claims"`.

## Reasoning

**What the source clause actually demands, and whether it is covered.** The
clause excludes *historical LLM alpha claims*. The proof demanded is a claim-level
absence: that current bytes make no representation of historical LLM performance
as alpha evidence. `REQ-DEF-09-NO-IMPLEMENTATION` carries it, and
`REQ-DEF-09-SPEC-REVIEW` binds S25 ("quant validation and historical leakage"),
which is the spec that would have to make such a claim if anything did.

**The `REG-E-10` overlap, checked for a swallowed obligation.** `REG-E-10`
("Publish historical-replay leakage policy", `Deferred`) ends its acceptance text
with "historical LLM results are not represented as clean alpha evidence" — near
enough to this clause that a missing obligation could plausibly hide in the seam.
It does not. `E-10`'s obligation is to *publish a tested leakage policy* and it
carries its own acceptance evidence for that; this row's obligation is that no
alpha claim exists now. The two are different proofs of different facts, each
enumerated on its own row. The one thing that would be a real gap — an
unmentioned obligation to disclose model-weight leakage as an uncontrollable
limitation — is likewise `E-10`'s acceptance text, not this clause's.

**Distinct from `DEF-08`.** Both bind S25 through their `SPEC-REVIEW` items.
`DEF-08` excludes constructing portfolios; this row excludes claiming historical
alpha. Two obligations, two rows, two negative proofs.
...
|---|---|---|---|
| `REQ-DEF-09-ACCEPTANCE` | `ARTIFACT` | `CONTENT_HASH` | the clause's own acceptance scope |
| `REQ-DEF-09-SPEC-REVIEW` | `REVIEW` | `CONTENT_HASH` | a persisted clean review of the current specification bytes |
| `REQ-DEF-09-NO-IMPLEMENTATION` | `ARTIFACT` | `CONTENT_HASH` | current negative proof that the deferred scope has no implementation in the current bytes |
...
whose `program_disposition` is `REJECTED_ACCOUNTED` (`:2734-2736`). `DEF-09` is
...
`REQ-DEF-09-NO-IMPLEMENTATION`. Its future satisfaction is governed by the
...
- *`TYPED_APPROVAL`.* Goal L484-487 requires a `TYPED_APPROVAL` item to name
  ...
  requirement is `APR-DEF-09-01`, of type `DELEGATED_ARTIFACT_APPROVAL`. Swept all
...
- *`REVIEW` — present, and checked against the known defect population.* Of the
  ...
  ("All 32 disposition components omit required review evidence"). `DEF-09` is in
...
Two references: `EV-DEF-09-SOURCE` (UTF8_LINE_SPAN over `docs/blueprint/funda-blueprint-implementation-decision-register-v2.md`, L183-183, captured `2026-08-13T02:49:11Z`); `EV-DEF-09-SPEC-DRAFT` (FILE_BYTES over `docs/specs/equity-os-s25-quant-validation-historical-leakage.md`, captured `2026-08-15T07:13:28Z`). Every one resolved and re-digested during the structural validation
...
1. **The `ACCEPTANCE` item's description is still positively framed.**
   `REQ-DEF-09-ACCEPTANCE.description` reads "Current proof satisfying:
   historical LLM alpha claims" — a deferral clause demands no proof that its
...
2. **A vendor model lane survives in `REQ-DEF-09-SPEC-REVIEW.description`**
...
`required_evidence` for `DEF-09` is complete at the input bytes pinned above: the
