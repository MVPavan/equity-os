# Inventory review — DEF-09 / SCOPE / r0
...
- `text_digest` recomputed over the normalized L183-183 span
  ...
  `07c843b73d2139dbcaffbbca91a6e26460e7251f2adb0eeed4a236e20b63224a`,
  equal to the stored `text_digest`.
- `required_acceptance_text` is `"historical LLM alpha claims"`, which is
  that span with the list marker and terminal punctuation removed — the exact
  clause content.

## Reasoning

**Kind.** Bullet 9 of register §G (L183), excluding historical LLM alpha claims.
This bullet has the closest textual echo of any row in the batch to a register
decision, so the kind question here is really an alias question, and I treated it
as the load-bearing one — see below.
...
**Related register IDs.** `[]`, contract-forced.

**The alias reading, tested and rejected.** `REG-E-10` ("Publish
historical-replay leakage policy", Status `Deferred`) ends its acceptance text
with "historical LLM results are not represented as clean alpha evidence" —
nearly this bullet's content. If `DEF-09` were a non-canonical restatement it
would be a `derivative_alias`, with `scope_derivation=null`, `primary_spec=null`,
and all three review slots `null` (goal L148, L280, L495-496, L623-624). It is
not one, for three independent reasons: (1) goal L148 defines aliases as repeated
*executive summary, accepted-change recap, or other non-canonical restatement*,
and §G is a canonical section with its own exact-minimum-13 normalization rule at
L142; (2) `E-10` is a deferred decision to **publish a policy** while this bullet
is a first-release exclusion of the **claims themselves** — different objects,
not the same statement twice; (3) `E-10` is a §E table row inventoried under
L140's separate rule, and `E-05` depends on it, so it carries live register
scope this bullet does not.

`E-10` therefore proves the dormancy of the replay-policy decision through its
own `CONDITIONAL_UNACTIVATED` derivation and activate-deferred
`PRODUCT_OWNER_DECISION`; this row proves the exclusion is active now. Both are
required.

**Source anchor and span.** `source_anchor` is `DEF-09`, a clause ordinal.
...
**Applicable review slot.** `DEF-09` is a non-register canonical row, so
...
only that `DEF-09`'s scope derivation is correct at the input bytes pinned above.
