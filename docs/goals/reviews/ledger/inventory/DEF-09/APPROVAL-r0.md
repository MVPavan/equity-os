# Inventory review — DEF-09 / APPROVAL / r0
...
```json
{"approval_records":[],"human_review_id":["HR-0004"],"required_approvals":[{"actor":null,"approval_id":"APR-DEF-09-01","approval_type":"DELEGATED_ARTIFACT_APPROVAL","evidence_ref_ids":[],"matched_record_id":null,"required_authority":"Delegated fresh Sol xhigh specification reviewer","scope":"DEF-09 under S25","status":"UNRESOLVED","timestamp":null}],"security_exception_ids":[]}
```
...
- `reviewed_inventory_sha256` (pre-record): `52e9b84d8495bebbd132ff21c4cc88cd0eb18fffd6b6f579f53faf4b3e90bd2a`
...
`text_digest` recomputed over the normalized L183-183 span →
`07c843b73d2139dbcaffbbca91a6e26460e7251f2adb0eeed4a236e20b63224a`, matching the stored value.
...
**The strongest case for an omitted authority in this batch, and why it fails.**
Historical alpha claims are performance representations. If any §G bullet were
going to need an external sign-off — legal, regulatory, or distribution — it is
this one, and I treated that as the load-bearing question.

The resolution is that the obligation attaches to publication, not to
abstention, and the ledger already places it there. `REG-E-08` ("Gate
paid/public/personalized research on current legal review", `Deferred`, Critical)
carries `LEGAL_REVIEW`, `REGULATORY_REVIEW`, and `DISTRIBUTION_APPROVAL` — the
complete external set — and `REG-E-10` carries the leakage-policy obligation with
its own activate-deferred approval. This clause states that no such claim is made
in the first release. There is no authority in the closed vocabulary whose
sign-off is required for a claim not to be made, and inventing one would require
a reconciled, reviewed, approved vocabulary change under goal L583-584.

**The enumerated obligation.** One requirement, `APR-DEF-09-01`, type
`DELEGATED_ARTIFACT_APPROVAL`, authority "Delegated fresh Sol xhigh
specification reviewer", scope "DEF-09 under S25", `UNRESOLVED`. It is the delegated
...
| `PRODUCT_OWNER_DECISION` (incl. `Product owner authorized to activate deferred blueprint scope`) | `REG-E-10` ('Publish historical-replay leakage policy', `Deferred`) carries the activate-deferred authority and is the row whose acceptance text already forbids representing historical LLM results as clean alpha evidence. |
...
| `DATA_RIGHTS_APPROVAL`, `LEGAL_REVIEW`, `REGULATORY_REVIEW` | No data is acquired. `REGULATORY_REVIEW` deserves a specific word: performance claims are the one subject in §G with an obvious regulatory surface, and the ledger's single `REGULATORY_REVIEW` (`Competent regulatory reviewer`) sits on `REG-E-08`, which gates paid/public/personalized research. A regulatory review authorises *making* claims to an audience under obligation; refusing to make them creates no such surface. |
...
`required_approvals`. `DEF-09` is linked to `HR-0004` only.
...
`required_approvals` for `DEF-09` is complete at the input bytes pinned above: the
source clause demands exactly one typed approval — the delegated artifact
approval already enumerated as `APR-DEF-09-01` — and no authority whose
