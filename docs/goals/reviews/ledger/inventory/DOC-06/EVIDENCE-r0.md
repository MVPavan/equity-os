# Inventory review — DOC-06 / EVIDENCE / r0
  ...
  "evidence_refs": [
    {
      ...
      "content_sha256": "7957d45fe26f2d2958305609e6707ca202e3943a9212ce8cb31df7995806a67d",
      "digest_mode": "UTF8_LINE_SPAN",
      "end_line": 475,
      "evidence_ref_id": "EV-DOC-06-SOURCE",
      "path": "docs/blueprint/funda-third-order-review-disposition-report.md",
      "scope": "Exact authoritative source occurrence for DOC-06",
      "start_line": 475
    }
  ],
  "required_evidence": [
    {
      "approval_ids": [],
      "description": "Current proof satisfying: This avoids review-document recursion while preserving traceability.",
      "evidence_id": "REQ-DOC-06-ACCEPTANCE",
      "evidence_ref_ids": [],
      "evidence_type": "ARTIFACT",
      "proof_mode": "CONTENT_HASH",
      "scope": "DOC-06 acceptance and delivery scope",
      "status": "UNRESOLVED"
    }
  ],
...
### Obligation audit for line 475

The clause is a rationale sentence: the §9 structure "avoids review-document recursion while
preserving traceability". `REQ-DOC-06-ACCEPTANCE` reproduces it verbatim and classifies it
`ARTIFACT` / `CONTENT_HASH`.

The question worth asking on a rationale clause is whether it demands *any* proof at all, and
if so whether one artifact obligation is the right amount. It does state a testable property
of the document set — that no further review document recurses on the earlier ones, and that
traceability from finding to disposition survives — and that property is evidenced by the
state of the documents §9 enumerates. A content-hash artifact obligation over that state is
the appropriate and maximal demand a rationale clause of this shape can make. There is no
second obligation hiding in it: the sentence names no artifact of its own, no actor, and no
condition.

- **`COMMAND`?** No test, replay, or demonstration verb.
- **`TYPED_APPROVAL`?** No authority named, and none of goal L487-490's typed subject matters
  is touched.
- **`REVIEW`?** No spec artifact owned (`primary_spec: null`; `applicable_spec_ids` forbidden
  for this kind by goal L229-232).

### Consistency check against the identically-shaped clause in the same document

`SEQ-11` is §8's closing rationale ("This ordering avoids both circularity and premature
freezing …", line 462) — the same clause shape by the same author in the same report. It
carries exactly one `ARTIFACT`/`CONTENT_HASH` acceptance item and no approvals, and it is the
only one of the 11 sequence clauses to do so, precisely because it names no register IDs and
no specs. `DOC-06` is treated identically. Independent confirmation that one acceptance item
is this ledger's settled treatment of a closing-rationale clause, not an under-enumeration
specific to `DOC-06`.
