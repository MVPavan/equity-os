# Inventory review — PG-2-02 / EVIDENCE / r0
...
```json
{"evidence_refs":[{"captured_at":"2026-08-13T02:49:11Z","content_sha256":"28e6abf5a629ee2d726d3d1707e590e967f5d1f1a763531f8598f30e45ed780e","digest_mode":"UTF8_LINE_SPAN","end_line":165,"evidence_ref_id":"EV-PG-2-02-SOURCE","path":"docs/blueprint/funda-blueprint-implementation-decision-register-v2.md","scope":"Exact authoritative source occurrence for PG-2-02","start_line":165}],"required_evidence":[{"approval_ids":[],"description":"Current proof satisfying: stale and contradicted conclusions are surfaced","evidence_id":"REQ-PG-2-02-ACCEPTANCE","evidence_ref_ids":[],"evidence_type":"ARTIFACT","proof_mode":"CONTENT_HASH","scope":"PG-2-02 acceptance and delivery scope","status":"UNRESOLVED"}],"verification_command":{"commands":[],"mode":"UNRESOLVED","not_applicable_review":null}}
```
...
- `reviewed_inventory_sha256` (pre-record): `0e9d8c29cd3c6d3891a9d05f191e0c672ee89ff7b543e31e4aeae4c68c2c61ce`
...
**What the clause actually demands.** Two failure classes — stale conclusions
and contradicted conclusions — and one behaviour over both: they are surfaced.
As elsewhere in the ledger this is one obligation over a set, carried by
`REQ-PG-2-02-ACCEPTANCE` (`ARTIFACT`/`CONTENT_HASH`, description quoting the
clause verbatim, scope "PG-2-02 acceptance and delivery scope").
...
**Why the absent `COMMAND_RESULT` item deserves an explicit argument here.**
Of the eleven clauses in this batch, this is the one whose predicate is most
obviously a test: `AP-PG-2-02` counts planted fixtures and surfaced detections
and requires exact recall. A reader could reasonably expect a `COMMAND-PROOF`
item alongside it, as `PG-2-03` has.
...
`PG-1-04`, `PG-1-05`, `PG-1-06`, `PG-2-03`, and `PG-2-04`. `PG-2-02` is not among
...
So the contract's answer is unambiguous, but I checked *why* `PG-2-03` is in the
pinned set and `PG-2-02` is not, rather than accepting the set as arbitrary.
`PG-2-03` asserts an impossibility — "canonical promotion **cannot** diverge from
SQL metadata" — which is a safety property whose proof is an executed test
showing zero escapes; a divergence count of zero means nothing without the run
that tried to produce one. `PG-2-02` asserts a capability — detections are
surfaced — whose proof is the benchmark report `D-02` already produces, with the
counts recorded in it. The predicate reads those counts out of the evidence JSON;
it does not need a second, command-shaped copy of them. That distinction also
explains `PG-2-04` ("…have been **tested**", where the word test is in the clause
itself) and the Phase 1 members `PG-1-04` and `PG-1-05` (fail-closed and
cutoff-exclusion controls, both safety properties).
...
`validate_ledger_structural.py:2553-2562`. `PG-2-02` is `CONDITIONAL_UNACTIVATED` and carries the predicate `AP-PG-2-02`, so the sentence — which describes a clause that has **no** predicate because aggregation made it `REQUIRED_NOW` — does not apply to this row. The prose and the
...
artifact to have reviewed or delegated-approved. The spec-review and delegated-approval obligations sit on `REG-D-02` under S20.
...
**`evidence_refs`.** One reference, re-verified: `EV-PG-2-02-SOURCE`,
`UTF8_LINE_SPAN` over register v2 L165-165, digest
`28e6abf5a629ee2d726d3d1707e590e967f5d1f1a763531f8598f30e45ed780e`, captured
...
`required_evidence` for `PG-2-02` is complete at the input bytes pinned above.
