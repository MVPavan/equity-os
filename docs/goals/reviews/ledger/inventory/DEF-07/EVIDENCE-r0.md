# Inventory review — DEF-07 / EVIDENCE / r0
...
```json
{"evidence_refs":[{"captured_at":"2026-08-13T02:49:11Z","content_sha256":"8712d6404526ef10ca6c24b7160d5ad29acfc5695f44ff2c7cca300dddb5c5e2","digest_mode":"UTF8_LINE_SPAN","end_line":181,"evidence_ref_id":"EV-DEF-07-SOURCE","path":"docs/blueprint/funda-blueprint-implementation-decision-register-v2.md","scope":"Exact authoritative source occurrence for DEF-07","start_line":181},{"captured_at":"2026-08-15T07:13:28Z","content_sha256":"284d496f4b173c2489b1214e5662af0d6d7454db2558f106bbb649878c57ac14","digest_mode":"FILE_BYTES","end_line":null,"evidence_ref_id":"EV-DEF-07-SPEC-DRAFT","path":"docs/specs/equity-os-s02-source-rights-providers-consensus-policy.md","scope":"Current draft specification bytes for DEF-07","start_line":null}],"required_evidence":[{"approval_ids":[],"description":"Current proof satisfying: licensed consensus estimates unless essential","evidence_id":"REQ-DEF-07-ACCEPTANCE","evidence_ref_ids":[],"evidence_type":"ARTIFACT","proof_mode":"CONTENT_HASH","scope":"DEF-07 acceptance and delivery scope","status":"UNRESOLVED"},{"approval_ids":[],"description":"Persisted clean fresh Sol xhigh review of the current specification bytes","evidence_id":"REQ-DEF-07-SPEC-REVIEW","evidence_ref_ids":[],"evidence_type":"REVIEW","proof_mode":"CONTENT_HASH","scope":"DEF-07 under S02","status":"UNRESOLVED"},{"approval_ids":[],"description":"Current negative proof that the deferred scope has no implementation in the current bytes","evidence_id":"REQ-DEF-07-NO-IMPLEMENTATION","evidence_ref_ids":[],"evidence_type":"ARTIFACT","proof_mode":"CONTENT_HASH","scope":"DEF-07 current no-implementation proof","status":"UNRESOLVED"}],"verification_command":{"commands":[],"mode":"UNRESOLVED","not_applicable_review":null}}
```
...
- `reviewed_input_sha256` (pre-record): `e265bfea773a99891605b0af122bdb163455ffdf06970ac3aa467de1f0d7efa9`
- `reviewed_inventory_sha256` (pre-record): `d250ddc2a35f93e57b96848f47d27d5c96071908894d2fa531e5be5641fdf984`
...
181, inside `## G. Explicitly deferred from the first release` (L173):

> - licensed consensus estimates unless essential;

`text_digest` and `EV-DEF-07-SOURCE.content_sha256` both recomputed over the
normalized L181-181 span → `8712d6404526ef10ca6c24b7160d5ad29acfc5695f44ff2c7cca300dddb5c5e2`, matching the
stored values. `required_acceptance_text` is `"licensed consensus estimates unless essential"`.

## Reasoning

**What the source clause actually demands, and whether it is covered — the
conditional case.** The clause is "licensed consensus estimates unless
essential". Its escape makes the deferred scope *conditional in extension*:
what is excluded is non-essential licensed consensus estimates. I checked whether
that demands a second, separately enumerated proof — an essentiality
determination — and concluded it does not belong on this row.

The determination is `REG-C-13`'s: "Decide treatment of consensus estimates —
Licensed and necessary, or explicitly excluded from the MVP", Status `Open`,
derived `REQUIRED_NOW`, carrying both a `PRODUCT_OWNER_DECISION` and a
`DATA_RIGHTS_APPROVAL` — the latter backed on that row by
`REQ-REG-C-13-DATA_RIGHTS_APPROVAL` (`DATA_RIGHTS`/`TYPED_APPROVAL`), read
directly this round. Upstream, `REG-A-05` ("Create
provider and data-rights register scoped to the declared use") holds the rights
inventory itself. Duplicating that adjudication here would violate goal L613-615,
which requires two explicit obligations for two decisions rather than inferred
coverage — the inverse error, but the same rule.

What remains on this row is the standing default, and it is enumerated:
`REQ-DEF-07-NO-IMPLEMENTATION` proves current bytes carry no licensed consensus
estimate outside an essentiality finding, and `REQ-DEF-07-SPEC-REVIEW` binds S02
("source rights, providers, and consensus policy") — the spec that *states* the
policy the escape clause would be adjudicated under.

**A data-rights evidence item, tested and rejected.** `DATA_RIGHTS` evidence
travels as `TYPED_APPROVAL` (goal L487-490) and requires a component-local
`DATA_RIGHTS_APPROVAL` requirement to name. This row has none, and correctly so
— see this component's `APPROVAL` review, which reaches that conclusion
independently. Not acquiring a licence acquires no rights and so evidences none.
...
|---|---|---|---|
| `REQ-DEF-07-ACCEPTANCE` | `ARTIFACT` | `CONTENT_HASH` | the clause's own acceptance scope |
| `REQ-DEF-07-SPEC-REVIEW` | `REVIEW` | `CONTENT_HASH` | a persisted clean review of the current specification bytes |
| `REQ-DEF-07-NO-IMPLEMENTATION` | `ARTIFACT` | `CONTENT_HASH` | current negative proof that the deferred scope has no implementation in the current bytes |
...
whose `program_disposition` is `REJECTED_ACCOUNTED` (`:2734-2736`). `DEF-07` is
...
`REQ-DEF-07-NO-IMPLEMENTATION`. Its future satisfaction is governed by the
...
- *`TYPED_APPROVAL`.* Goal L484-487 requires a `TYPED_APPROVAL` item to name
  ...
  requirement is `APR-DEF-07-01`, of type `DELEGATED_ARTIFACT_APPROVAL`. Swept all
...
- *`REVIEW` — present, and checked against the known defect population.* Of the
  ...
  ("All 32 disposition components omit required review evidence"). `DEF-07` is in
...
Two references: `EV-DEF-07-SOURCE` (UTF8_LINE_SPAN over `docs/blueprint/funda-blueprint-implementation-decision-register-v2.md`, L181-181, captured `2026-08-13T02:49:11Z`); `EV-DEF-07-SPEC-DRAFT` (FILE_BYTES over `docs/specs/equity-os-s02-source-rights-providers-consensus-policy.md`, captured `2026-08-15T07:13:28Z`). Every one resolved and re-digested during the structural validation
...
1. **The `ACCEPTANCE` item's description is still positively framed.**
   `REQ-DEF-07-ACCEPTANCE.description` reads "Current proof satisfying:
   licensed consensus estimates unless essential" — a deferral clause demands no proof that its
...
2. **A vendor model lane survives in `REQ-DEF-07-SPEC-REVIEW.description`**
...
`required_evidence` for `DEF-07` is complete at the input bytes pinned above: the
