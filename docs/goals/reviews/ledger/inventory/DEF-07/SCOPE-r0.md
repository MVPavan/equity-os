# Inventory review — DEF-07 / SCOPE / r0
...
- `text_digest` recomputed over the normalized L181-181 span
  ...
  `8712d6404526ef10ca6c24b7160d5ad29acfc5695f44ff2c7cca300dddb5c5e2`,
  equal to the stored `text_digest`.
- `required_acceptance_text` is `"licensed consensus estimates unless essential"`, which is
  that span with the list marker and terminal punctuation removed — the exact
  clause content.

## Reasoning

**Kind.** Bullet 7 of register §G (L181), excluding licensed consensus estimates
**unless essential**. This is one of only two §G bullets carrying a conditional
escape (the other is `DEF-13`'s "before observed need"), so I tested the
conditional readings hardest here. It is still a `first_release_deferral`: the
escape is an exception *inside* the exclusion's own text, not a separate
reconsideration control, and §G is the source section.
...
**Related register IDs.** `[]`, contract-forced — and this is the component where
the forcing needed the most care, because the escape clause is adjudicated by a
register row that is **`Open`, not `Deferred`**: `REG-C-13` ("Decide treatment of
consensus estimates"), whose acceptance is "Licensed and necessary, or explicitly
excluded from the MVP", derived `REQUIRED_NOW`, carrying both a
`PRODUCT_OWNER_DECISION` (`Product owner`) and a `DATA_RIGHTS_APPROVAL`
(`Data-rights authority`). `REG-A-05` ("Create provider and data-rights register
scoped to the declared use", `Open`) holds the same territory upstream.

I checked whether the "unless essential" escape makes *this component*
conditional and therefore mis-derived. It does not. A conditional derivation
would require a non-null `activation_predicate` (goal L285-286), which
`REQUIRED_NOW` forbids (L288-290), and — decisively — the essentiality
determination is not this row's to make: it is `C-13`'s open decision. The §G
bullet's own content is the standing default (excluded), which is active now. The
derivation is right.

**Source anchor and span.** `source_anchor` is `DEF-07`, a clause ordinal.
...
**Applicable review slot.** `DEF-07` is a non-register canonical row, so
...
only that `DEF-07`'s scope derivation is correct at the input bytes pinned above.
