# Inventory review — DEF-07 / APPROVAL / r0
...
```json
{"approval_records":[],"human_review_id":["HR-0004"],"required_approvals":[{"actor":null,"approval_id":"APR-DEF-07-01","approval_type":"DELEGATED_ARTIFACT_APPROVAL","evidence_ref_ids":[],"matched_record_id":null,"required_authority":"Delegated fresh Sol xhigh specification reviewer","scope":"DEF-07 under S02","status":"UNRESOLVED","timestamp":null}],"security_exception_ids":[]}
```
...
- `reviewed_inventory_sha256` (pre-record): `403dd106be3fb761295408ca3c36362afe27146ea3aaa38322edf647fe8ca5a4`
...
`text_digest` recomputed over the normalized L181-181 span →
`8712d6404526ef10ca6c24b7160d5ad29acfc5695f44ff2c7cca300dddb5c5e2`, matching the stored value.
...
**The escape clause, tested for a hidden authority.** "Unless essential" means
some future actor may find licensed consensus estimates essential and bring them
in. Someone must be authorised to make that finding — so does this row omit that
authority?

No, and the answer is checkable rather than argued. The essentiality finding is
`REG-C-13`'s decision, whose acceptance text is "Licensed and necessary, or
explicitly excluded from the MVP", and that row carries exactly the two
authorities the finding needs: `PRODUCT_OWNER_DECISION` (`Product owner`) for the
necessity call and `DATA_RIGHTS_APPROVAL` (`Data-rights authority`) for the
licence, the latter backed by `REQ-REG-C-13-DATA_RIGHTS_APPROVAL`. `REG-A-05`
holds the upstream provider and data-rights register with its own
`DATA_RIGHTS_APPROVAL`. Duplicating either here would violate goal L188 — "one
approval never implies another", and by the same logic one decision does not
belong to two rows — and would create a second obligation for a single
real-world decision, which goal L613-615 addresses from the opposite direction.

What remains on this row is the default position, and a default that is already
the register's stated position needs no approval to hold.

**The enumerated obligation.** One requirement, `APR-DEF-07-01`, type
`DELEGATED_ARTIFACT_APPROVAL`, authority "Delegated fresh Sol xhigh
specification reviewer", scope "DEF-07 under S02", `UNRESOLVED`. It is the delegated
...
| `PRODUCT_OWNER_DECISION` (incl. `Product owner authorized to activate deferred blueprint scope`) | `REG-C-13` ('Decide treatment of consensus estimates') carries a plain `Product owner` decision — and it is `Open`, `REQUIRED_NOW`, not deferred, so the *decision* about consensus estimates is live now while this clause states the standing default until it is made. |
...
| `DATA_RIGHTS_APPROVAL`, `LEGAL_REVIEW`, `REGULATORY_REVIEW` | **This is the sharpest check in the batch.** 'Licensed' is a rights word, and `DATA_RIGHTS_APPROVAL` (`Data-rights authority`) exists on five rows including `REG-A-05` (the provider and data-rights register) and `REG-C-13` (the consensus-estimate decision itself). But a data-rights approval authorises *acquiring and using* rights. Declining to license acquires nothing, and there is no authority whose job is to approve the absence of a licence. Legal and regulatory likewise: no contract is entered. |
...
`required_approvals`. `DEF-07` is linked to `HR-0004` only.
...
`required_approvals` for `DEF-07` is complete at the input bytes pinned above: the
source clause demands exactly one typed approval — the delegated artifact
approval already enumerated as `APR-DEF-07-01` — and no authority whose
