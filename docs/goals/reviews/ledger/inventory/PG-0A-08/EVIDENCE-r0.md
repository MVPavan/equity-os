# Inventory review — PG-0A-08 / EVIDENCE / r0
...
```json
{"evidence_refs":[{"captured_at":"2026-08-13T02:49:11Z","content_sha256":"47924ca3115e35ab64986b0b6e0fdeb55bed240956f0e67214835860670321f3","digest_mode":"UTF8_LINE_SPAN","end_line":133,"evidence_ref_id":"EV-PG-0A-08-SOURCE","path":"docs/blueprint/funda-blueprint-implementation-decision-register-v2.md","scope":"Exact authoritative source occurrence for PG-0A-08","start_line":133}],"required_evidence":[{"approval_ids":[],"description":"Current proof satisfying: the golden-set owner and initial cases exist","evidence_id":"REQ-PG-0A-08-ACCEPTANCE","evidence_ref_ids":[],"evidence_type":"ARTIFACT","proof_mode":"CONTENT_HASH","scope":"PG-0A-08 acceptance and delivery scope","status":"UNRESOLVED"}],"verification_command":{"commands":[],"mode":"UNRESOLVED","not_applicable_review":null}}
```
...
- `reviewed_inventory_sha256` (pre-record): `67f18ff6ab12bb87cb30f383feb097823b092ad47d259be7ca170dc816f246a5`
...
|---|---|---|---|---|---|
| `REQ-PG-0A-08-ACCEPTANCE` | `ARTIFACT` | `CONTENT_HASH` | `UNRESOLVED` | `[]` | `[]` |
...
**Clause-by-clause coverage.** Two named existents, one proof: the golden-set
owner exists and the initial cases exist. Register A-08 makes both concrete —
"Named owner, repository location, review cadence, and first twenty labeled
cases, including prompt-injection/source-confusion cases". The owner is a name in
a document and the cases are a labelled set in a repository location the same
document records, so a single `ARTIFACT`/`CONTENT_HASH` item over that record
covers both. This is the last bullet of the Phase 0A block and its terminal
punctuation is `.` rather than `;`, which I confirmed against the span; the
acceptance description drops it, as it drops the `;` on the other seven.

**No `COMMAND` item is owed at this gate — but note where the ledger does place
one.** "Existence" is not a runtime property. The automation obligation for the
same golden set appears one phase later, at `PG-05-10` ("the first golden cases
are automated or consistently reviewable"), and even there the ledger declares no
`COMMAND` item, because "automated **or** consistently reviewable" is a
disjunction whose second branch is human. I checked that row rather than assuming
symmetry. Nothing in *this* clause asks for an executed check.

**No `TYPED_APPROVAL` item is owed.** `REG-A-08` carries `APR-REG-A-08-02`
(`NAMED_OWNER_COMMITMENT`, "Golden-set owner") — one of only three
`NAMED_OWNER_COMMITMENT` requirements in the whole ledger — with a paired
`REQ-REG-A-08-NAMED_OWNER_COMMITMENT` typed item. This gate carries neither,
because its verb is "exist", not "is appointed" or "has committed": the register
decision is "**Appoint** golden-test-set owner", and appointment is what carries
the commitment. The commitment obligation is therefore inventoried, on the row
whose source text demands it. Whether this row's empty `required_approvals` is
right is decided by this component's `APPROVAL` review.
...
**`evidence_refs`.** One object, `EV-PG-0A-08-SOURCE`, a
`UTF8_LINE_SPAN` binding to register v2 L133-133 with scope "Exact authoritative source occurrence for PG-0A-08".
I re-hashed it against current bytes: `47924ca3115e35ab64986b0b6e0fdeb55bed240956f0e67214835860670321f3`, equal to
...
**Corroboration, treated as corroboration only.** `PG-0A-08` is in neither list
of the program-level evidence-inventory review, and `human_review_id` is `null`.
