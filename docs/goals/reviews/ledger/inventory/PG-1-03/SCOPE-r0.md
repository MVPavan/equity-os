# Inventory review — PG-1-03 / SCOPE / r0
...
```json
{"activation_predicate":null,"disposition_refs":[],"gate_refs":[],"related_register_ids":["B-05","C-03"],"scope_derivation":{"authority_effect":null,"derived_program_disposition":"REQUIRED_NOW","related_register_ids":["B-05","C-03"],"rule":"RELATED_REGISTER_SCOPE"}}
```
...
- `reviewed_inventory_sha256` (pre-record): `7d3faec0bedc13872407de3cdf80ab3b3ce800f47b860c666e4e92ebf788d53f`
...
**`related_register_ids` = `["B-05", "C-03"]` — a definition register plus its
Phase-1 implementation.** The clause enumerates five attributes that must be
explicit: units, period, currency, statement scope, and definition. `B-05`
(register L55), "Derive minimum source and fact schemas from actual use", is the
register decision that carries every one of them: its acceptance reads "Schema
supports raw/normalized values, **dimensions** (units and currency), **scope**,
source location, **valid time** (period), knowledge time, revisions, **definition
version**, and quality/reconciliation status". `C-03` (register L74), "Implement
append-only observation and revision model", is the Phase-1 decision under which
observations carrying those fields are actually stored without silent overwrite.

**Why a Phase-1 gate needs both.** This is a Phase 1 exit condition, so it cannot
rest only on a Phase 0.5 schema decision — it asks whether stored observations
*are* explicit, which requires the Phase-1 store as well as the schema. That is
the same shape the two neighbouring Phase-1 gates use: `PG-1-01` and `PG-1-02`
each pair a policy register (`A-10`, Phase 0A) with its Phase-1 implementation
(`C-04`). Here the pairing is `B-05` (Phase 0.5) with `C-03` (Phase 1).

**Contrary candidate, considered and rejected — the strongest in this batch.**
`B-12` ("Establish versioned metric and predicate registries", acceptance
"Registry definitions, aliases, **object/unit/dimension rules**, addition
approval, deprecation, and versioning exist") owns the words "unit" and
"definition" more explicitly than B-05 does, and is a genuine candidate. It is
rejected because B-12's gate home is `PG-05-06` (register L142, "fact
identity/revision rules and metric/predicate registries are in use"), where it is
paired with `B-11`; `REG-B-12.gate_refs == ["PG-05-06"]`. Claiming B-12 here as
well would double-book one register decision across two gates on *inferred*
rather than exact semantics — this clause is about the attributes present on an
observation, not about the registry those attributes resolve into.

**A corroborating cross-check on the pairing.** `REG-B-05.disposition_refs` and
`REG-C-03.disposition_refs` are both exactly `["M-2"]`. M-2 (disposition report
L130, "Fact identity and revision semantics") item 3 defines the "economic
measurement slot" as "the intended metric, entity, **period, scope, dimensions,
and definition**" — the clause's five attributes, named by the same finding that
both of these registers descend from. The third register in the M-2 cluster,
`B-11`, is claimed by `PG-05-06`, so the cluster is partitioned across the two
gates with no overlap.

**Back-reference verified.** `REG-B-05.gate_refs == ["PG-05-07", "PG-1-03"]` and
`REG-C-03.gate_refs == ["PG-1-03"]`; the `gate_map` equality holds on both.

**Aggregation → `REQUIRED_NOW`.** Both related registers are `REQUIRED_NOW`
(`source_status=Open`); aggregate is `REQUIRED_NOW` under goal L248-250. Stored
values agree.
...
related registers rather than duplicating it: here `B-05` → `M-2`; `C-03` → `M-2`. That inheritance
