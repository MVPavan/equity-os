# Inventory review — DISP-M-8 / EVIDENCE / r0
...
```json
{"evidence_refs":[{"captured_at":"2026-08-13T02:49:11Z","content_sha256":"671b7ce1b3059934ba4fda735adb70538afeb9c33015d328b6bb70beccfd69bb","digest_mode":"UTF8_LINE_SPAN","end_line":250,"evidence_ref_id":"EV-DISP-M-8-SOURCE","path":"docs/blueprint/funda-third-order-review-disposition-report.md","scope":"Exact authoritative source occurrence for DISP-M-8","start_line":240},{"captured_at":"2026-08-13T02:49:11Z","content_sha256":"96add3840a6ff8c6280a70a75242b20a1615e50e9a49236d1bd3636f7e22d9ba","digest_mode":"FILE_BYTES","end_line":null,"evidence_ref_id":"EV-DISP-M-8-SPEC-DRAFT","path":"docs/specs/equity-os-s08-success-metrics-budgets-capacity.md","scope":"Current draft specification bytes for DISP-M-8","start_line":null}],"required_evidence":[{"approval_ids":[],"description":"Current proof satisfying: ### M-8 — Results-season throughput\n\n**Disposition: Accept and fold into the success-metric contract.**\n\nCoverage capacity during clustered reporting periods is a product constraint. It need not become a separate architecture subsystem, but the register should track:\n\n- reports reviewable per analyst per week;\n- peak-week document and claim volume;\n- backlog age;\n- percent of updates completed before the next material event;\n- capacity at the selected Phase 1 company count.","evidence_id":"REQ-DISP-M-8-ACCEPTANCE","evidence_ref_ids":[],"evidence_type":"ARTIFACT","proof_mode":"CONTENT_HASH","scope":"DISP-M-8 acceptance and delivery scope","status":"UNRESOLVED"}],"verification_command":{"commands":[],"mode":"UNRESOLVED","not_applicable_review":null}}
```
...
- `reviewed_inventory_sha256` (pre-record): `955148931921504541cf4516027cbf13efaa054ae40752d175d546f7895e8227`
...
**Enumerated.** One item: `REQ-DISP-M-8-ACCEPTANCE`, `ARTIFACT` /
`CONTENT_HASH`, scope "DISP-M-8 acceptance and delivery scope", `UNRESOLVED`.

**Demand-by-demand.** The clause's five bullets are all *tracking* demands on the
success-metric contract — reports reviewable per analyst per week, peak-week
document and claim volume, backlog age, percent of updates completed before the
next material event, and capacity at the selected Phase 1 company count. Each is
satisfied by the contract text containing that metric, which is a content-hash
obligation. The clause's own carve-out — "It need not become a separate
architecture subsystem" — rules out an implementation demand.

**The capacity trap, checked.** Four of the five bullets are capacity-flavoured,
and the goal is explicit that capacity evidence "always uses `TYPED_APPROVAL` and
the typed approval/human-review path, never a fabricated shell command". If this
clause demanded a capacity *commitment*, a missing `CAPACITY`/`TYPED_APPROVAL`
item would be a real omission. It does not: it demands that the register *track*
the metrics. The commitment obligations exist and are enumerated on the exact rows
this component's derivation names — `REQ-REG-C-18-CAPACITY_COMMITMENT`
(`CAPACITY` / `TYPED_APPROVAL`) on REG-C-18, and `REQ-REG-A-12-BUDGET_APPROVAL`
plus `REQ-REG-A-12-CAPACITY_COMMITMENT` on REG-A-12. Duplicating them here would
be the error the goal warns against, since one approval never implies another and
each obligation is recorded once in its own scope.

**No command obligation.** Consistent with the above, neither `DISP-M-8` nor
REG-C-18 appears in the validator's pinned `EXPECTED_COMMAND_PROOF_COMPONENTS`
manifest (`:2635-2649`), while its section-mate `DISP-M-9`, whose clause demands
test cases, does. Throughput measured over analyst work is operational data
accepted by a capacity owner, not a repository command.

**`evidence_refs` as read.** Two objects, both re-hashed this round against
current bytes: `EV-DISP-M-8-SOURCE` (`UTF8_LINE_SPAN` L240-250, whose
`content_sha256` equals the row's `text_digest`) and `EV-DISP-M-8-SPEC-DRAFT`
(`FILE_BYTES` over the S08 spec). Both resolve; neither is stale. They are
evidence objects, not obligations — their presence does not satisfy the
...
only that `DISP-M-8`'s `required_evidence` inventory is correct at the input bytes pinned above.
