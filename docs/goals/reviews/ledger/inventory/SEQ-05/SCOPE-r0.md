# Inventory review — SEQ-05 / SCOPE / r0
...
**`source_register_ids` = `["A-04"]`, verified against the clause.** The clause's
lead is "**A-04 v0:**" — one register ID, qualified by a version marker. The `v0`
is not a second reference and correctly contributes no ID; it is what distinguishes
this step from `SEQ-07` ("**A-04 final:**"), which sources the same register row.
`validate_ledger_structural.py:2484` pins `SEQ-05: (["A-04"], ["S06"])`.
...
register, single owning spec, so the closure is `{S06}` and the multi-spec
applicability question does not arise here.

**`SEQ-05` and `SEQ-07` share a `SCOPE` inventory digest — checked, not assumed.**
Both reduce to the same `SCOPE` projection
(`91…`/`ba33f31d930ecfff00a38181745d4b52f4b34fb54788d9741f484a401a1441a9` for
both), because every field the projection covers is identical: same rule, same
empty related/disposition/gate refs, same null predicate, same single register and
single spec. That is correct rather than a collision: the two clauses differ in
their acceptance text, source span, and `text_digest`, all of which are covered by
`reviewed_input_sha256` (which does differ: `b916732e…` here versus `dad5d2e8…`
on `SEQ-07`), so the two reviews are not interchangeable. What this review decides
that the shared digest cannot is whether the live occurrence justifies the
derivation — and it does: a provisional contract created to instrument the baseline
is program-wide sequencing, not a scoped register relation.

**Source anchor and span.** `SEQUENCE-05` at L455-455. `text_digest` recomputed
over the normalized span → `c75aebea…`, matching; `required_acceptance_text`
...
and a `blocked_scope` entry for bead `eqos-0xb.6`. None of these fields is in the
`SCOPE` inventory projection or is a `scope_derivation` input under goal L226-235;
the blocker constrains delivery, not derivation. It is inside
`reviewed_input_sha256`, so a change to it correctly stales this review.
...
only that `SEQ-05`'s scope derivation is correct at the input bytes pinned
above.
