# Inventory review — PG-2-01 / APPROVAL / r0
...
`primary_spec` is `null` on every phase-gate row. The artifact approvals sit on `REG-D-02` and `REG-D-05` under S20.
...
**Applying it to this clause.** "…**improves** measurable current-scale workflow
outcomes" is a comparative measurement verb. Nothing is approved, accepted, or
decided by the clause; a benchmark either clears a pre-committed bar or does
not, and `AP-PG-2-01` says so in four metrics with no `..._improved` flag
permitted (`validate_ledger_structural.py:2529`, `:2542`).

**The near-miss, and it is a real one: "the **selected** memory approach".**
Selection is a decision, and `REG-D-05` carries `PRODUCT_OWNER_DECISION` /
"Product owner for memory adoption" for exactly that act (plus the separate
"Product owner authorized to activate deferred blueprint scope" requirement for
un-deferring `D-05` at all). So why does the selection word not pull a
product-owner obligation onto this gate? Because it is a **participial
reference to an upstream decision**, in the same grammatical position as "under
A-10" in `PG-1-01`/`PG-1-02` and "the agreed threshold" in `PG-1-08` — all of
which carry empty approval lists although their referenced rows carry
authorities. The gate does not select; it measures whatever was selected. The
gate that *does* carry the Phase 2 product-owner judgment is `PG-2-05`
("operational burden **is acceptable**"), where the acceptance is the gate's own
object. Adding a second product-owner obligation here would be the duplicate
coverage goal L613-615 requires be handled by distinct explicit obligations
...
|---|---|
| `PRODUCT_OWNER_DECISION` | The near-miss, argued above. Two of the ledger's 23 instances are on `REG-D-05` and one is on `REG-D-02`, covering both selection and activation; only one sits on a phase-gate row at all (`PG-2-05`). |
| `MEMORY_PROMOTION` | Its single instance is `REG-C-10`'s, for canonical promotion of narrative memory in Phase 1. This clause promotes nothing; it evaluates a memory *approach*. |
| `ANALYST_ACCEPTANCE` | "Analyst time" is one of `D-02`'s measured dimensions, so an analyst appears as a measurement subject, not as an accepting authority. No thesis, narrative, or output is accepted. |
| `DOMAIN_EXPERT_ACCEPTANCE` | A benchmark comparison is not a calculation, data, entity, or vocabulary ruling; none of the five authority literals fits. |
| `BUDGET_APPROVAL`, `CAPACITY_COMMITMENT` | `D-02` measures cost and operations but commits neither spend nor capacity. Where activation would create budget exposure the contract says so explicitly — `REG-E-03` and `REG-E-05` carry `BUDGET_APPROVAL`; `REG-D-02` and `REG-D-05` do not. |
| `NAMED_OWNER_COMMITMENT` | No owner appointed. |
| `DATA_RIGHTS_APPROVAL`, `LEGAL_REVIEW`, `REGULATORY_REVIEW` | No acquisition or regulated activity. The GBrain dependency-licence review is `REG-D-04`'s `LEGAL_REVIEW`, on the row that actually examines the repository. |
