# Inventory review — DEF-13 / APPROVAL / r0
...
```json
{"approval_records":[],"human_review_id":["HR-0003","HR-0004"],"required_approvals":[{"actor":null,"approval_id":"APR-DEF-13-01","approval_type":"DELEGATED_ARTIFACT_APPROVAL","evidence_ref_ids":[],"matched_record_id":null,"required_authority":"Delegated fresh Sol xhigh specification reviewer","scope":"DEF-13 under S10","status":"UNRESOLVED","timestamp":null}],"security_exception_ids":[]}
```
...
- `reviewed_inventory_sha256` (pre-record): `13c86e8a9651683f0ebd678ba6d60018db03f66927754a844197c7cd903b77d0`
...
`text_digest` recomputed over the normalized L187-187 span →
`1c36076cced7ec5976c3da55182647e1f54fe00fc931e254e0165f499721efb4`, matching the stored value.
...
**The open finding names an authority. That is the check this review turns on.**
`DEF-13` carries `R3-F-01` (severity Important, `load_bearing: true`, `status:
OPEN_BLOCKING`, disposition `UPHELD`), and that finding records a
`required_authority` object of its own:

> `approval_type: GOAL_OR_PROCESS_AUTHORIZATION`, `authority: Explicit rank-1
> current-user authority`, `ordinary_r5_permitted: false`, `required_for: A
> targeted post-cap S10 amendment and a separate fresh exact-byte Sol xhigh
> amendment review.`

So an authority *is* demanded somewhere in this row's orbit and is *not* in
`required_approvals`. I treated that as a candidate omission and resolved it
against the contract rather than by inspection:

1. `GOAL_OR_PROCESS_AUTHORIZATION` is in the approval-type vocabulary (goal
   L540) but **deliberately absent** from the closed required-authority table
   (goal L562-575). Goal L583-584: an absent type "has no obligation in this
   inventory and gains one only through a reconciled, reviewed, approved change."
2. Mechanically, `validate_ledger_structural.py:2629` asserts every
   non-delegated `approval_type` is in `REQUIRED_AUTHORITY_VOCABULARY`. An entry
   of this type would fail structural validation, so it is not merely
   discouraged — it is **unrepresentable** in `required_approvals`.
3. The contract routes it instead: the finding carries its own
   `required_authority` object, and the row is linked to `HR-0003`, the
   human-review entry that holds the blocked cone. That is the designated path
   for an authority an agent cannot establish (goal L1001-1028, whose row 'Why human/external' at L1028 is exactly that field).

Its absence from `required_approvals` is therefore contract-correct rather than
an omission, and this is the one component in the batch where the emptiness of
the non-delegated set had to be defended against a live counter-example rather
than a hypothetical.

**What this verdict does not touch.** `R3-F-01` remains `OPEN_BLOCKING`; the
`HR-0003` blocked cone (`REG-B-03`, `REG-C-11`, `DEF-13`, `SCALE-SQLITE-01`…`04`,
`DISP-T-3`, `DISP-R-5`, product-implementation scope `ALL_PRODUCT_IMPLEMENTATION`)
remains in force; `delivery_status` remains `REVIEW_BLOCKED`. None of
`open_findings`, `blocked_scope`, or `delivery_status` is in the `APPROVAL`
inventory projection, and this review decides only the completeness of
`required_approvals`.

**The enumerated obligation.** One requirement, `APR-DEF-13-01`, type
`DELEGATED_ARTIFACT_APPROVAL`, authority "Delegated fresh Sol xhigh
specification reviewer", scope "DEF-13 under S10", `UNRESOLVED`. It is the delegated
...
| `PRODUCT_OWNER_DECISION` (incl. `Product owner authorized to activate deferred blueprint scope`) | `REG-D-01` ('Implement `MemoryStore` interface before choosing engine') is `Open` and `REQUIRED_NOW`, so no activate-deferred authority attaches to the engine question at all — the register's position is engine-neutrality now, not a dormant migration awaiting a signature. |
...
`required_approvals`. `DEF-13` is the only §G row linked to two human-review entries, `['HR-0003','HR-0004']`, and the only one carrying an open finding — see below.
...
enumerate. `human_review_id` is `HR-0003`, `HR-0004`, normalized to a sorted set by `normalized_human_review_id` before the projection is digested.
...
`required_approvals` for `DEF-13` is complete at the input bytes pinned above: the
source clause demands exactly one typed approval — the delegated artifact
approval already enumerated as `APR-DEF-13-01` — and no authority whose
