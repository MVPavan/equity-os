# Inventory review — DEF-13 / SCOPE / r0
...
- `text_digest` recomputed over the normalized L187-187 span
  ...
  `1c36076cced7ec5976c3da55182647e1f54fe00fc931e254e0165f499721efb4`,
  equal to the stored `text_digest`.
- `required_acceptance_text` is `"migration to a distributed workflow engine or PostgreSQL before observed need"`, which is
  that span with the list marker and terminal punctuation removed — the exact
  clause content.

## Reasoning

**Kind.** Bullet 13 of register §G (L187) — the section's only bullet ending in a
full stop rather than a semicolon, being the list terminator — excluding
migration to a distributed workflow engine or PostgreSQL **before observed
need**. `first_release_deferral` is correct, and here the `scale_trigger`
alternative is not merely plausible but genuinely adjacent, because "observed
need" is precisely what §H's triggers define. It is still the wrong kind for
*this* occurrence: goal L143 pins `scale_trigger` to the eight §H bullets at
L197-200 and L204-207, which are already inventoried as `SCALE-SQLITE-01..04`
and `SCALE-WORKFLOW-01..04`. This is a §G bullet and takes the §G rule.
...
**Related register IDs.** `[]`, contract-forced, and this is the batch's richest
case for checking what the forcing costs. This bullet's surrounding scope is
carried by **four separate inventory populations**, only one of which is a
register row:

- `REG-D-01` ("Implement `MemoryStore` interface before choosing engine",
  Status `Open`, `REQUIRED_NOW`), the engine-neutrality obligation — the one
  register row in scope, and the only ID `related_register_ids` could even
  express;
- the eight `scale_trigger` rows from §H, which define "observed need"
  operationally and each carry a `REQ-SCALE-*-REEVALUATION-CONTROL` obligation;
- `AUTH-REG-002` (register L193, "These are operating notes, not Phase 0.5
  blockers") and `AUTH-REG-003` (register L209, "No specific replacement
  technology is committed by this register") — both `authority_clause` rows
  pinned by name and line in goal L145.

Three of those four are non-register components, which `related_register_ids`
structurally cannot hold: the field takes register IDs only (goal L232-234). So
even a rule that permitted the field would capture at most `D-01` and would
misrepresent the scope by omitting the rest. `[]` plus
`PROGRAM_WIDE_ACTIVE_CONTROL` is the accurate encoding.

**`DEF-13`-specific state, recorded so this verdict is not over-read.** This is
the only §G row with a nonempty `open_findings` (`R3-F-01`, severity Important,
`status: OPEN_BLOCKING`, disposition `UPHELD`, load-bearing), the only one whose
`delivery_status` is `REVIEW_BLOCKED`, the only one at `review_round: 4`, the
only one carrying five `evidence_refs` rather than two, and the only one linked
to two human-review entries (`['HR-0003','HR-0004']`). None of `open_findings`,
`delivery_status`, `review_round`, or `evidence_refs` appears in the `SCOPE`
inventory projection, so none is within this review's decided question. This
`CLEAN` verdict decides the scope derivation only and clears nothing about
`R3-F-01` or `HR-0003`.

**Source anchor and span.** `source_anchor` is `DEF-13`, a clause ordinal.
...
**Applicable review slot.** `DEF-13` is a non-register canonical row, so
...
only that `DEF-13`'s scope derivation is correct at the input bytes pinned above.
