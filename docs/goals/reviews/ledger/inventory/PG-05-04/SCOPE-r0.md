# Inventory review — PG-05-04 / SCOPE / r0
...
```json
{"activation_predicate":null,"disposition_refs":[],"gate_refs":[],"related_register_ids":["B-04","B-13"],"scope_derivation":{"authority_effect":null,"derived_program_disposition":"REQUIRED_NOW","related_register_ids":["B-04","B-13"],"rule":"RELATED_REGISTER_SCOPE"}}
```
...
- `reviewed_inventory_sha256` (pre-record): `f1d7573535a72de445b305eadebc45b7f632f3bf7289b37afe1b4834b08d3eae`
...
**Related register IDs — `["B-04", "B-13"]`.** The clause has a positive half and a
negative half, and the two registers cover them:

- "claim-level review telemetry and correction categories are available" → B-04
  (register L54), whose acceptance enumerates "claim count; **per-claim disposition and
  time**; source-locate and calculation-check time; accepted/edited/rejected/deferred
  counts; **correction categories**". Both nouns in the clause appear verbatim there.
- "without invalid percentile claims" → B-04 again, which ends "**no report-level P90 is
  used at n=3**", and B-13 (register L63), "Add reviewer-bias and measurement controls",
  whose acceptance requires results to be "stratified by materiality and epistemic class"
  and instrumentation to be "symmetric and overhead measured". B-13 is the row that makes
  a measurement claim valid rather than merely present, which is what the negative half
  of this clause constrains.

I checked whether B-13 is padding. It is not: `REG-B-13.gate_refs == ["PG-05-04"]` — this
is B-13's *only* gate, so if this clause did not draw on B-13, B-13 would be gated
nowhere, and the validator's `gate_map` assertion (`:2659-2666`) would have to be
satisfied by some other phase-gate row. None mentions reviewer bias or measurement
validity. B-04's backlink is `["PG-05-03", "PG-05-04", "PG-1-08"]`, distinguishing the
timing record (`PG-05-03`), this telemetry clause, and the Phase 1 economics gate.

**Aggregated disposition.** B-04 and B-13 are both `REQUIRED_NOW`; the aggregation
yields `REQUIRED_NOW` (goal L248-250), matching the stored value.
...
**A negative constraint does not make this a negative control.**
`ACTIVE_NEGATIVE_CONTROL` "proves that named capabilities stay dormant or rejected"
(goal L259-264). This clause forbids a *measurement practice*, not a capability, and its
related rows are active, not dormant. It is correctly `RELATED_REGISTER_SCOPE`; the
validator independently forecloses the alternative by pinning the negative-control set to
`{"PG-1-11"}` (`:2520-2523`).
