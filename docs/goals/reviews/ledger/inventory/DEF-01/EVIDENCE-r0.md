# Inventory review — DEF-01 / EVIDENCE / r0
...
```json
{"evidence_refs":[{"captured_at":"2026-08-13T02:49:11Z","content_sha256":"2758ee1a5c8ef072c353836c26cbee3c26b7dd792f1649bd5529e1d92fa51570","digest_mode":"UTF8_LINE_SPAN","end_line":175,"evidence_ref_id":"EV-DEF-01-SOURCE","path":"docs/blueprint/funda-blueprint-implementation-decision-register-v2.md","scope":"Exact authoritative source occurrence for DEF-01","start_line":175},{"captured_at":"2026-08-15T07:13:28Z","content_sha256":"3f3e371f7a71683c3befed5e6f0d6daff4bd2a5f630ad4aa9b62c31b1070885e","digest_mode":"FILE_BYTES","end_line":null,"evidence_ref_id":"EV-DEF-01-SPEC-DRAFT","path":"docs/specs/equity-os-s05-discovery-company-vertical-slice.md","scope":"Current draft specification bytes for DEF-01","start_line":null}],"required_evidence":[{"approval_ids":[],"description":"Current proof satisfying: full company initiation as an automated product","evidence_id":"REQ-DEF-01-ACCEPTANCE","evidence_ref_ids":[],"evidence_type":"ARTIFACT","proof_mode":"CONTENT_HASH","scope":"DEF-01 acceptance and delivery scope","status":"UNRESOLVED"},{"approval_ids":[],"description":"Persisted clean fresh Sol xhigh review of the current specification bytes","evidence_id":"REQ-DEF-01-SPEC-REVIEW","evidence_ref_ids":[],"evidence_type":"REVIEW","proof_mode":"CONTENT_HASH","scope":"DEF-01 under S05","status":"UNRESOLVED"},{"approval_ids":[],"description":"Current negative proof that the deferred scope has no implementation in the current bytes","evidence_id":"REQ-DEF-01-NO-IMPLEMENTATION","evidence_ref_ids":[],"evidence_type":"ARTIFACT","proof_mode":"CONTENT_HASH","scope":"DEF-01 current no-implementation proof","status":"UNRESOLVED"}],"verification_command":{"commands":[],"mode":"UNRESOLVED","not_applicable_review":null}}
```
...
- `reviewed_inventory_sha256` (pre-record): `3645effeba3147cc3034c63315c8dba0586fa6178fa2aa08a1d6844964704f38`
...
`text_digest` and `EV-DEF-01-SOURCE.content_sha256` both recomputed over the
normalized L175-175 span → `2758ee1a5c8ef072c353836c26cbee3c26b7dd792f1649bd5529e1d92fa51570`, matching the
stored values. `required_acceptance_text` is `"full company initiation as an automated product"`.

## Reasoning

**What the source clause actually demands, and whether it is covered.** The
clause excludes *full company initiation as an automated product*. Read against
the register's positive commitments — `REG-A-02` (one discovery company, four
consecutive quarters) and `REG-A-03` (Quarter 0 performed manually) — the proof
the clause demands is that the current bytes contain no automated company-onboarding
product path. `REQ-DEF-01-NO-IMPLEMENTATION` carries exactly that obligation
against the current bytes, and `REQ-DEF-01-SPEC-REVIEW` binds it to the spec that
would be the place to violate it: S05, "Discovery-company vertical slice, manual
baseline, and bootstrap thesis" — a spec whose own title commits to a *slice*
with a *manual baseline*. There is no second, differently-typed proof the clause
demands: it names no measurement, no third party, no resource, and no
distribution act.
...
|---|---|---|---|
| `REQ-DEF-01-ACCEPTANCE` | `ARTIFACT` | `CONTENT_HASH` | the clause's own acceptance scope |
| `REQ-DEF-01-SPEC-REVIEW` | `REVIEW` | `CONTENT_HASH` | a persisted clean review of the current specification bytes |
| `REQ-DEF-01-NO-IMPLEMENTATION` | `ARTIFACT` | `CONTENT_HASH` | current negative proof that the deferred scope has no implementation in the current bytes |
...
whose `program_disposition` is `REJECTED_ACCOUNTED` (`:2734-2736`). `DEF-01` is
...
`REQ-DEF-01-NO-IMPLEMENTATION`. Its future satisfaction is governed by the
...
- *`TYPED_APPROVAL`.* Goal L484-487 requires a `TYPED_APPROVAL` item to name
  ...
  requirement is `APR-DEF-01-01`, of type `DELEGATED_ARTIFACT_APPROVAL`. Swept all
...
- *`REVIEW` — present, and checked against the known defect population.* Of the
  ...
  ("All 32 disposition components omit required review evidence"). `DEF-01` is in
...
Two references: `EV-DEF-01-SOURCE` (UTF8_LINE_SPAN over `docs/blueprint/funda-blueprint-implementation-decision-register-v2.md`, L175-175, captured `2026-08-13T02:49:11Z`); `EV-DEF-01-SPEC-DRAFT` (FILE_BYTES over `docs/specs/equity-os-s05-discovery-company-vertical-slice.md`, captured `2026-08-15T07:13:28Z`). Every one resolved and re-digested during the structural validation
...
1. **The `ACCEPTANCE` item's description is still positively framed.**
   `REQ-DEF-01-ACCEPTANCE.description` reads "Current proof satisfying:
   full company initiation as an automated product" — a deferral clause demands no proof that its
...
2. **A vendor model lane survives in `REQ-DEF-01-SPEC-REVIEW.description`**
...
`required_evidence` for `DEF-01` is complete at the input bytes pinned above: the
