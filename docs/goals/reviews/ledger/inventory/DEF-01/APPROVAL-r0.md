# Inventory review — DEF-01 / APPROVAL / r0
...
```json
{"approval_records":[],"human_review_id":["HR-0004"],"required_approvals":[{"actor":null,"approval_id":"APR-DEF-01-01","approval_type":"DELEGATED_ARTIFACT_APPROVAL","evidence_ref_ids":[],"matched_record_id":null,"required_authority":"Delegated fresh Sol xhigh specification reviewer","scope":"DEF-01 under S05","status":"UNRESOLVED","timestamp":null}],"security_exception_ids":[]}
```
...
- `reviewed_inventory_sha256` (pre-record): `eca5de346e21a866c9abb7c8be4d895b0035694e71f1271abe036961773bb935`
...
`2758ee1a5c8ef072c353836c26cbee3c26b7dd792f1649bd5529e1d92fa51570`, matching the stored value.
...
**The clause tested for a hidden authority.** "Full company initiation as an
automated product" is a *product-shape* statement, and product shape is exactly
where `PRODUCT_OWNER_DECISION` lives in this ledger — seven rows carry the plain
`Product owner` authority, including `REG-A-01` ("Freeze initial user and
distribution boundary") and `REG-A-02` ("Select one discovery company and four
consecutive quarters"). So the candidate is real and I checked it rather than
dismissing it. It fails on the direction of the decision: `A-01` and `A-02` are
the rows where the owner *chooses* the boundary, and they carry the approval;
this §G bullet states what falls outside the boundary already chosen. Goal
L613-615 requires two explicit obligations for two decisions rather than inferred
coverage — but there is only one decision here, and it is `A-01`/`A-02`'s.

**The enumerated obligation.** One requirement, `APR-DEF-01-01`, type
`DELEGATED_ARTIFACT_APPROVAL`, authority "Delegated fresh Sol xhigh
specification reviewer", scope "DEF-01 under S05", `UNRESOLVED`. It is the delegated
...
| `PRODUCT_OWNER_DECISION` (incl. `Product owner authorized to activate deferred blueprint scope`) | `REG-A-01` and `REG-A-02` carry `PRODUCT_OWNER_DECISION` (`Product owner`) for the product and universe boundary decisions themselves. This clause records a consequence of those decisions rather than making one, and no register row is `Deferred` for automated company initiation, so the activate-deferred authority has nothing here to activate. |
...
`required_approvals`. `DEF-01` is linked to `HR-0004`, the reconciliation that recorded this row's current authority state; it carries no open finding and no unresolved human question of its own.
...
`required_approvals` for `DEF-01` is complete at the input bytes pinned above: the
source clause demands exactly one typed approval — the delegated artifact
approval already enumerated as `APR-DEF-01-01` — and no authority whose
