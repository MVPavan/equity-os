# Inventory-review recording design — r4
...
### 0.0 r3 → r4 — why this round exists (bead `eqos-w78`, round 3)
...
### 0.0a r2 → r3 — why r3 existed (bead `eqos-w78`)
...
### 0.0b Round lineage below this point
...
quote the superseded round freely: quoted text inside a fenced block or a
blockquote is not scanned (§2.2.1).
...
*(New in r3, per §0.0a; corrected in r4 per §0.0. This subsection is the
...
**Which lines are scanned at all — fences and blockquotes (r4, r3-review F-1).**
Before any rule below is applied, the recorder walks the artifact line by line
holding one bit of state, the open fence:
...
| Reason | Condition |
|---|---|
| `MISSING_FIELD` | one or more of the eleven fields (other than `verdict`) has no accepted row after forms A, A′ and B are applied, or `reviewer` / `model` / `effort` is whitespace-only |
| `AMBIGUOUS_FIELD` | a field (other than `verdict`) has two or more accepted rows carrying different normalized values |
| `MISSING_VERDICT` | **(r4)** no verdict carrier outside fences and blockquotes — no verdict line and no 2-cell `verdict` row |
...
five aborted with descriptive prose only (r3-review F-3). All ten tokens above
now appear literally in the abort text, so triage across an 18-batch, 447-review
program is mechanical. That is deliberate: a
