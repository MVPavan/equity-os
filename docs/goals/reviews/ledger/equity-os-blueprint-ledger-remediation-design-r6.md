# Ledger authority and remediation design r6

Next action: obtain one clean independent exact-byte **Reviewer** review of this
file at the predetermined path
`docs/goals/reviews/ledger/equity-os-blueprint-ledger-remediation-design-r6-review-r0.md`.
...
the goal amendment set that approval covered, and r6 changes the goal amendment
set again. That approval remains void for every purpose, must not be reused,
quoted as current authority, or embedded in any candidate goal record, and
`scratchpad/hr0004/approval-evidence.md` becomes historical context only. **No
r5-bound approval was ever asked for or given**: r5 was blocked at independent
review, so no §5.2 question bound to r5 was ever presented. A fresh §5.2 answer
bound to r6 is required.

## 1. Status, defects corrected, and authority boundary

This document starts from the exact r5 design whose SHA-256 is
`faa7bc26987f0cf74129efb243ffe2ff227da5f451688e73fbe549c06c64d19a` and
supersedes its operational design. It preserves every mechanically verified r5
non-finding and corrects exactly the findings in the independent r5 review at
`scratchpad/hr0004/review/r5-review-findings.md` (SHA-256
`2bac0bb9538abf040c095308a034101814ef2ed8e47d74dd5ab75b877c8bdec8`), whose
verdict line reads `Verdict: BLOCKED — 0 Critical, 1 Important` and which
records one Important and five Minor findings. r5 in turn started from r4
...
terminology requirements; §1.1 preserves those dispositions and §1.2 disposes of
the r5 findings. It does not approve any earlier revision, amend the active
goal, or authorize a migration.

1. **Approval binds the exact independent review by digest.** The literal
   question binds this r6 path and SHA-256, the predetermined review path and
   SHA-256, the review's `CLEAN` verdict, its reviewed-input SHA equal to the r6
...
4. **The canonical structural invariant is not relaxed for convenience.** r6
   ...
   `REJECTED_ACCOUNTED`. That rule survives r6 unchanged in meaning
   (§3.5, §3.9).

The protections carried forward into r6 remain binding: the canonical goal,
...
r6 states the requirement and the migrator must satisfy it; "design" means r6
changes what the migrator must build.

| ID | Disposition | r6 location |
|---|---|---|
...
| M-2 | **Design closure.** `controlled_state.scope_definition` stays the exact three-key projection permanently; the kind-specific keys are digest-bound through the review projections, which r6 makes a required validator rule. | §3.2, §3.10 |
...
here. No other change was made to r5's operational design.
...
must retain. Every value below was freshly recomputed while authoring r6 and
equals both the r4 and the r5 §2 value; the goal, ledger, and human-review
...
gate. r6 therefore adopts resolution (b), forced by the (a)-side grounding:
...
creates a permanent trap. r6 closes the vocabulary. The post-state ledger must
...
additions are r6-authorized because the pre-state `NAMED_OWNER_COMMITMENT`
...
rows, which §1 forbids. r6 therefore chooses **schema-level redefinition now,
...
§7.3 B, in three parts: B.1 explicit-token passages, B.2 the ten bare-`Sol`
...
### 3.9 What r6 explicitly refuses to authorize
...
The r5 and r6 changes were both checked against the affected-row set and neither
moves it. The C-1 resolution touches `PG-2-04` and `REG-D-01`, both already in
scope; the §3.7 vocabulary touches only rows §3.6 already placed in scope; and
the §3.8 terminology reconciliation changes zero ledger rows. r6 adds no ledger
row to the set either: its §7.3 B.2/B.3 and D.2 corrections are goal-prose and
extractor changes that touch no row, and its §3.5.1 pinned `scope` literal
constrains a requirement on `PG-2-04`, which is already in scope. The scope is
therefore unchanged from r4 and was re-derived from source rather than copied.
...
r6, is `bf6fee00d0f4510316b42b50ec13f74148df9ed44e472f2ad8be114ee3add894`.
...
1. the exact r6 path and its freshly computed full SHA-256, substituted
   externally for every `<R6_SHA256>` in the literal question without editing
   this file;
...
3. the predetermined independent review path
   ...
   `<R6_REVIEW_SHA256>`, reviewer/session identity, UTC timestamp, and its exact
   binding values: reviewed r6 path, reviewed-input SHA equal to `<R6_SHA256>`,
...
exactly two substitutions — `<R6_SHA256>` and `<R6_REVIEW_SHA256>` — and **zero
...
those review bytes are final. This r6 file retains placeholders, so neither the
...
conversation/goal-tool identifier, the r6 path/SHA, review
...
review **by digest** — review path plus `<R6_REVIEW_SHA256>` — together with its
...
HR-0004 entry and resolution use a `UTF8_LINE_SPAN` evidence object that points to that exact
...
Before any canonical write, structural reconciliation reads the fixed r6 and
...
It also requires the reviewed-input SHA to equal the actual r6 SHA. A missing,
...
review artifacts including this r6 file and its predetermined independent
...
3. Read and retain every preimage byte string in memory and in a private
   ...
   pre-state hashes, the full r6 digest, predetermined clean-review
   ...
   r6/review bindings must also pass the independent structural reconciliation
...
1. **Recheck authority.** Verify every §2 prehash, the three script pre-state
   hashes, the exact r6 and predetermined review bindings, completed approval
...
**B.1 — passages containing an explicit `xhigh` or `gpt-5.6-sol` token** (the r5
list, unchanged): lines 26–43; line 116; line 251; line 386; lines 423 and 439;
line 516; line 654; lines 752–786; lines 792–824 including the role table and
the CLI invocation block; line 829; line 858; lines 1158–1173; and line 4089.
...
Each patch is exact-match and fail-loud like every other item in §7.3. Lines
519–520 and 4076 wrap in the pre-state, so the amendment covers the wrapped
sentence; reflowing is permitted only where the substitution forces it, and no
other word on those lines may change.
...
`xhigh`, `gpt-5.6`, `Codex`, `Agent Matrix`, matched as case-sensitive
word-boundary occurrences — and asserts that the candidate goal contains **zero**
occurrences of any of them outside two exempt regions:

1. the preserved §"Activation record" span (§7.3 C), located by its exact
   heading line `## Activation record` through end of file; and
...
**Verification performed while authoring r6.** Running D.2's exact enumeration
over the pre-state goal yields 74 matching lines. Subtracting the B.1 line set,
the B.2 line set, the B.3 assert lines, the §"Activation record" span, and the
three program spans leaves **zero** residual lines. The five in-scope
Activation-record occurrences (4114, 4115, 4116, 4132, 4135) are all inside the
§7.3 C preserved span, and the four in-program occurrences are exactly
1486/1487/3246/3247. The amendment list is therefore complete under its own
mechanical closure — the check r5 lacked.
...
exception of §7.2. In reconciliation mode it also owns the exact r6/review
...
- canonical posthashes equal the prepared candidate hashes and differ from a
  ...
  `CONTEXT.md` is byte-unchanged; the actual r6 and review paths, byte hashes,
...
- HR-0004 has exactly one active `RECONCILE_AUTHORITY` resolution, exact
  144-ID structured scope, exact approval/review/r6/prehash evidence, and valid
...
- the §7.3 D.2 lane-token exclusion check passes against the post-state goal.
  ...
  holds and is asserted here: zero word-boundary occurrences of `Sol`, `Terra`,
  `Luna`, `xhigh`, `gpt-5.6`, `Codex`, or `Agent Matrix` anywhere in the
  post-state goal outside the preserved §"Activation record" span, whose five
  occurrences are byte-unchanged; and every B.1, B.2, and B.3 patch applied
  exactly once;
...
- transaction ID, timestamps, lock identity, **the complete final journal
  ...
  **rollback result including per-path byte and mode proof**, the rollback
  ...
  re-derivations;
- r6 path/SHA, predetermined review path/SHA, parsed `CLEAN` verdict, reviewed-
...
- every candidate and canonical validator argv, exit code, stdout/stderr
  ...
  lane-token-exclusion results, external-tool precondition result, `git diff --check`, `git status --porcelain`, and final
  `git status --short --branch`.
...
1. Commission one clean, independent, exact-byte `REVIEWER`-role review of this
   r6 file at the predetermined r6-review-r0 path. The reviewer must not be the
   agent or context that authored r6. Any finding or binding mismatch stops the
   lane and requires a new reviewed design revision.
2. After `CLEAN`, compute the r6, review, and scope digests; externally fill only
...
exact r6 SHA and a fresh current-user approval explicitly bound to that review
and the completed §5 question. The approval recorded against r4 is void and
cannot be substituted. A failed precondition, changed r6 or review byte,
