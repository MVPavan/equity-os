# Inventory-review recording design — r5
...
### 0.0 r4 → r5 — why this round exists (bead `eqos-w78`, round 4)
...
in §0.0a, **a round is never amended in place**; this correction is a new round
...
### 0.0a r3 → r4 — why r4 existed (bead `eqos-w78`, round 3)
...
### 0.0b r2 → r3 — why r3 existed (bead `eqos-w78`)
...
### 0.0c Round lineage below this point
...
quote the superseded round freely: quoted text inside a fenced block, a
blockquote, an HTML comment, or a block indented by four or more spaces (or a
tab) is not scanned (§2.2.1 — r5 widened this from fences and blockquotes alone,
per r4-review M-1). A fenced block or an HTML comment that is never closed is a
hard skip, not a licence to stop reading the artifact: see `UNTERMINATED_FENCE`
in §2.2.1.
...
*(New in r3, per §0.0b; corrected in r4 per §0.0a and in r5 per §0.0. This subsection is the
...
widened in r5, r4-review I-1 and M-1).** Before any rule below is applied, the
recorder walks the artifact line by line holding two bits of state, the open
fence and the open HTML comment:
...
- **(r5)** With no fence open, a line containing `<!--` **opens** an HTML
  comment unless the same line also contains a later `-->`; the line itself is
  never scanned either way. Every line while a comment is open is **not
  scanned**, including a fence delimiter — a comment, once open, swallows
  everything up to and including the first line containing `-->`. The two states
  are therefore mutually exclusive: the fence machine runs first and a comment
  can only open when no fence is open.
- A line matching `^[ \t]*>` — a Markdown blockquote — is **not scanned**,
  whether or not a fence is open.
- **(r5)** A line matching `^(?: {4,}|\t)` — four or more leading spaces or a
  leading tab, i.e. a Markdown indented code block — is **not scanned**. This
  filter is applied **after** the fence machine, so an indented fence delimiter
  still toggles fence state exactly as in r4 and no r4-scanned line becomes
  unscanned by way of a perturbed fence state.
...
**r5's two additions were adopted on measurement, not on taste.** The choice
turned on whether widening the guard could change any current parse, so the
corpus was measured **before** implementing
(`scratchpad/w78/r5/measure_m1.py`): of the 444 artifacts, **0** contain `<!--`
anywhere, **0** carry an accepted identity row that is indented, **0** carry a
scanned verdict line that is indented, and **0** contain an indented fence
delimiter. All four figures being zero is what licenses both rules; had any
accepted row or verdict line been indented, the count would be reported here and
the stated rule would instead have been that indented lines **are** scanned.
...
to r4's over all 444 (`scratchpad/w78/r5/regress_r4_r5.py`).
...
line. Lines inside fenced blocks or blockquotes are not scanned and are
therefore not verdict lines, however they are written.
...
The rule is keyed on **distinct values**, not on the number of lines, and §0.0
...
| `MISSING_VERDICT` | **(r4)** no verdict carrier among the scanned lines — no verdict line and no 2-cell `verdict` row outside fences, blockquotes, HTML comments and indented blocks |
| `AMBIGUOUS_VERDICT` | **(r4)** two or more verdict carriers stating different verdicts |
...
five aborted with descriptive prose only (r3-review F-3). All fourteen tokens
above now appear literally in the abort text, so triage across an 18-batch,
447-review program is mechanical. That is deliberate: a
...
prove it — that is the injected condition), and the temp and preimage files are
*required to survive*, because step 10 preserves them for the operator. The
`temp_files_surviving == 0` requirement therefore applies to L1–L4 only.
...
```json
{
  ...
  "replica_ledger_prestate_sha256": "de236d7e…",
...
7. **Rehearsal ledger pre-state — the replica's own starting bytes, never the
   ...
   `de236d7e8dcf02e307ec58797f5722ae2f85d1d8cdba57d4ecc07df5383c9c97`, which is
...
**Status (rewritten in r5; r4-review I-2 — r4 still said the recorder did not
exist and the rehearsal had not been performed, and both were false of the r4
package itself).** As of r5, all of the following are true and were measured this
round:

| Fact | Value |
|---|---|
| Recorder | `scripts/equity_os_blueprint/record_inventory_review.py` exists, SHA-256 `8d0410cd1803c82b276d801da30d47f877778c08eb10bb5db7f4c618b6fb3032` (untracked; the r4 bytes were `32947a02…`) |
| Rehearsal | **performed**, this round, on five fresh full-tree replicas — **5/5 legs pass** (`scratchpad/w78/r5/rehearsal-r5.log`) |
| Proof object | `scratchpad/inventory-reviews/rehearsal/proof.json`, SHA-256 `b075280cc2dd70e553fadeeee87bffed5cb0e592179239bd9bc4d1959c7d771f`, schema `inventory-review-rollback-rehearsal/v2` |
| Recorder binding | the proof's `recorder_sha256` is `8d0410cd…` — **equal to the live recorder's bytes**, i.e. the proof is bound to the tool that would perform the write |
| Transcript | `scratchpad/inventory-reviews/rehearsal/transcript.txt`, SHA-256 `e995f8191e26a611178822733992866a7ab6174d00c4141801b353471101648f`, equal to the proof's `transcript_sha256` |
| §3.8 executed | twice this round under `--dry-run`, both exit **0**: `batch-01-authority_clause` (12 reviews, pending 447 → 435, `stale_after` 0, `structural_candidate_exit` 0, `candidate_sha256` `b168928031153a438bc71fd8eff5b859bd279031faff7b7e0e11c19b3aa15437`) and `batch-doc-document_strategy_clause` (18 reviews, 447 → 429, `candidate_sha256` `b85b8efe71f407b007bdd7a0c187c8e90844602472b8cb8e1880ac557c29f21c`); `committed: false` for both, canonical ledger `de236d7e…` unchanged |

The gate demonstrably works in the direction that matters: the first `--dry-run`
after this round's recorder edits **refused** with `§3.10 assert 5: the recorder
changed since the rehearsal (rehearsed 32947a02…, running 8d0410cd…); a fresh
rehearsal is required`, and only a re-run rehearsal cleared it. Any further edit
to the recorder or to the §1.1 pinned bytes invalidates this proof again, by
design.
...
Probes A–H were executed in the r1 round against the §1.1 bytes and are carried
into r2 unchanged; the r2 round re-verified all seven §1.1 hashes and the three
...
9. **Scale of the untested surface.** Probes A–H cover the digest, ordering,
   ...
   **Rewritten in r5 (r4-review I-2).** r0–r4 closed this item by saying the
   ...
   - The recorder exists at SHA-256 `8d0410cd…`. Manifest parsing, the lock, the
     journal, the rollback path, the `SIGINT`/`SIGTERM` guards, the
     `RECOVERY_REQUIRED` path and the precondition set are implemented.
   - The five-leg rehearsal has been **performed** — 5/5 legs pass, on five
     fresh full-tree replicas — and the proof object at
     `scratchpad/inventory-reviews/rehearsal/proof.json` (`b075280c…`) binds
     `recorder_sha256 = 8d0410cd…`, the live bytes.
   - §3.8 has been executed twice under `--dry-run`, both exit 0, both leaving
     the canonical ledger at `de236d7e…`.
   ...
   **What remains genuinely untested**, stated so nothing above is credited with
   ...
   design — it is no longer an *unimplemented* one.
