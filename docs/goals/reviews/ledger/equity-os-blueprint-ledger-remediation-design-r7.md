# Ledger authority and remediation design r7

Next action: obtain one clean independent exact-byte **Reviewer** review of this
file at the predetermined path
`docs/goals/reviews/ledger/equity-os-blueprint-ledger-remediation-design-r7-review-r0.md`.
...
**The prior approval is void and no later approval exists.** The only user
approval ever recorded for this lane is in
`scratchpad/hr0004/approval-evidence.md`, bound to r4 SHA-256
`c1ab125880ec1895a344b57f7aaef8d372836fa0ded9c900a1aae9284b295e00` and to the
r4 review. r5 already changed the ledger content, the validator contract, and
the goal amendment set that approval covered, and r6 and r7 changed the goal
amendment set again. That approval remains void for every purpose, must not be
reused, quoted as current authority, or embedded in any candidate goal record,
and `scratchpad/hr0004/approval-evidence.md` becomes historical context only.
**No r5-bound or r6-bound approval was ever asked for or given**: r5 and r6 were
each blocked at independent review, so no §5.2 question bound to either was ever
presented. A fresh §5.2 answer bound to r7 is required.

## 1. Status, defects corrected, and authority boundary

This document starts from the exact r6 design whose SHA-256 is
`87220b14d9a090b7a6700eeab4de7a0c4dd316056a30ce9ae7ee2c8eb84d0964` and
supersedes its operational design. It preserves every mechanically verified r6
non-finding and corrects exactly the findings in the independent r6 review at
`scratchpad/hr0004/review/r6-review-findings.md` (SHA-256
`4d6a197d51a50a4dfa58620cdc9590ec0c900ac8d81ab5f2c7c1e1af54a3705b`), whose
verdict line reads `Verdict: BLOCKED — 0 Critical, 1 Important, 2 Minor` and
which records one Important and two Minor findings. r6 in turn started from r5
(`faa7bc26987f0cf74129efb243ffe2ff227da5f451688e73fbe549c06c64d19a`) and
corrected the independent r5 review at
`scratchpad/hr0004/review/r5-review-findings.md` (SHA-256
`2bac0bb9538abf040c095308a034101814ef2ed8e47d74dd5ab75b877c8bdec8`,
`Verdict: BLOCKED — 0 Critical, 1 Important`); r5 in turn started from r4
(`c1ab125880ec1895a344b57f7aaef8d372836fa0ded9c900a1aae9284b295e00`) and
corrected the candidate review at `scratchpad/hr0004/review/review-r0.md`
(`Verdict: BLOCKED — 1 Critical, 7 Important`) plus the two user-instructed
terminology requirements; §1.1 preserves those dispositions, §1.2 preserves the
r5-finding dispositions, and §1.3 disposes of the r6 findings. It does not
approve any earlier revision, amend the active goal, or authorize a migration.

1. **Approval binds the exact independent review by digest.** The literal
   question binds this r7 path and SHA-256, the predetermined review path and
   SHA-256, the review's `CLEAN` verdict, its reviewed-input SHA equal to the r7
   SHA, and the reviewer **role** `REVIEWER` under the `CONTEXT.md` binding of
   §3.8. It does **not** carry the reviewer's model, effort, or
   `role_binding_sha256` values as literal text; pinning the review's own
   SHA-256 fixes every field inside the immutable review artifact transitively.
   Those three values are compared between the review artifact and the candidate
   goal's approval record (§5.3), not against question text. The completed
   question and response bytes are recorded after approval, and structural
   reconciliation verifies every bound review field against the immutable review
...
4. **The canonical structural invariant is not relaxed for convenience.** r7
   ...
   `REJECTED_ACCOUNTED`. That rule survives r7 unchanged in meaning
   (§3.5, §3.9).

The protections carried forward into r7 remain binding: the canonical goal,
...
r7 states the requirement and the migrator must satisfy it; "design" means r7
changes what the migrator must build.

| ID | Disposition | r7 location |
|---|---|---|
...
| M-2 | **Design closure.** `controlled_state.scope_definition` stays the exact three-key projection permanently; the kind-specific keys are digest-bound through the review projections, which r7 makes a required validator rule. | §3.2, §3.10 |
...
### 1.2 Disposition of every r5-review finding

Every finding in `scratchpad/hr0004/review/r5-review-findings.md` is disposed of
here. These dispositions were made in r6, were each independently verified
closed by the r6 review, and are carried into r7 unchanged; §1.3 disposes of the
r6 findings.

| ID | Disposition | r7 location |
|---|---|---|
| I-R5-1 | **Design change.** §7.3 B's exact amendment list gains the ten bare-`Sol` forward-looking goal lines, each with its exact role-vocabulary replacement, and §7.3 D gains a mechanical assertion that no lane token survives outside the preserved `Activation record` span and the three embedded program spans. | §7.3 B, §7.3 D |
| M-R5-1 | **Corrected.** §2 now states the invariant ("no dirty or untracked path is a transaction target") instead of an enumeration its own untracked file falsified. | §2 |
| M-R5-2 | **Corrected.** §1 item 1, §5.3, and §8.3 now say the question binds the review **by digest**, and that model, effort, and `role_binding_sha256` are compared between the review artifact and the candidate goal record. §5.2's text is unchanged in substance and still renders with exactly two substitutions. | §1, §5.2, §5.3, §8.3 |
| M-R5-3 | **Design change.** The exact `REQ-PG-2-04-COMMAND-PROOF` `scope` string is pinned as a validator-owned literal, like §3.6's proof-object reset. | §3.5.1, §3.7, §8.1, §8.3 |
| M-R5-4 | **Corrected.** §2's inventory-review enumeration is now exhaustive: 107 `semantic_review` + 167 `evidence_inventory_review` + 167 `approval_inventory_review` = 441, all `PENDING`. | §2 |
| M-R5-5 | **Corrected.** §7.3 B now names the four assert lines 1486/1487 and 3246/3247 (the `model` and `effort` pairs) instead of "two embedded validator literals". | §7.3 B |

### 1.3 Disposition of every r6-review finding

Every finding in `scratchpad/hr0004/review/r6-review-findings.md` is disposed of
here. No other change was made to r6's operational design. Apart from
supersession, path, placeholder, and lineage plumbing — the header paragraph,
§1's opening, §1.2's intro, §2's "equals the r4, r5, and r6 §2 values", and §4's
scope-invariance restatement, none of which changes an operational requirement —
the §1.3 rows below are the whole r6→r7 delta.

| ID | Disposition | r7 location |
|---|---|---|
| I-R6-1 | **Design change, both coupled halves.** (a) §7.3 B.1's anchor is extended from 792–824 to **791–824**, so the paragraph's first sentence — the forward-looking `codex exec` subagent mandate — is rewritten with the rest of its paragraph instead of being left standing as a live vendor-tool mandate plus a dangling fragment; B.1 now also fixes the exact replacement wording for the merged 791–796 paragraph. (b) §7.3 D.2's lane-token matching is **case-insensitive**, §8.3's postcondition states the same qualifier, and both carry one explicit, closed exemption for `.`-prefixed repository path literals, whose only current occurrences are enumerated. §3.8's one-line pointer to the §7.3 B parts is restated to match B.1's widened scope. | §3.8, §7.3 B.1, §7.3 D.2, §8.3 |
| M-R6-1 | **Corrected.** §7.3 B.2's wrapping note now names all seven wrapping entries (345, 441, 512, 519–520, 632, 981, 4076) and states the patch unit as the wrapped sentence, not the physical line, so a strict exact-match implementer cannot stall on an unnamed case. | §7.3 B.2 |
| M-R6-2 | **Corrected.** The two over-long lines in §5.3 and §8.4 are reflowed to the document's wrap width. No wording changed. | §5.3, §8.4 |
...
must retain. Every value below was freshly recomputed while authoring r7 and
equals the r4, r5, and r6 §2 values; the goal, ledger, and human-review
artifacts are therefore still byte-identical to the r4 baseline and no baseline
moved:
...
- every content-bound inventory review is `PENDING`, and the enumeration is
  exhaustive: 107 `semantic_review` objects (one per non-register canonical
  `scope_derivation`), 167 `evidence_inventory_review` objects, and 167
  `approval_inventory_review` objects — 441 review objects in total, all
  `PENDING`, none `COMPLETE`, and all carrying exactly the same pre-state key
  set; every one of the 210 `verification_command` policies is `UNRESOLVED` with
  `not_applicable_review=null`, so no `NOT_APPLICABLE` verification review
  exists; and
...
untouched. The invariant, not an enumeration, is what this design asserts: **no
dirty or untracked path is a transaction target**, and none of the six §6.1
targets is dirty at authoring time. This document deliberately does not list the
dirty set, because that set grows as this design and its predetermined review
are written; §6.2 step 3 captures the real baseline fresh under the lock and
enforces it, so an implementer must never treat any list here as the expected
baseline.
...
gate. r7 therefore adopts resolution (b), forced by the (a)-side grounding:
...
- the observable conjunction r4 §3.5 specified for `PG-2-04` becomes the exact
  ...
  state. **The conjunction's content is pinned, not left to prose.** Because
  §3.7 otherwise constrains a new `scope` string only to be nonempty and
  lane-free, structural validation additionally owns this exact literal — the
  fully expanded conjunction, in the fixed operation order `correction`,
  `deletion`, `backup`, `export` and the fixed per-operation field order
  `_test_result_id`, `_cases_executed`, `_failure_count` — and rejects any
  other value, exactly as §3.6 pins the `DISP-R-1` proof-object reset:

```json
{"scope":"PG-2-04 command proof: correction_test_result_id != \"\" and correction_cases_executed > 0 and correction_failure_count == 0 and deletion_test_result_id != \"\" and deletion_cases_executed > 0 and deletion_failure_count == 0 and backup_test_result_id != \"\" and backup_cases_executed > 0 and backup_failure_count == 0 and export_test_result_id != \"\" and export_cases_executed > 0 and export_failure_count == 0"}
```

  The literal contains no vendor model, lane, or tool name, so it satisfies
  §3.7's string rule. It is a scope string, not a predicate expression: the
  canonical predicate grammar of §3.2 bullet 7 is not extended, and nothing in
  this transaction evaluates it. Pinning it means a later agent cannot silently
  weaken the conjunction that the removed activation predicate used to carry;
...
creates a permanent trap. r7 closes the vocabulary. The post-state ledger must
...
additions are r7-authorized because the pre-state `NAMED_OWNER_COMMITMENT`
...
- the manifest is ID-only with exactly one pinned-content exception:
  `REQ-PG-2-04-COMMAND-PROOF.scope` must equal the exact literal fixed in
  §3.5.1, byte-for-byte. Every other new `scope` and `description` string is
  checked for the nonempty and lane-free properties above only.
...
rows, which §1 forbids. r7 therefore chooses **schema-level redefinition now,
...
§7.3 B, in three parts: B.1 explicit-token passages plus the paragraph line
their §"Agent routing" passage continues from, B.2 the ten bare-`Sol`
passages, and B.3 the four embedded validator asserts). The historical passages
listed above are preserved verbatim. The claim that the amended goal never
restates a model binding is not left to the completeness of that list: §7.3 D.2
asserts it mechanically over the whole file.

### 3.9 What r7 explicitly refuses to authorize
...
The r5, r6, and r7 changes were each checked against the affected-row set and
none moves it. The C-1 resolution touches `PG-2-04` and `REG-D-01`, both already
in scope; the §3.7 vocabulary touches only rows §3.6 already placed in scope;
and the §3.8 terminology reconciliation changes zero ledger rows. r6 added no
ledger row to the set either: its §7.3 B.2/B.3 and D.2 corrections are
goal-prose and extractor changes that touch no row, and its §3.5.1 pinned
`scope` literal constrains a requirement on `PG-2-04`, which is already in
scope. r7 adds none either: its §7.3 B.1 anchor extension and its D.2/§8.3
case-insensitivity and path-literal exemption are likewise goal-prose and
extractor changes. The scope is therefore unchanged from r4 and was re-derived
...
r7, is `bf6fee00d0f4510316b42b50ec13f74148df9ed44e472f2ad8be114ee3add894`.
...
1. the exact r7 path and its freshly computed full SHA-256, substituted
   externally for every `<R7_SHA256>` in the literal question without editing
   this file;
...
3. the predetermined independent review path
   ...
   `<R7_REVIEW_SHA256>`, reviewer/session identity, UTC timestamp, and its exact
   binding values: reviewed r7 path, reviewed-input SHA equal to `<R7_SHA256>`,
...
exactly two substitutions — `<R7_SHA256>` and `<R7_REVIEW_SHA256>` — and **zero
...
those review bytes are final. This r7 file retains placeholders, so neither the
...
conversation/goal-tool identifier, the r7 path/SHA, review
path/SHA/verdict/reviewed-input SHA/role/model/effort/binding digest, scope
digest, and all five pre-state hashes. The completed question bytes bind the
review **by digest** — review path plus `<R7_REVIEW_SHA256>` — together with its
verdict, reviewed-input SHA, and role; the reviewer's model, effort, and
`role_binding_sha256` are recorded in the goal record and in the review artifact
but are deliberately **not** carried as literal question text, because §5.2 must
render with exactly two substitutions and could never gain them later. Pinning
the review digest fixes those values transitively, so the stored
question/response pair still proves which exact review the user approved. The
HR-0004 entry and resolution use a `UTF8_LINE_SPAN` evidence object that points
to that exact post-transaction goal span and hashes it under the existing
evidence rules.
...
Before any canonical write, structural reconciliation reads the fixed r7 and
review paths, hashes their exact bytes, parses the review's unique binding, and
compares, field by field, against the two records that carry each field:

- design SHA, review SHA, verdict, reviewed-input SHA, and reviewer role are
  compared against **both** the completed question bytes and the candidate goal
  record, because the question carries all five;
- reviewer model, reviewer effort, and `role_binding_sha256` are compared
  between the **review artifact and the candidate goal record only**, because
  §5.2 does not carry them. Requiring them in the question text would be
  unsatisfiable and is forbidden.

It also requires the reviewed-input SHA to equal the actual r7 SHA. A missing,
ambiguous, changed, substituted, non-`CLEAN`, wrong-input, or wrong-role review
fails before replacement; matching agent-authored fields in the candidate do
not suffice.
...
review artifacts including this r7 file and its predetermined independent
...
3. Read and retain every preimage byte string in memory and in a private
   ...
   pre-state hashes, the full r7 digest, predetermined clean-review
   ...
   r7/review bindings must also pass the independent structural reconciliation
...
1. **Recheck authority.** Verify every §2 prehash, the three script pre-state
   hashes, the exact r7 and predetermined review bindings, completed approval
...
**B. Role terminology (user instruction and findings M-12, I-R5-1).** Rewrite
these forward-looking passages into `ORCHESTRATOR`/`IMPLEMENTER`/`REVIEWER`
vocabulary with a single pointer to `CONTEXT.md` "Agent roles (harness-wide)" as
the binding table, restating no model binding in the goal.

**B.1 — passages containing an explicit `xhigh` or `gpt-5.6-sol` token, plus the
paragraph line those passages continue from** (the r5 list with one anchor
extended; finding I-R6-1): lines 26–43; line 116; line 251; line 386; lines 423
and 439; line 516; line 654; lines 752–786; **lines 791–824** — the entire body
of §"Agent routing and delegated authority", whose heading is line 789 —
including the 791–796 paragraph, the role table, and the CLI invocation block;
line 829; line 858; lines 1158–1173; and line 4089.

**The 791–824 anchor is 791, not 792 (finding I-R6-1).** The r5/r6 list started
this passage at 792 because 792 is the first line carrying an explicit `xhigh`
or `gpt-5.6-sol`-family token under a case-sensitive scan. Line 791 carries the
paragraph's first sentence — the forward-looking mandate that every subagent is
invoked through an explicit lowercase `` `codex exec` `` — and the first half of
its second sentence, which wraps into 792. Rewriting 792–824 alone would leave
791 standing as a live vendor-tool invocation mandate followed by a dangling
sentence fragment, immediately above a paragraph rewritten into role vocabulary,
and with the CLI invocation block that mandate refers to already deleted by the
same patch. The paragraph is therefore patched as one unit.

**Exact replacement for the merged 791–796 paragraph.** The whole pre-state
paragraph — the `` `codex exec` `` mandate, the host-native/matrix prohibition
that wraps 791–792, and the model/effort sentences at 792–796 — is replaced,
in full and as a single unit, by exactly:
...
This is the same rule the rest of B.1 applies — role vocabulary, one pointer to
`CONTEXT.md` "Agent roles (harness-wide)", no model binding restated in the goal
— written out here because the region merges two sentences across a wrap and an
implementer must not have to infer where the replacement starts and stops. The
replacement text contains no D.2 lane token, so the amended paragraph passes the
D.2 check on its own bytes.

**B.2 — passages spelling the lane as bare `Sol` (new in r6; finding I-R5-1).**
The r5 list was built by searching for `xhigh`/`gpt-5.6-sol` and therefore
missed these ten forward-looking lines. Each was re-verified against the
pre-state goal at SHA-256
`dabad7bfe3d2765a5ac9687376029d8587b4b2ac95bc03494155a974e5ddc67f`; none lies
inside an embedded program span (1226–3326, 3333–3414, 3422–4012) or inside
§"Activation record" (line 4101 through the end of the file, 4146 lines).

**The replacement text is fixed and identical in all ten cases.** The single
word `Sol` is replaced by the fifteen-character sequence consisting of a
backtick, the eight letters `REVIEWER`, a backtick, and the five characters
`-role`. Nothing else on the line changes: the surrounding article, adjectives,
and following noun are preserved exactly as they stand. Quoted fragments below
are pre-state text with the replaced word marked, not code spans, so their own
backticks are literal.

| # | Line | Pre-state fragment (the replaced word is **Sol**) | Result |
|---|---:|---|---|
| B2-1 | 123 | "A **Sol** review cannot grant that product authority." | "A `REVIEWER`-role review cannot grant that product authority." |
| B2-2 | 185 | "…records whether a fresh **Sol** review has checked the component's source clauses for omitted approval types." | "…a fresh `REVIEWER`-role review has checked…" |
| B2-3 | 345 | "…but its **Sol** scope review, rationale, and current no-implementation proof remain mandatory." | "…but its `REVIEWER`-role scope review…" |
| B2-4 | 441 | "It is valid only when a fresh evidenced **Sol** review confirms why no mechanical command can prove the component." | "…a fresh evidenced `REVIEWER`-role review confirms…" |
| B2-5 | 497 | "…null human-resolution fields and carries the persisted clean **Sol** review." | "…the persisted clean `REVIEWER`-role review." |
| B2-6 | 512 | "Ordinary **Sol** evidence/inventory review remains automated review;…" | "Ordinary `REVIEWER`-role evidence/inventory review remains…" |
| B2-7 | 519–520 | "Neither this completeness review nor a **Sol** approval grants any non-delegated authority." | "…nor a `REVIEWER`-role approval grants…" |
| B2-8 | 632 | "The status-only **Sol** review and refreshed content-bound reviews must then pass…" | "The status-only `REVIEWER`-role review and refreshed…" |
| B2-9 | 981 | "Ordinary automated evidence or **Sol** review never appears in `resolutions`…" | "…or `REVIEWER`-role review never appears in `resolutions`…" |
| B2-10 | 4076 | "…or a current evidenced **Sol** `NOT_APPLICABLE` review;" | "…a current evidenced `REVIEWER`-role `NOT_APPLICABLE` review;" |

Each patch is exact-match and fail-loud like every other item in §7.3. **The
patch unit is the sentence, not the physical line.** Seven of the ten entries
wrap in the pre-state — lines 345, 441, 512, 519–520, 632, 981, and 4076, each
of whose quoted sentence continues onto the following line — so for those the
matched and replaced region spans both lines, and a single-line exact-match
patch is the wrong unit. The remaining three entries (123, 185, 497) end on
their own line. Reflowing is permitted only where the substitution forces it,
and no other word in the matched sentence may change.

Four of these ten lines — 185, 441, 512, and 519 — describe the very review
schemas §3.2 bullet 12 makes role-based, so leaving them would have shipped a
canonical goal whose prose contradicts its own embedded validators.

**B.3 — embedded validator literals.** Four assert lines, not two: lines 1486
(`assert review["model"] == "gpt-5.6-sol"`) and 1487
(`assert review["effort"] == "xhigh"`), and lines 3246
(`assert review["model"] == "gpt-5.6-sol"`) and 3247
(`assert review["effort"] == "xhigh" and review["verdict"] == "CLEAN"`). All
four lie inside the single embedded structural program span (1226–3326); the
preimplementation (3333–3414) and terminal (3422–4012) programs contain no lane
token at all. The model and effort assertions are replaced by the §3.8 role rule
— `role == "REVIEWER"` plus shape-only nonempty `model`/`effort` — and line
3247's `verdict == "CLEAN"` conjunct is retained unchanged.
...
**D. Mechanical anti-drift checks.** `extract_goal_validators.py` already reads
the goal, so both closures below live there and both exit nonzero on failure.

**D.1 — required-marker check (finding I-1).** A closed list of exact required
marker substrings — one per item A1–A11 — each of which must appear at least
once in the goal outside the three embedded program spans. This makes a future
prose/validator divergence fail loudly instead of silently, which is the defect
class I-1 identified.

**D.2 — lane-token exclusion check (finding I-R5-1).** D.1 is a presence check
and therefore structurally incapable of catching an *omitted* rewrite, which is
exactly how the ten B.2 lines survived r5. The extractor
therefore also gains a closed lane-token list — `Sol`, `Terra`, `Luna`,
`xhigh`, `gpt-5.6`, `Codex`, `Agent Matrix`, matched as **case-insensitive**
word-boundary occurrences — and asserts that the candidate goal contains **zero**
occurrences of any of them outside three exempt regions:

1. the preserved §"Activation record" span (§7.3 C), located by its exact
   heading line `## Activation record` through end of file;
2. the three embedded program spans, located by their fence lines, not by
   hard-coded line numbers; and
3. **path-literal contexts only**: a match whose first character is immediately
   preceded by `.` and whose last character is immediately followed by `/`, i.e.
   a token appearing as the leading segment of a literal repository path such as
   `.codex/…`. Nothing else about the surrounding text is consulted, and no
   other lowercase or mixed-case context is exempt.

**Why the matching is case-insensitive, and why exemption 3 is required
(finding I-R6-1).** A case-sensitive check is defeated by a case variant, which
is the same defect class as I-R5-1's spelling variant one level down: the
pre-state goal's line 791 mandate is spelled lowercase `` `codex exec` ``, so a
case-sensitive scan reports zero on a post-state goal that still contains it.
Making the match case-insensitive closes that hole but newly matches three real,
resolvable repository path references — `.codex/project/invariants.md` on goal
line 71, and `.codex/project/verification.md` twice on goal line 4018 (once in
the link text, once in the link target); both files were verified to exist under
`.codex/project/`. Those are directory names on disk, not lane names, and they
are not rewritten by any §7.3 item. Exemption 3 is stated closed and mechanical
rather than left to the implementer, because a case-insensitive D.2 without it
turns a passing build into a failing one at `--execute` time, and an open-ended
"paths and code spans are fine" exemption would re-admit line 791.

Any occurrence elsewhere exits nonzero and names the offending line. This turns
the defect class from a silent omission into a build failure, and it is the
reason B.1/B.2 completeness no longer depends on an author's search string
being well chosen. Line numbers are never hard-coded in the check: the pre-state
numbers in B.1, B.2, B.3, and C are authoring anchors for exact-match patches,
while the check itself is positional only through the two region delimiters
above, so it keeps working after the amendments shift the file.

Two consequences are stated so a later agent does not misread the exemption.
First, the exemption of the program spans is a scoping decision, not permission:
after B.3 replaces the four assert lines, the three embedded programs in fact
contain no lane token either, and D.2's exemption exists only so the check
cannot be defeated by a future validator that legitimately needs to quote a
historical `invoked_model` value. Second, `invoked_model` values inside the
*ledger* are untouched historical evidence (§3.8) and are not in this check's
scope, which covers the goal file only.

**Verification performed while authoring r7.** D.2's exact enumeration was
re-run over the pre-state goal at SHA-256
`dabad7bfe3d2765a5ac9687376029d8587b4b2ac95bc03494155a974e5ddc67f` (4146 lines),
case-insensitively, over the closed token list above:

- **77 matching lines** case-insensitively. The case-sensitive scan r6 ran
  matches **74** of them; the three lines only the case-insensitive scan reaches
  are exactly **71, 791, and 4018** — the same three the r6 review identified.
- Subtracting the B.1 line set (now including 791–824), the B.2 line set, the
  B.3 assert lines, the §"Activation record" span (heading
  `## Activation record` at line 4101 through line 4146), and the three program
  spans (1226–3326, 3333–3414, 3422–4012, located from the fence lines
  1226/3326, 3333/3414, 3422/4012) leaves a residual of exactly **two lines: 71
  and 4018**, carrying three matches in total.
- Every one of those three residual matches satisfies exemption 3 — each is a
  `codex` immediately preceded by `.` and immediately followed by `/`, inside
  `.codex/project/invariants.md` (line 71) and `.codex/project/verification.md`
  (line 4018, twice). **Zero** residual matches fall outside the exemption.
- The five in-scope Activation-record occurrences (4114, 4115, 4116, 4132, 4135)
  are all inside the §7.3 C preserved span, and the four in-program occurrences
  are exactly 1486/1487/3246/3247, unchanged from r6.

The amendment list is therefore complete under its own mechanical closure, now
including the case variant that closure previously missed — and the residual is
exactly the exempted path references plus the deliberately preserved spans,
nothing else.
...
- Extractor: optional `--goal-path`, `--structural-output`,
  ...
  checked without a canonical write. It also runs both §7.3 D checks — the
  D.1 required-marker check and the D.2 lane-token exclusion check — and exits
  nonzero on either failure.
...
§3.8 role vocabulary; the exact pinned `REQ-PG-2-04-COMMAND-PROOF.scope`
literal of §3.5.1; and the single-row `PG-2-04` `program_disposition`
exception of §7.2. In reconciliation mode it also owns the exact r7/review
...
- canonical posthashes equal the prepared candidate hashes and differ from a
  ...
  `CONTEXT.md` is byte-unchanged; the actual r7 and review paths, byte hashes,
  `CLEAN` verdict, reviewed-input SHA, and reviewer role equal every value bound
  in the completed question, the response record, and the structural
  reconciliation result; the reviewer model, effort, and role-binding digest
  equal the values recorded in the review artifact and in the candidate goal
  record — §5.2 binds the review **by digest** and does not carry those three
  values as literal text, so no postcondition may require them in the question
  bytes (§5.3);
...
- HR-0004 has exactly one active `RECONCILE_AUTHORITY` resolution, exact
  144-ID structured scope, exact approval/review/r7/prehash evidence, and valid
...
- the §7.3 D.2 lane-token exclusion check passes against the post-state goal.
  Because B.3 also removes the four in-program asserts, the stronger property
  holds and is asserted here: zero **case-insensitive** word-boundary
  occurrences of `Sol`, `Terra`, `Luna`, `xhigh`, `gpt-5.6`, `Codex`, or
  `Agent Matrix` anywhere in the post-state goal outside the preserved
  §"Activation record" span, whose five occurrences are byte-unchanged, and
  outside D.2's exemption 3 for path literals, whose occurrences are exactly the
  three `.codex/…` repository-path references on pre-state lines 71 and 4018 and
  which must remain byte-unchanged. Both qualifiers — case-insensitivity and the
  closed path-literal exemption — are stated here and in §7.3 D.2 in the same
  terms, so this postcondition is the property D.2 actually checks and not a
  stronger claim the check cannot support; and every B.1, B.2, and B.3 patch
  applied exactly once, including B.1's 791–824 anchor;
- `REQ-PG-2-04-COMMAND-PROOF` exists on `PG-2-04`, is `UNRESOLVED` with empty
  `evidence_ref_ids`, and its `scope` equals the exact §3.5.1 literal
  byte-for-byte;
...
- transaction ID, timestamps, lock identity, **the complete final journal
  ...
  **rollback result including per-path byte and mode proof**, the rollback
  ...
  re-derivations;
- r7 path/SHA, predetermined review path/SHA, parsed `CLEAN` verdict, reviewed-
...
- every candidate and canonical validator argv, exit code, stdout/stderr
  digest, extractor byte-equality result, §7.3 D.1 marker-check and D.2
  lane-token-exclusion results, external-tool precondition result,
  `git diff --check`, `git status --porcelain`, and final
  `git status --short --branch`.
...
1. Commission one clean, independent, exact-byte `REVIEWER`-role review of this
   r7 file at the predetermined r7-review-r0 path. The reviewer must not be the
   agent or context that authored r7. Any finding or binding mismatch stops the
   lane and requires a new reviewed design revision.
2. After `CLEAN`, compute the r7, review, and scope digests; externally fill only
...
exact r7 SHA and a fresh current-user approval explicitly bound to that review
and the completed §5 question. The approval recorded against r4 is void and
cannot be substituted. A failed precondition, changed r7 or review byte,
