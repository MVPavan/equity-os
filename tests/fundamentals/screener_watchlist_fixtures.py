...
live surface was verified to have — one ``tbody`` whose header row repeats every
sixteen data rows, an ``S.No.`` header spanning two columns, a notebook button
cell with no header of its own, value headers whose ``data-tooltip`` is the full
metric name, four company-link shapes, a page-embedded export form, and a CSV
whose rows are alphabetical rather than in watchlist order — with invented
names, codes, ids and numbers throughout.
...
Fixtures are self-checking. A builder that claims to make N rows asserts that
its own markup parses to N, because a Slice 3 fixture once produced zero rows
for months without any test noticing.
"""
...
# Two distinct synthetic tokens. The contract forbids ever using the form token
# as the cookie value, so the two must differ for a test to tell them apart.
...
def dropdown(names: tuple[str, ...], *, selected: int | None = 0) -> str:
    ...
    return f'<ul class="dropdown-content">{"".join(entries)}</ul>'
